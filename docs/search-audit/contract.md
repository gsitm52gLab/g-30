...
`searchTransaction`은 G14 조회 한 번의 SERIALIZABLE UoW 안에서 요청된 `get(kind,id)`와 `list(kind,context)`의 원본 행만 lazy 재사용한다. 명시적으로 요청되지 않은 kind는 미리 읽지 않는다. 반환은 항상 깊은 복사이며 create/update는 거부하고 callback 종료 시 view를 만료시킨다. 권한 판단과 clock은 매번 실행하고 요청 간 값/판단은 재사용하지 않는다. 같은 트랜잭션의 일관된 snapshot 의미를 유지하며 새 요청은 변경된 사용자·membership·원본 권한을 다시 DB에서 읽는다. 실제 동시 철회 UI는 기존 late-response 검증을 유지한다.
