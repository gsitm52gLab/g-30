...
BIGINT values arrive from pg as decimal strings and are validated before conversion to a JavaScript number. Revisions through `9007199254740991` are exact; fractional, zero, negative, malformed or unsafe values are rejected. CAS crosses the signed 32-bit boundary correctly. Updating the final safe revision is rejected atomically instead of rounding or silently overwriting.
