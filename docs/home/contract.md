...
- 브랜드 약기법 사전검토 메뉴는 기존 /ai-input의 문안·파일 입력 흐름, GSG는 /ai-review의 내부 검토 흐름에 연결한다. 브랜드 내부 결과 접근 거부는 기존 서버 정책을 유지한다. 홈의 업무 링크는 자동 prefetch 없이 실제 선택 시 조회해 원격 DB 요청 폭주를 피한다.
- PR·행사 신규 /campaigns 목록은 현재 task 및 campaign 권한으로 기존 /tasks/[id]/campaigns?campaign=<id>의 정확한 행사 상세에 연결한다. 운영 설정은 최소 role API가 GSG임을 확인한 경우만 보인다. 각 실제 서버 API가 권한을 다시 확인하므로 메뉴 숨김 자체를 권한 검증으로 취급하지 않는다.
...
부분 검증 이후에는 `HOME_CONTINUE_FROM=/absolute/private/previous/result.json`을 같은 명령에 추가할 수 있다. PG01~04 PASS와 저장된 fixture를 검증하고 새 로그인으로 UI·재시작·철회만 이어간다. 이전 실패 결과는 보존하며 새 출력 경로를 지정한다.
