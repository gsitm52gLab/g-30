...
- **신규 입점·스팟 업무:** 요청 설명·참고자료·필수/선택 응답·마감, 프로젝트·선행 관계, 주/공동 담당, 공개 요청 버전과 변경 이력. 업무 등록·변경 저장 즉시 지정 브랜드에 요청이 전달됩니다. 기존에 초안으로 저장된 업무는 해당 업무에서 ‘브랜드에 요청 보내기’를 눌러 전달합니다.
...
- **자료·Excel:** 원본과 참조 양측 권한, 정확한 과거 FileVersion, 시트·열 매핑·검증·미리보기·원자 적용·내보내기. 잘못된 숫자·수식·외부 링크 등은 명시적으로 검증합니다.
...
[데모 시나리오와 발표 원고](docs/demo/demo-script.md)는 상품정보부터 요청·부분/전체 제출·문의·AI·수동 완료까지 이어집니다. 현재 최종 통합 리허설 전이며, 실제 수행 범위는 최종 검증 패킷에서 확인해야 합니다.
