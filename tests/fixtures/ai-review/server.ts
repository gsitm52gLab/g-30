...
export async function actualSubmission(identity: IdentityService) {
    const tasks = new TaskService(identity), content = blankContent();
    content.title = 'G16 실제 제출 출처';
    content.description = '일본어 합성 문안의 실제 제출';
    content.deadline.responsibleUserId = 'user-gsg';
    content.requirements = [{ ...blankRequirement('claim', 'long_text'), label: '일본어 문안' }];
    ...
    const submissions = new SubmissionService(identity);
...
export function revisedCorpus() {
    const c = curatedRelease();
    c.id = 'synthetic-corpus-revision-2';
    const source = c.sources.find(s => s.sourceId === 'mhlw2017')!, previous = source.id;
    source.id += '-test2';
    source.originalHash = sha256('synthetic source revision');
    source.revision = 'SYNTHETIC_REVISION_NOT_LEGAL';
    for (const e of c.excerpts.filter(e => e.sourceVersionId === previous))
        e.sourceVersionId = source.id;
    for (const t of c.translations.filter(t => t.sourceVersionId === previous)) {
        t.id += '-test2';
        t.sourceVersionId = source.id;
        t.sourceHash = source.originalHash;
    }
