...
export async function completionCampaign(identity: IdentityService, general = false) {
    ...
    async function command(command: string, extra: Record<string, unknown> = {}, token = admin) { const row = await campaigns.detail(token, campaignId); return campaigns.command(token, { command, contextId, taskId, campaignId, expectedRevision: row.revision, idempotencyKey: randomUUID(), ...extra }); }
    const versionId = (await command('publish')).ids[1];
    async function select(response = 'participate') { return command('participate', { campaignVersionId: versionId, response, selectedMenus: response === 'participate' ? [draft.menus[0].identity] : [], providedBy: person, note: '' }, brand); }
