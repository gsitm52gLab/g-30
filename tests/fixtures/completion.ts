...
export async function completionSubmission(identity: IdentityService, directory: string, taskId: string) {
    const service = new SubmissionService(identity);
    let workspace = await service.workspace(brand, taskId);
    ...
    if (upload.items[0].state !== 'ready')
        throw Error('actual fixture upload failed');
    const fileId = upload.items[0].file.id;
    ...
    workspace = await service.workspace(brand, taskId);
    const id = (await service.submit(brand, taskId, { baseRequestId: workspace.request.id, expectedDraftRevision: workspace.draft!.revision, expectedTaskRevision: workspace.taskRevision, mode: 'partial', idempotencyKey: randomUUID() })).ids[0];
...
export async function completionInquiry(identity: IdentityService, taskId: string, actor = brand, text = '외부 회신이 필요한 질문') {
    ...
    const d = await service.detail(admin, draft.conversationId);
    if (d.phase !== 'active')
        throw Error('active inquiry required');
