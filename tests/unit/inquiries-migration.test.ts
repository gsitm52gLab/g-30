...
it('G09 migration preserves populated accepted G08 notice and all old rows; repeat migration and seed write no rows',async()=>{
    ...
    try{
        for(const name of readdirSync(source).filter(n=>n.endsWith('.sql')&&!n.startsWith('0008-')))copyFileSync(path.join(source,name),path.join(dir,name));
        expect(migrate(db,dir)).toEqual({applied:6,total:6});repo=createSqliteRepository(db);const identity=await policyFixture(repo),notice=new NoticeService(identity),admin=tokenFor('user-admin');
        ...
        const before=db.prepare('SELECT * FROM records ORDER BY kind,id').all();expect(migrate(db)).toEqual({applied:1,total:7});expect(db.prepare('SELECT * FROM records ORDER BY kind,id').all()).toEqual(before);
        expect(migrate(db)).toEqual({applied:0,total:7});expect((await seed(repo)).inserted).toBe(0);expect(db.prepare('SELECT * FROM records ORDER BY kind,id').all()).toEqual(before);
