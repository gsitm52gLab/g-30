...
it('G09 migration preserves populated accepted G08 notice and all old rows; repeat migration and seed write no rows',async()=>{
    ...
    try{
        ...
        const before=db.prepare('SELECT * FROM records ORDER BY kind,id').all();expect(migrate(db)).toEqual({applied:1,total:8});expect(db.prepare('SELECT * FROM records ORDER BY kind,id').all()).toEqual(before);
        expect(migrate(db)).toEqual({applied:0,total:8});expect((await seed(repo)).inserted).toBe(0);expect(db.prepare('SELECT * FROM records ORDER BY kind,id').all()).toEqual(before);
