...
it('G09 migration preserves populated accepted G08 notice and all old rows; repeat migration and seed write no rows', async () => {
    const dir = mkdtempSync(path.join(tmpdir(), 'g09-migrate-')), db = openDatabase(':memory:', true), source = path.resolve('src/server/db/migrations');
    let repo: ReturnType<typeof createSqliteRepository> | undefined;
    try {
        for (const name of readdirSync(source).filter(n => n.endsWith('.sql') && Number(n.slice(0, 4)) <= 7))
            copyFileSync(path.join(source, name), path.join(dir, name));
        expect(migrate(db, dir)).toEqual({ applied: 7, total: 7 });
        repo = createSqliteRepository(db);
        const identity = await policyFixture(repo), notice = new NoticeService(identity), admin = tokenFor('user-admin');
