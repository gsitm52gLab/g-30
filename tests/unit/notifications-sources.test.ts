...
for (const mode of ['mock', 'sqlite'] as const) describe(`${mode} G13 actual producer source adapters`, () => {
    ...
    async function setup() {
        ...
        identity = await policyFixture(repo); dir = await mkdtemp(join(tmpdir(), 'g13-source-'));
        const c = blankContent(); c.title = 'G13 실제 공개 일정'; c.description = '공개 요청'; c.internalOriginal = 'PRIVATE_G13_ORIGINAL'; c.internalMemo = 'PRIVATE_G13_MEMO';
    ...
    async function read<T>(token: string, fn: (s: UnitOfWork, p: ReturnType<IdentityService['principal']>) => T) { return repo.transaction(s => fn(s, identity.principal(s, token))); }
    ...
    async function event(token: string, type: string, target = taskId) { const rows = await events(type, target); expect(rows.length).toBeGreaterThan(0); return read(token, (s, p) => notificationEventSource(s, p, rows.at(-1)!.id, () => NOW)); }
    async function schedules(token = brand, id = taskId) { return read(token, (s, p) => taskSchedules(s, p, id, () => NOW)); }
    ...
    it('S13-01 actual G04 public request/current recipients, private milestone exclusion, unknown fields never spread', async () => {
        await setup(); const published = (await events('TASK_PUBLISHED'))[0], raw = await repo.get('domainEvent', published.id);
        await repo.transaction(s => { const t = s.get('task', taskId)!; s.update('task', t.id, t.revision, { ...t.data, privateExtra: 'PRIVATE_G13_EXT' } as typeof t.data); });
        ...
        expect(b).toMatchObject({ disposition: 'eligible', recipientId: 'user-co' }); expect(non.disposition).toBe('other_recipient');
        const rows = await schedules(); expect(rows).toHaveLength(2); expect(await schedules(admin)).toHaveLength(3);
        expect(rows[0].need).toMatchObject({ remaining: 2, required: true }); expect(sourceReminderDecision(rows[0], NOW)).toMatchObject({ eligible: true, calendar: { basis: 'tentative' } });
        expect(JSON.stringify([a, rows])).not.toContain('PRIVATE_G13'); expect(await repo.get('domainEvent', published.id)).toEqual(raw);
    });
    it('S13-02 actual G05 partial submission routes to owner and updates current missing without consuming read/draft state', async () => {
        await setup(); const before = await schedules(); expect(before[0].need).toMatchObject({ remaining: 2 });
        ...
        expect(await repo.get('submission', target.submissionId)).toEqual(sub); expect(await repo.list('submissionDraft')).toEqual(draft);
    });
    it('S13-03 current session/membership authority defeats previously captured principal and wrong context', async () => {
        await setup(); const id = (await events('TASK_PUBLISHED'))[0].id;
        ...
        await expect(repo.transaction(s => notificationEventSource(s, old, id, () => NOW))).rejects.toMatchObject({ status: 404 });
    ...
    it('S13-05 G08 draft0, exact published recipients/current AND historical audience, read state untouched', async () => {
        ...
        const id = (await notices.create(admin, { contextId, content: c, idempotencyKey: randomUUID() })).ids[0]; expect(await events('NOTICE_PUBLISHED', id)).toHaveLength(0);
        await notices.command(admin, id, { command: 'publish', expectedRevision: 1, idempotencyKey: randomUUID() });
        const old = (await events('NOTICE_PUBLISHED', id))[0], reads = await repo.list('noticeRead'); expect((await event(brand, 'NOTICE_PUBLISHED', id)).disposition).toBe('eligible');
        ...
        await expect(read(brand, (s, p) => notificationEventSource(s, p, old.id, () => NOW))).rejects.toMatchObject({ status: 404 });
        expect((await event(co, 'NOTICE_REVISED', id)).disposition).toBe('eligible'); expect(await repo.list('noticeRead')).toEqual(reads);
    });
    it('S13-06 actual G09 private draft/internal note0; question→GSG, public answer→initiator, external date GSG-only', async () => {
        await setup(); const inquiries = new InquiryService(identity), r = await completionInquiry(identity, taskId);
        expect((await event(gsg, 'INQUIRY_PUBLISH_FIRST', r.conversationId)).disposition).toBe('eligible');
        expect((await read(brand, (s, p) => inquirySchedules(s, p, r.conversationId, () => NOW)))).toEqual([]);
        const rows = await read(gsg, (s, p) => inquirySchedules(s, p, r.conversationId, () => NOW)); expect(rows).toHaveLength(1); expect(rows[0]).toMatchObject({ recipientState: 'current_recipient', source: { versionId: null, itemKey: r.questionId }, deadline: { value: '2026-10-05', timezone: 'Asia/Tokyo' } });
        const before = (await repo.list('domainEvent')).length;
        await inquiries.command(admin, r.conversationId, { command: 'internal_note', questionId: r.questionId, content: { clientMessageId: randomUUID(), body: 'PRIVATE_G13_NOTE', fileVersionIds: [] }, idempotencyKey: randomUUID() }); expect(await repo.list('domainEvent')).toHaveLength(before);
        const detail = await inquiries.detail(admin, r.conversationId); if (detail.phase !== 'active') throw Error('fixture');
        ...
        expect(await read(gsg, (s, p) => inquirySchedules(s, p, r.conversationId, () => NOW))).toEqual([]);
    });
    it('S13-07 actual G10 internal opinions3/draft0 then one public batch facts per recipient, private originals0', async () => {
        await setup(); const target = await completionSubmission(identity, dir, taskId), corrections = new CorrectionService(identity), items = [];
        ...
        const rows = await Promise.all([event(brand, 'CORRECTION_BATCH_PUBLISHED'), event(co, 'CORRECTION_BATCH_PUBLISHED')]); expect(rows.map(r => r.disposition)).toEqual(['eligible', 'eligible']); expect(rows[0].source.eventId).toBe(rows[1].source.eventId); expect(JSON.stringify(rows)).not.toContain('PRIVATE_G13');
    });
    it('S13-08 actual G12 selected/decline/cancellation preserve facts and no brand nag; general requirement survives decline', async () => {
        ...
        const noMaterials = (await schedules(brand, f.taskId))[0]; expect(noMaterials.need).toMatchObject({ remaining: 0, required: false }); expect(sourceReminderDecision(noMaterials, NOW).eligible).toBe(false);
        expect((await read(brand, (s, p) => campaignSchedules(s, p, f.campaignId, () => NOW))).every(r => !r.active)).toBe(true); expect(await repo.list('campaignExternalFact')).toEqual(records);
        const general = await completionCampaign(identity, true); await general.select(); await general.applied(); await general.select('decline'); expect((await schedules(brand, general.taskId))[0].need).toMatchObject({ required: true, remaining: 1 });
    });
    it('S13-09 actual G11 complete/reopen evaluates live task need while immutable completion snapshot stays intact', async () => {
        await setup(); expect(sourceReminderDecision((await schedules())[0], NOW).eligible).toBe(true);
        const id = await complete(), snapshot = await repo.get('completionSnapshot', id);
        expect(sourceReminderDecision((await schedules())[0], NOW)).toEqual({ eligible: false, reason: 'task_inactive' }); expect((await event(brand, 'TASK_MANUALLY_COMPLETED')).disposition).toBe('eligible');
        ...
        expect(sourceReminderDecision((await schedules())[0], NOW).eligible).toBe(true); expect((await event(brand, 'TASK_REOPENED')).disposition).toBe('eligible'); expect(await repo.get('completionSnapshot', id)).toEqual(snapshot);
    });
    it('S13-10 missing assignee has explicit needs_assignment; source adapters never write events/read markers', async () => {
        await setup(); for (const uid of ['user-luna', 'user-co']) await repo.transaction(s => { const m = s.list('membership', contextId).find(m => m.data.userId === uid)!; s.update('membership', m.id, m.revision, { ...m.data, status: 'suspended' }); });
        ...
        expect((await schedules(admin))[0].recipientState).toBe('needs_assignment'); expect((await event(admin, 'TASK_PUBLISHED')).disposition).toBe('needs_assignment');
