...
for (const mode of ["mock", "sqlite"] as const) {
  describe(`${mode} shared repository contract`, () => {
    ...
    function create() { if (mode === "mock") repository = createMockRepository(clock); else { const db = openDatabase(":memory:", true); migrate(db); repository = createSqliteRepository(db, clock); } return repository; }
    ...
    it("rejects duplicates and stale revisions", async () => {
      const repo = create(); const input = { id: "one", contextId: null, data: { value: "initial" } };
    ...
    it("rejects async transaction callbacks and blocks escaped writes", async () => {
      const repo = create(); let escaped: UnitOfWork | undefined;
      ...
      await Promise.resolve(); expect(await repo.list("checkpoint")).toEqual([]);
    ...
    it("seeds idempotently and preserves existing edits and extra records", async () => {
      const repo = create(); expect((await seed(repo)).inserted).toBe(fixtures.length);
      ...
      expect((await repo.get("task", "task-pop"))?.data.title).toBe("User edited title"); expect(await repo.get("checkpoint", "extra")).not.toBeNull();
    });
    it("has explicit empty and invalid-record behavior", async () => {
      const repo = create(); expect(await repo.list("task")).toEqual([]); expect(await repo.get("task", "unknown")).toBeNull();
...
describe("migration and persistence setup", () => {
  it("migrates an empty DB twice without losing writes", () => {
    ...
    try { expect(migrate(db)).toEqual({ applied: 1, total: 1 }); db.prepare("INSERT INTO records VALUES ('checkpoint','preserved',NULL,'{}',1,'now','now')").run(); expect(migrate(db)).toEqual({ applied: 0, total: 1 }); expect(db.prepare("SELECT id FROM records").all()).toEqual([{ id: "preserved" }]); } finally { db.close(); }
  });
  ...
  it("gives mock and SQLite the exact same seeded domain shape", async () => {
    const mock = createMockRepository(clock); const db = openDatabase(":memory:", true); migrate(db); const sqlite = createSqliteRepository(db, clock);
    try { await seed(mock); await seed(sqlite); for (const kind of ["context", "user", "task", "product"] as const) expect(await sqlite.list(kind)).toEqual(await mock.list(kind)); } finally { mock.close(); sqlite.close(); }
  });
});
