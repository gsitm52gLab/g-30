...
for (const mode of ['mock', 'sqlite'] as const)
    describe(`${mode} G13 persisted server`, () => {
        ...
        async function setup() {
            ...
            await policyFixture(repo);
            identity = new IdentityService(repo, () => now);
            await repo.transaction(async (s) => { for (const r of (await s.list('session')))
                (await s.update('session', r.id, r.revision, { ...r.data, expiresAt: '2026-12-01T00:00:00Z' })); });
            ...
            const task = new TaskService(identity);
            taskId = (await task.create(admin, { category: 'spot', content: c, targets: [{ contextId, ownerId: 'user-gsg', assigneeId: 'user-luna', coAssigneeIds: ['user-co'], productIds: ['product-serum'] }], idempotencyKey: randomUUID() })).ids[0];
        ...
        it('N13-01 AC13-01 manual immutable versions/conflicts/CAS/same-intent/private projection/invalid date', async () => {
            await setup();
            const service = new SchedulingService(identity), original = content({ statements: [{ id: 'a', raw: ' 9/23 수요일 원문 ', source: '원문 A', version: 'v1', locator: 'p1' }, { id: 'b', raw: '9/24 목요일', source: '원문 B', version: 'v2', locator: 'p2' }], conflicts: [{ id: 'c', statementIds: ['a', 'b'], state: 'unresolved', resolution: '' }] });
            const input = save(original), result = await service.command(admin, input);
            expect(await service.command(admin, input)).toEqual(result);
            let detail = await service.detail(gsg, result.ids[0]);
            expect(detail.current.content.statements[0].raw).toBe(' 9/23 수요일 원문 ');
            expect(detail.calendar.reminder).toMatchObject({ eligible: false, reason: 'unresolved_conflict' });
            ...
            detail = await service.detail(admin, result.ids[0]);
        ...
        it('N13-02 AC13-03 actual event + recipient atomic sync idempotency and isolated notification read', async () => {
            ...
            expect(await repo.list('notificationAttempt')).toHaveLength(2);
        ...
        it('N13-03 AC13-02/03 publication canonical same recipient1; selection/fact actual request keeps brand alerts', async () => {
            await setup();
            const f = await completionCampaign(identity), service = new NotificationService(identity);
            for (const token of [brand, co]) {
                ...
                expect(notices[0].source.kind === 'event' && (await repo.get('domainEvent', notices[0].source.eventId))?.data.eventType).toBe('CAMPAIGN_PUBLISHED');
            }
            ...
            for (const token of [brand, co, gsg])
                await service.sync(token, contextId);
        ...
        it('N13-04 AC13-03 fault after notification rolls back, failure retry persists exact one notice+receipt', async () => {
            await setup();
            const broken = new NotificationService(identity, { fault: () => { throw Error('synthetic transaction failure'); } });
            const failed = await broken.sync(brand, contextId);
            expect(failed.failed).toBe(2);
            expect(await repo.list('notification')).toHaveLength(0);
            expect(await repo.list('notificationReceipt')).toHaveLength(0);
            expect(failed.failures).toHaveLength(2);
            await broken.sync(brand, contextId);
            expect(await repo.list('notificationAttempt')).toHaveLength(2);
            const healthy = new NotificationService(identity);
            for (const item of failed.failures) {
                await healthy.retry(brand, item.id);
                await healthy.retry(brand, item.id);
            }
            expect(await repo.list('notification')).toHaveLength(2);
            expect(await repo.list('notificationReceipt')).toHaveLength(2);
            expect((await healthy.list(brand, contextId)).failures).toHaveLength(0);
        });
        ...
        it('N13-06 AC13-01/04 D-2/day/overdue only current local day; completion and reopen reevaluate without catchup', async () => {
            await setup();
            const service = new NotificationService(identity);
            await service.sync(brand, contextId);
            ...
            await service.sync(brand, contextId);
            expect((await repo.list('notification')).filter(n => n.data.source.kind === 'reminder')).toHaveLength(3);
            const task = (await repo.get('task', taskId))!;
            await complete.command(admin, { command: 'reopen', taskId, completionId: done.ids[0], expectedTaskRevision: task.revision, reason: '자료 보완', idempotencyKey: randomUUID() });
            await service.sync(brand, contextId);
        ...
        it('N13-08 SA-52 explicit task-assigned brand shipping gets next action; arbitrary team member rejected', async () => {
            await setup();
            const scheduling = new SchedulingService(identity), notifications = new NotificationService(identity);
        ...
        it('N13-09 SA-52 submission recipients are current task assignees; confirmation actor policy is explicit', async () => {
            await setup();
            const scheduling = new SchedulingService(identity), id = (await scheduling.command(admin, save(content({ kind: 'submission' })))).ids[0];
            for (const token of [brand, co])
                expect((await scheduling.detail(token, id)).calendar).toMatchObject({ recipientPolicy: 'task_assignees', recipientState: 'current_recipient', deadline: { responsibleUserId: 'user-gsg' } });
        ...
        it('N13-10 SA-52 explicit brand operation follows current assignment, completion and campaign nonparticipation', async () => {
            await setup();
            const scheduling = new SchedulingService(identity), notifications = new NotificationService(identity), f = await completionCampaign(identity);
            await f.select();
            ...
            await notifications.sync(co, contextId);
            const baseline = (await repo.list('notification')).filter(n => n.data.source.kind === 'reminder' && n.data.source.logicalKey === `manual:${manual}`).length;
            expect(baseline).toBe(1);
            await f.select('decline');
            now = '2026-09-23T12:00:00Z';
            expect((await scheduling.detail(co, manual)).calendar.reminder).toMatchObject({ eligible: false, reason: 'participation_inactive' });
            await notifications.sync(co, contextId);
            ...
            const completion = new CompletionService(identity), w = await completion.workspace(admin, taskId);
            await completion.command(admin, { command: 'complete', taskId, expectedTaskRevision: w.taskRevision, expectedBasisHash: w.preview!.basisHash, memo: '', idempotencyKey: randomUUID() });
            expect((await scheduling.detail(co, ordinary)).calendar.reminder).toMatchObject({ eligible: false, reason: 'task_inactive' });
            const current = (await repo.get('task', f.taskId))!;
            await new TaskService(identity).command(admin, f.taskId, { command: 'assign', expectedRevision: current.revision, assignment: { ownerId: 'user-gsg', assigneeId: 'user-luna', coAssigneeIds: [] }, idempotencyKey: randomUUID() });
        ...
        it('N13-11 AC13-02/03 real notice and multi-item correction draft0/public recipient1, read markers unchanged', async () => {
            await setup();
            directory = await mkdtemp(join(tmpdir(), 'g13-notification-'));
            const notifications = new NotificationService(identity), notices = new NoticeService(identity), corrections = new CorrectionService(identity);
            ...
            await notifications.sync(brand, contextId);
            expect((await notifications.list(brand, contextId)).items.some(n => ['공지 수신', '공개 보완 묶음'].includes(n.title))).toBe(false);
            await notices.command(admin, noticeId, { command: 'publish', expectedRevision: 1, idempotencyKey: randomUUID() });
            await corrections.command(admin, { command: 'publish', taskId, draftId: draft, expectedRevision: 1, idempotencyKey: randomUUID() });
            for (const token of [brand, co]) {
                await notifications.sync(token, contextId);
                await notifications.sync(token, contextId);
                const list = await notifications.list(token, contextId);
                expect(list.items.filter(n => n.title === '공개 보완 묶음')).toHaveLength(1);
                expect(list.items.filter(n => n.title === '공지 수신')).toHaveLength(token === brand ? 1 : 0);
            }
            expect(await repo.list('noticeRead')).toHaveLength(0);
            expect(JSON.stringify(await notifications.list(brand, contextId))).not.toContain('PRIVATE_NOTIFICATION');
        });
        it.each(['task', 'campaign'] as const)('N13-12 SA-52 actual %s brand date confirmer does not replace GSG next-action owner', async (producer) => {
            await setup();
            const tasks = new TaskService(identity), scheduling = new SchedulingService(identity);
            ...
            if (producer === 'task') {
                ...
                await f.command('save', { draft });
                const versionId = (await f.command('publish')).ids[1];
            ...
            await repo.transaction(async (s) => { const m = (await s.list('membership', contextId)).find(m => m.data.userId === 'user-gsg')!; (await s.update('membership', m.id, m.revision, { ...m.data, status: 'suspended' })); });
        ...
        it('N13-07 actual G04 activity identity and external wait GSG-only source remain separate', async () => {
            ...
            const e = (await repo.list('domainEvent')).find(e => e.data.eventType === 'TASK_SCHEDULE_CHANGE_REQUESTED')!;
            expect((await repo.get('taskActivity', e.data.sourceVersionId!))?.data.kind).toBe('schedule');
            await completionInquiry(identity, taskId);
            now = '2026-10-03T12:00:00Z';
            const service = new NotificationService(identity);
            await service.sync(gsg, contextId);
            await service.sync(brand, contextId);
            const external = (await repo.list('notification')).filter(n => n.data.source.kind === 'reminder' && n.data.source.source.kind === 'inquiry_external');
            expect(external.map(n => n.data.recipientId)).toEqual(['user-gsg']);
        });
    });
