...
for (const mode of ["mock", "sqlite"] as const)
    describe(`${mode} G06 contract review regressions`, async () => {
        ...
        async function setup() {
            ...
            products = new ProductService(identity);
        }
        ...
        it("explicit internal upload capability includes nonprice GSG and excludes brand without exposing a price capability", async () => {
            ...
            expect(gsg.capabilities.uploadInternalFile).toBe(true);
            expect(gsg.capabilities.editInternalPrice).toBe(false);
        ...
        it("CR04 legacy product view carries the resolved context while the common record stays context-null", async () => {
            ...
            for (const contextId of [A, "ctx-jp-b-luna"]) {
                const view = await repo.transaction(async (s) => (await projectProduct(s, (await identity.principal(s, brand)), (await s.get("product", "product-serum"))!, () => NOW, contextId)));
        ...
        it("R1 rejects nonfinite/noninteger/nonpositive pagination rather than serializing null or silently changing request", async () => {
            ...
            for (const value of ["Infinity", "-Infinity", "NaN", "bad", "0", "-1", "1.5", "1e100", ""]) {
                await expect((await products.list(brand, { page: value }))).rejects.toMatchObject({ status: 422 });
                await expect((await products.list(brand, { pageSize: value }))).rejects.toMatchObject({ status: 422 });
            }
        ...
        it("R2 persisted receipt unknown/nested result extensions are excluded from idempotent response without mutating receipt", async () => {
            ...
            const receipt = await repo.transaction(async (s) => (await s.create("commandReceipt", { id: randomUUID(), contextId: A, data: { key, actorId: "user-luna", command, bodyHash, result: { ids: ["product-serum", { private: marker }], extra: { private: marker } } as unknown as {
                        ids: string[];
                    } } })));
        ...
        it("R3 public task file must belong to a published request before cross-owner product bind/download/capture; own draft publication still works", async () => {
            ...
            const taskCommand = async (command: string, extra: Record<string, unknown> = {}) => (await tasks.command(admin, tid, { command, expectedRevision: (await repo.get("task", tid))!.revision, idempotencyKey: randomUUID(), ...extra }));
            ...
            await expect((await files.download(brand, file.id, tid, "download"))).rejects.toMatchObject({ status: 404 });
            for (const token of [brand, team, admin]) {
                ...
                await expect((await products.command(token, "product-serum", { contextId: A, command: "save_files", files: [blankFileBinding("unpublished", file.id)], expectedContextRevision: d.contextRevision, idempotencyKey: randomUUID() }))).rejects.toMatchObject({ status: 422 });
            }
            // Stored prior malformed binding is a contract fixture, not a permitted producer.
            await repo.transaction(async (s) => { const cp = (await s.list("contextProduct", A)).find(r => r.data.productId === "product-serum")!, old = (await s.get("contextProductVersion", cp.data.currentVersionId!))!; const v = (await s.create("contextProductVersion", { id: randomUUID(), contextId: A, data: { ...old.data, sequence: old.data.sequence + 1, previousId: old.id, files: [blankFileBinding("unpublished", file.id)] } })); (await s.update("contextProduct", cp.id, cp.revision, { ...cp.data, currentVersionId: v.id })); });
            const d = await products.detail(brand, "product-serum", A);
            expect(d.files).toEqual([]);
            await expect((await files.download(brand, file.id, { kind: "product", contextId: A, productId: "product-serum" }, "original"))).rejects.toMatchObject({ status: 422 });
            await expect(repo.transaction(async (s) => (await captureProductUse(s, (await identity.principal(s, brand)), { contextId: A, productId: "product-serum", expectedCommonRevision: d.commonRevision, expectedContextRevision: d.contextRevision, bindingIds: ["unpublished"], retailPriceVersionId: null, asOfDate: "2026-09-21", ownerType: "prior_use_fixture", ownerId: "prior-r3", taskId: null, requestId: null }, () => NOW)))).rejects.toMatchObject({ status: 422 });
