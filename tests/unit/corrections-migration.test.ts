...
it('G10 populated accepted six migrations preserve all old business rows and actual file bytes; repeat migration/seed no writes', async () => {
    const dir = mkdtempSync(path.join(tmpdir(), 'g10-migration-')), sql = path.join(dir, 'sql');
    const fs = await import('node:fs');
    fs.mkdirSync(sql);
    const db = openDatabase(':memory:', true), source = path.resolve('src/server/db/migrations');
    let repo: ReturnType<typeof createSqliteRepository> | undefined;
    try {
        for (const name of readdirSync(source).filter(n => n.endsWith('.sql') && (Number(n.slice(0, 4)) <= 5 || Number(n.slice(0, 4)) === 7)))
            copyFileSync(path.join(source, name), path.join(sql, name));
        expect(migrate(db, sql)).toEqual({ applied: 6, total: 6 });
