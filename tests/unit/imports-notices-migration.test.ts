...
it.each([6, 7])('populated chain through module 000%i gains missing sibling and inquiries; rows/checksums/files/replays remain exact', async firstModule => {
    ...
    expect(names).toEqual(['0001-foundation.sql', '0002-identity.sql', '0003-tasks.sql', '0004-products.sql', '0005-submissions.sql', '0006-evidence-imports.sql', '0007-notices.sql', '0008-inquiries.sql', '0009-corrections.sql']);
    ...
    try {
        ...
        expect(migrate(db)).toEqual({ applied: 3, total: 9 });
        ...
        console.info('G07_UNION_MIGRATION_EVIDENCE ' + JSON.stringify({ firstModule, missingAdded: [firstModule === 6 ? '0007-notices.sql' : '0006-evidence-imports.sql', '0008-inquiries.sql', '0009-corrections.sql'], priorRecords: before.length, priorRowsSha256: hash(JSON.stringify(before)), fileSha256: fileHash, migrations: allMigrations, rowsUnchanged: true, receiptReplayUnchanged: true, bothModuleProducersAndImmutableGuards: true }));
