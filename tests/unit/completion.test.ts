...
for (const mode of ['mock', 'sqlite'] as const)
    describe(`${mode} actual G11 producer integration`, () => {
        ...
        it('AC11-01/02 actual partial, product use, G09 external wait and unresolved G10 remain; memo/proof absent still completes', async () => {
            ...
            for (const token of [brand, team])
                await expect(service.command(token, await completeInput())).rejects.toMatchObject({ status: 403 });
            await expect(service.command('ai-untrusted-no-session', await completeInput())).rejects.toMatchObject({ status: 401 });
        ...
        it('AC11-02 basis detects actual G09 change without task revision; preview is pure and stale complete has no writes', async () => {
            ...
            await expect(service.command(admin, cmd)).rejects.toMatchObject({ code: 'BASIS_CHANGED', status: 409 });
        ...
        it('AC11-03 same exact file two purposes and external performer distinct recorder; no external/task state changed by record', async () => {
            ...
            await expect(service.command(admin, { ...input, action: { ...input.action, source: { ...input.action.source, submissionContentHash: 'f'.repeat(64) } }, idempotencyKey: randomUUID() })).rejects.toMatchObject({ status: 404 });
        ...
        it('AC11-04 reasoned reopen, second completion and real followup link keep old immutable snapshots; direct publish cannot bypass reopen', async () => {
            ...
            await expect(service.command(admin, { command: 'reopen', taskId, completionId: c, expectedTaskRevision: w.taskRevision, reason: '', idempotencyKey: randomUUID() })).rejects.toMatchObject({ status: 422 });
        ...
        it('AC11-05 same intent yields one event/receipt; distinct stale CAS loses; every late fault rolls all records back', async () => {
            ...
            await expect(new CompletionService(identity, () => { throw Error('late complete'); }).command(admin, cmd)).rejects.toThrow('late complete');
            expect(await rows()).toEqual(before);
            const both = await Promise.all([service.command(admin, cmd), service.command(admin, cmd)]);
            ...
            await expect(service.command(admin, { ...cmd, idempotencyKey: randomUUID() })).rejects.toMatchObject({ status: 409 });
            ...
            await expect(new CompletionService(identity, () => { throw Error('late reopen'); }).command(admin, reopen)).rejects.toThrow('late reopen');
            ...
            await expect(service.command(admin, cmd)).rejects.toMatchObject({ status: 401 });
        });
        it('A19 completion history omits other brand inquiry and internal action/count/markers; current revoke denies replay and history', async () => {
            ...
            await expect(service.snapshot(brand, c)).rejects.toMatchObject({ status: 404 });
            await expect(service.workspace(brand, taskId)).rejects.toMatchObject({ status: 404 });
        });
        it('Stored nested unknown extras do not project; malformed known snapshot field safely fails503 and immutable rows remain', async () => {
            ...
            await expect(service.snapshot(admin, bad)).rejects.toMatchObject({ status: 503 });
        ...
        it('D02 after-IO reauthorization rejects downloaded external evidence when membership is revoked', async () => {
            ...
            await expect(new CompletionFiles(new IdentityService(wrapped, () => NOW), directory).download(brand, input.action.source.fileVersionIds[0], new URLSearchParams({ externalActionId: id }))).rejects.toMatchObject({ status: 404 });
            expect(n).toBe(2);
        });
        it('Internal external record uses exact source and existing internal evidence without releasing it to brand', async () => {
            ...
            await expect(service.external(brand, id)).rejects.toMatchObject({ status: 404 });
            const c = (await service.command(admin, await completeInput())).ids[0];
        ...
        it('A20 late external/followup writes roll back and failed association keeps actual created followup ID for retry', async () => {
            ...
            await expect(faulty.command(admin, input)).rejects.toThrow('late completion command');
            expect(await rows()).toEqual(before);
            ...
            await expect(faulty.command(admin, cmd)).rejects.toThrow('late completion command');
        ...
        it('Current request and product changes stay separate from original submission/product-use and block stale preview', async () => {
            ...
            await expect(service.command(admin, preview)).rejects.toMatchObject({ status: 409 });
            ...
            await expect(service.command(admin, currentPreview)).rejects.toMatchObject({ code: 'BASIS_CHANGED' });
