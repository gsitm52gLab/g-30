...
for (const mode of ['mock', 'sqlite'] as const)
    describe(`${mode} actual G16 server contracts (synthetic engine)`, async () => {
        ...
        it('GSG-only every read/write/raw/corpus path, other context and current revocation; G15 brand input stays readable', async () => {
            ...
            for (const actor of [brand, tokenFor('user-wave'), tokenFor('user-team')]) {
                await expect((await service.list(actor, ctx))).rejects.toMatchObject({ status: 404 });
                await expect((await service.workspace(actor, f.input.id))).rejects.toMatchObject({ status: 404 });
                await expect((await service.detail(actor, r.runId))).rejects.toMatchObject({ status: 404 });
                await expect((await service.raw(actor, r.runId))).rejects.toMatchObject({ status: 404 });
                await expect((await service.corpus(actor, ctx))).rejects.toMatchObject({ status: 404 });
                await expect((await service.start(actor, f.input.id, f.body))).rejects.toMatchObject({ status: 404 });
            }
            ...
            await expect((await service.detail(staff, r.runId))).rejects.toMatchObject({ status: 404 });
        ...
        it('same-intent replay keeps one run/result, new intent needs latest run and never silently selects a provider', async () => {
            ...
            await expect((await service.start(staff, f.input.id, { ...f.body, idempotencyKey: randomUUID() }))).rejects.toMatchObject({ status: 409 });
        ...
        it('one actual claim while engine awaits; current source/role checked again before dispatch and after result', async () => {
            ...
            const pending = (await worker.start(staff, f.input.id, f.body));
            await started;
        ...
        it('role changes during original IO cannot dispatch even if public G15 input remains readable', async () => {
            ...
            await expect((await worker.start(staff, f.input.id, f.body))).rejects.toMatchObject({ status: 404 });
            expect(calls).toBe(0);
        ...
        it('review accept/edit/reject are explicit append-only CAS facts; receipt retry and injected rollback preserve original', async () => {
            ...
            await expect((await failed.review(staff, r.runId, command))).rejects.toThrow('SYNTHETIC_REVIEW_FAULT');
            ...
            const writes = await Promise.allSettled([(await service.review(staff, r.runId, { ...command, expectedRevision: first.revision, decision: 'edit', editedSuggestion: '사람의 참고 수정안', idempotencyKey: randomUUID() })), (await service.review(admin, r.runId, { ...command, expectedRevision: first.revision, decision: 'reject', idempotencyKey: randomUUID() }))]);
            ...
            await expect((await service.review(brand, r.runId, command))).rejects.toMatchObject({ status: 404 });
        });
        it('deployment revision excludes stale current alignments, preserves historical result and re-seed never resets current head', async () => {
            ...
            expect((await service.start(staff, f.input.id, f.body)).runId).toBe(r.runId);
            await expect((await service.start(staff, f.input.id, { ...f.body, expectedRunId: r.runId, idempotencyKey: randomUUID() }))).rejects.toMatchObject({ code: 'CORPUS_CHANGED' });
        ...
        it('changed corpus during execution and malformed engine output preserve distinct failed attempts, not safe results', async () => {
            ...
            await expect((await worker.start(staff, f.input.id, f.body))).rejects.toMatchObject({ code: 'CORPUS_CHANGED' });
            expect(await repo.list('aiAnalysisResult')).toHaveLength(0);
            ...
            await expect((await broken.start(staff, next.input.id, next.body))).rejects.toMatchObject({ code: 'RESULT_INVALID' });
            const run = (await service.workspace(staff, next.input.id)).runs[0];
            expect(run).toMatchObject({ state: 'failed', issue: 'RESULT_INVALID' });
            await expect((await service.start(staff, next.input.id, { ...next.body, expectedRunId: run.id, idempotencyKey: randomUUID() }))).rejects.toMatchObject({ code: 'RETRY_UNAVAILABLE' });
        });
        it('unknown stored extensions are excluded and malformed known run scalar returns safe error without rewrite', async () => {
            ...
            await expect((await service.detail(staff, r.runId))).rejects.toMatchObject({ code: 'STORAGE_UNAVAILABLE', status: 503 });
        ...
        it('explicit new current basis recovers stale queued work and does not execute an old corpus', async () => {
            ...
            const one = (await worker.start(staff, a.input.id, a.body));
            while (calls < 1)
                await new Promise(r => setTimeout(r, 1));
            const two = (await worker.start(staff, b.input.id, b.body));
            ...
            await expect((await service.start(staff, c.input.id, c.body))).rejects.toMatchObject({ code: 'CORPUS_CHANGED' });
        ...
        it('three failed synthetic attempts have explicit bounded recovery capability and no result', async () => {
            ...
            for (let attempt = 1; attempt <= 3; attempt++) {
                await expect((await broken.start(staff, f.input.id, { ...f.body, expectedRunId: previous, idempotencyKey: randomUUID() }))).rejects.toMatchObject({ code: 'ENGINE_ERROR' });
            ...
            await expect((await service.start(staff, f.input.id, { ...f.body, expectedRunId: previous, idempotencyKey: randomUUID() }))).rejects.toMatchObject({ code: 'RETRY_UNAVAILABLE' });
            expect(await repo.list('aiAnalysisResult')).toHaveLength(0);
        });
        it('actual G05 answer -> exact finding -> deliberate G10 opinion; unrelated associated text cannot invent an answer target', async () => {
            ...
            await expect((await corrections.command(staff, command))).rejects.toMatchObject({ code: 'FINDING_REVIEW_REQUIRED' });
            ...
            await expect((await corrections.command(staff, { ...command, opinion: { ...command.opinion, source: { ...command.opinion.source, runId: other.runId, findingId: 'finding-1' } }, idempotencyKey: randomUUID() }))).rejects.toMatchObject({ code: 'TARGET_MISMATCH' });
        });
        it('actual failed AI changes G11 basisCAS but never gates memo-free completion; brand history is constant and legacy basis survives', async () => {
            ...
            await expect((await new AiReviewService(identity, directory, { analyze: async () => { throw Error('SYNTHETIC_ENGINE_FAILURE'); } }).start(staff, f.input.id, f.body))).rejects.toMatchObject({ code: 'ENGINE_ERROR' });
            ...
            await expect((await completion.command(admin, before))).rejects.toMatchObject({ code: 'BASIS_CHANGED' });
