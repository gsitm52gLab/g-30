    ...
    describe(`G02 ${mode}`, () => {
                ...
                for (const actor of actors)
                    for (const context of contexts) {
                        if (!(actor in expected))
                            await expect(principal(service, s, actor)).rejects.toThrow();
                        else {
                            const p = (await principal(service, s, actor));
                            const inScope = expected[actor].includes(context);
                            expect((await decide(s, p, "context.read", contextResource(context), clock)).allowed).toBe(inScope);
                            expect((await decide(s, p, "product.edit", resource("product", context), clock)).allowed).toBe(inScope);
                            expect((await decide(s, p, "task.manage", resource("task", context), clock)).allowed).toBe(inScope && ["user-admin", "user-selected-admin", "user-gsg", "user-price"].includes(actor));
                            expect((await decide(s, p, "membership.manage", resource("membership", context), clock)).allowed).toBe(inScope && ["user-admin", "user-selected-admin"].includes(actor));
                        }
                    }
            });
        });
        it("T02/T18: explicit current lead/co assignment, historical author and contributor do not grant writes", async () => {
            await repo.transaction(async (s) => {
                ...
                for (const user of ["user-luna", "user-co", "user-team"]) {
                    const p = (await principal(service, s, user));
                    expect((await decide(s, p, "task.read", assigned, clock)).allowed).toBe(true);
                    expect((await decide(s, p, "submission.write", assigned, clock)).allowed).toBe(user !== "user-team");
                    expect((await decide(s, p, "task.complete", assigned, clock)).allowed).toBe(false);
                }
                const unresolved = { ...assigned, unresolvedCount: 3, externalWait: true, aiStatus: "failed" };
                expect((await decide(s, (await principal(service, s, "user-gsg")), "task.complete", unresolved, clock)).allowed).toBe(true);
                expect((await decide(s, (await principal(service, s, "user-luna")), "submission.write", { ...assigned, assigneeUserId: "user-co", coAssigneeUserIds: [] }, clock)).allowed).toBe(false);
            });
        });
        it("SA03: non-assigned brand product edits and independent inquiry remain separate actions", async () => {
            await repo.transaction(async (s) => {
                const p = (await principal(service, s, "user-team"));
                expect((await decide(s, p, "product.edit", resource("product"), clock)).allowed).toBe(true);
                expect((await decide(s, p, "inquiry.create", resource("inquiry"), clock)).allowed).toBe(true);
                expect((await decide(s, p, "task.manage", resource("task"), clock)).allowed).toBe(false);
            });
        });
        it("T03: forged principal role/price flags, unknown action/kind/visibility cannot grant access", async () => {
            await repo.transaction(async (s) => {
                const real = (await principal(service, s, "user-team"));
                ...
                expect((await decide(s, fake, "membership.manage", resource("membership"), clock)).allowed).toBe(false);
                expect((await decide(s, fake, "price.read", resource("product"), clock)).allowed).toBe(false);
                expect((await decide(s, real, "constructor" as Action, resource("task"), clock)).allowed).toBe(false);
                expect((await decide(s, real, "task.read", resource("product"), clock)).allowed).toBe(false);
                expect((await decide(s, real, "task.read", { ...resource("task"), visibility: "unknown" as "public" }, clock)).allowed).toBe(false);
            });
        });
        it("T04/T05: projected task and product omit nested unknown fields and enforce explicit price grant", async () => {
            await repo.transaction(async (s) => {
                for (const user of ["user-luna", "user-gsg", "user-price", "user-selected-admin", "user-admin"]) {
                    const p = (await principal(service, s, user));
                    const task = (await projectTask(s, p, (await s.get("task", "task-onboarding"))!, clock));
                    const product = (await projectProduct(s, p, (await s.get("product", "product-serum"))!, clock));
                ...
                await expect(projectTask(s, (await principal(service, s, "user-wave")), (await s.get("task", "task-onboarding"))!, clock)).rejects.toThrow();
            });
        });
        it("D01 regression/T20: peer grants and unknown audit payloads never pass allowlists", async () => {
            await repo.transaction(async (s) => {
                const user = (await s.get("user", "user-admin"))!;
                ...
                const audit = (await s.create("audit", { id: "audit-projection", contextId: CONTEXT, data: { actorId: "user-admin", action: "membership.changed", targetId: "x", at: NOW,
        ...
        it("T06: original/preview/download need both source and reference scopes; internal stays private", async () => {
            await repo.transaction(async (s) => {
                ...
                for (const action of ["file.original", "file.preview", "file.download"] as const) {
                    expect((await decide(s, p, action, file, clock)).allowed).toBe(true);
                    for (const changed of [{ originalScope: undefined }, { originalScope: resource("task", "ctx-jp-b-luna") },
                        ...
                        expect((await decide(s, p, action, { ...file, ...changed }, clock)).allowed).toBe(false);
                    }
                    expect((await decide(s, (await principal(service, s, "user-none")), action, file, clock)).allowed).toBe(false);
                }
            });
        });
        it("T09: projection precedes keyword matching/count/order/page across future channels", async () => {
            await repo.transaction(async (s) => {
                ...
                const page = async (term: string) => visiblePage(rows, async (scope) => (await decide(s, p, "search.read", scope, clock)).allowed
                    ? (await projectChannel(s, p, "search.read", scope, { title: "public title", internalMemo: marker, unknown: { value: marker } }, clock)) : null, { matches: row => JSON.stringify(row).includes(term), compare: (a, b) => a.id.localeCompare(b.id), offset: 0, limit: 1 });
                expect((await page("public")).total).toBe(1);
                expect((await page(marker))).toEqual({ total: 0, items: [] });
                for (const [action, kind] of [["export.read", "export"], ["ai.input.read", "ai_input"], ["ai.result.read", "ai_result"], ["notification.read", "notification"]] as const) {
                    const dto = (await projectChannel(s, p, action, { ...resource(kind), recipientUserId: p.user.id }, { title: "public", internalOriginal: marker, internalSupplyPrice: marker }, clock));
                    expect(JSON.stringify(dto)).not.toContain(marker);
                }
                expect((await decide(s, p, "notification.read", { ...resource("notification"), recipientUserId: "user-luna" }, clock)).allowed).toBe(false);
                expect((await decide(s, p, "ai.result.read", { ...resource("ai_result"), sourceScopes: [resource("ai_input", "ctx-jp-a-wave")] }, clock)).allowed).toBe(false);
                const priceFile = { ...resource("file"), originalScope: { ...resource("product"), requiresInternalPrice: true }, referenceScope: resource("task") };
                expect((await decide(s, (await principal(service, s, "user-gsg")), "file.download", priceFile, clock)).allowed).toBe(false);
                expect((await decide(s, (await principal(service, s, "user-price")), "file.download", priceFile, clock)).allowed).toBe(true);
            });
        });
        it("T07/T16: reused principal re-reads price and membership changes in the same UoW", async () => {
            const saved = await repo.transaction(async (s) => (await principal(service, s, "user-price")));
            ...
            await repo.transaction(async (s) => expect((await decide(s, saved, "price.read", resource("product"), clock)).allowed).toBe(false));
            ...
            await expect(repo.transaction(async (s) => {
                (await authorize(s, saved, "task.manage", resource("task"), clock));
                (await s.update("task", before!.id, before!.revision, { ...before!.data, title: "must not commit" }));
        ...
        it("T11/T16: suspended account and old authVersion/session cannot be restored by stale principal", async () => {
            const saved = await repo.transaction(async (s) => (await principal(service, s, "user-luna")));
            ...
            await repo.transaction(async (s) => expect((await decide(s, saved, "task.read", resource("task"), clock))).toEqual({ allowed: false, status: 401 }));
        ...
        it("T11: expiry and authVersion invalidation fail closed", async () => {
            await repo.transaction(async (s) => {
                const p = (await principal(service, s, "user-team"));
                expect((await decide(s, p, "task.read", resource("task"), () => "2026-09-23T00:00:00.000Z")).allowed).toBe(false);
                const u = (await s.get("user", p.user.id))!;
                (await s.update("user", u.id, u.revision, { ...u.data, authVersion: 2 }));
                expect((await decide(s, p, "task.read", resource("task"), clock)).allowed).toBe(false);
            });
        });
        it("T17: failing permission store propagates failure instead of allowing or returning empty admin", async () => {
            await repo.transaction(async (s) => {
                ...
                await expect(decide(broken, p, "task.read", resource("task"), clock)).rejects.toThrow("STORAGE_UNAVAILABLE");
            });
        });
        it("T01: absent and forbidden resources have the same non-revealing 404", async () => {
            const errors = await Promise.all(["ctx-jp-a-wave", "missing-context"].map(async (c) => service.members(tokenFor("user-luna"), c).catch(e => ({ code: e.code, status: e.status, message: e.message }))));
        ...
        it("T04/T20: current member service excludes cross-context global audits and peer grants", async () => {
            await repo.transaction(async (s) => {
                (await s.create("membership", { id: "peer-admin", contextId: CONTEXT, data: { userId: "user-admin", role: "operator", status: "active", scope: "fixture", internalPriceAccess: false, activatedAt: NOW, suspendedAt: null } }));
                (await s.create("audit", { id: "unknown-global", contextId: null, data: { actorId: "user-admin", targetId: "user-admin", action: marker, before: { secret: marker }, after: {}, at: NOW } }));
            });
