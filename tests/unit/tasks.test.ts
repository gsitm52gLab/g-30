...
for (const mode of ["mock", "sqlite"] as const)
    describe(`${mode} G04 actual task/file contract`, () => {
        ...
        it("AC-04-01/02 SA-12 drafts stay private; preview equals brand publication and filters original/milestone", async () => {
            ...
            expect(preview.content.milestones).toEqual([]);
            await command(id, "publish");
            const b = await tasks.detail(brand, id);
            expect(b.request).toEqual(preview.content);
            expect(b.draft).toBeNull();
            expect((await tasks.detail(admin, id)).request!.milestones[0].deadline.raw).toContain("원문 충돌");
            const saved = { ...c, description: "미공개 초안" };
            await command(id, "save", { content: saved });
        ...
        it("AC-04-01/06 SA-07 project selection creates parallel independent work; dependency cycle and cross-project edge reject", async () => {
            await setup();
            const p = await tasks.createProject(admin, { title: "합성 신규 입점", target, templateVersionIds: ["builtin-documents-v1", "builtin-pop-v1", "builtin-video-v1"], idempotencyKey: randomUUID() });
            ...
            await repo.transaction(async (s) => { const t = (await s.get("task", p.ids[1]))!; (await s.update("task", t.id, t.revision, { ...t.data, status: "completed" })); });
        ...
        it("AC-04-04 SA-14 template revisions apply only selected tasks and retain immutable history", async () => {
            ...
            const original = (await tasks.detail(admin, first)).versions[0];
            const next = payload();
            next.requirements.push({ ...blankRequirement("new-required", "file"), label: "새 필수 파일" });
            ...
            const before = (await repo.list("requestVersion")).length;
            await tasks.applyTemplate(admin, input);
            await tasks.applyTemplate(admin, input);
            expect(await repo.list("requestVersion")).toHaveLength(before + 1);
            expect((await tasks.detail(brand, first)).versions).toHaveLength(2);
            expect((await tasks.detail(brand, second)).versions).toHaveLength(1);
            expect((await tasks.detail(admin, first)).versions[1]).toEqual(original);
            await expect(repo.transaction(async (s) => { const r = (await s.get("requestVersion", original.id))!; (await s.update("requestVersion", r.id, r.revision, r.data)); })).rejects.toMatchObject({ code: "INVALID_RECORD" });
        });
        it("AC-04-04 prior-submission fixture preserves compatible values and identifies new/changed/conditional/product work", async () => {
            await setup();
            const c = payload();
            c.requirements = [{ ...blankRequirement("pick", "choice"), label: "참여", options: ["yes", "no"] }, { ...blankRequirement("per-product", "number"), label: "제품 수량", productIds: target.productIds, unit: "개" }, { ...blankRequirement("optional"), label: "선택 메모", required: false }];
            const id = await publish(c), old = (await tasks.detail(admin, id)).versions[0];
            await repo.transaction(async (s) => (await s.create("priorSubmission", { id: "prior", contextId: ctx, data: { taskId: id, requestId: old.id, authorId: "user-luna", answers: [{ requirementKey: "pick", productId: null, value: "no", fileVersionIds: [] }, { requirementKey: "per-product", productId: "product-serum", value: 0, fileVersionIds: [] }] } })));
            ...
            expect(states.find(s => s.requirementKey === "pick")?.status).toBe("prior_received");
            expect(states.find(s => s.requirementKey === "per-product")).toMatchObject({ status: "needs_reconfirmation", humanReviewPending: true, sourceRequestId: old.id });
            expect(states.find(s => s.requirementKey === "conditional")?.status).toBe("not_applicable");
            expect(states.find(s => s.requirementKey === "added")?.status).toBe("missing");
            expect(states.find(s => s.requirementKey === "optional")?.status).toBe("optional");
        ...
        it("AC-04-05 CR01/02 schedule applies only published content, persists decision/new version and preserves private draft", async () => {
            ...
            const v1 = (await tasks.detail(brand, id)).versions[0];
            const draft = { ...payload(), description: "아직 공개하지 않을 설명", requirements: [{ ...blankRequirement("private-new"), label: "미공개 요건" }] };
            await command(id, "save", { content: draft });
            await command(id, "schedule", { deadline: { ...payload().deadline, value: "2026-11-20" }, reason: "일정 협의" }, brand);
            ...
            expect(b.request!.description).toBe(v1.content.description);
            expect(b.request!.requirements).toEqual(v1.content.requirements);
            expect(b.request!.deadline.value).toBe("2026-11-20");
            ...
            expect((await tasks.detail(brand, id)).request!.deadline.value).toBe("2026-11-20");
            expect((await tasks.detail(brand, id)).activities.at(-1)!.data.decision).toBe("keep");
        });
        it("AC-04-05 CR03 receipt retry creates acceptance/event once; CAS and injected failure roll back all writes", async () => {
            await setup();
            const id = await publish(), revision = (await repo.get("task", id))!.revision;
            const input = { command: "accept", expectedRevision: revision, idempotencyKey: randomUUID() };
            await tasks.command(brand, id, input);
            await tasks.command(brand, id, input);
        ...
        it("SA-09 invalid/foreign/inactive assignees reject; stopped co-assignee marks work and history preserves both arrays", async () => {
            ...
            expect(history.data.before.coAssigneeIds).toEqual(["user-co"]);
            expect(history.data.after.coAssigneeIds).toEqual(["user-team"]);
        ...
        it("AC-04-03 SA-08/13 multi-context clones and manual cycles/additional product tasks keep old state independent", async () => {
            ...
            await repo.transaction(async (s) => {
                (await s.create("membership", { id: "gsg-in-b", contextId: ctx2, data: { userId: "user-price", role: "operator", status: "active", scope: "B", internalPriceAccess: false, activatedAt: identity.clock(), suspendedAt: null } }));
                for (const [userId, role] of [["user-gsg", "operator"], ["user-luna", "brand"]] as const)
                    (await s.create("membership", { id: `third-${userId}`, contextId: "ctx-sg-a-luna", data: { userId, role, status: "active", scope: "third synthetic fixture", internalPriceAccess: false, activatedAt: identity.clock(), suspendedAt: null } }));
                const source = (await s.get("product", "product-serum"))!.data;
                (await s.create("product", { id: "product-third", contextId: "ctx-sg-a-luna", data: { name: source.name, code: source.code, brand: source.brand, size: source.size, category: source.category, status: source.status, missingMaterials: source.missingMaterials } }));
                (await migrateLegacyProducts(s));
            });
            const c = payload();
            c.requirements[0].productIds = ["product-serum"];
            ...
            const independentBefore = await Promise.all(result.ids.slice(1).map(id => repo.get("task", id)));
            const other = (await tasks.detail(admin, result.ids[1])).draft!;
            expect(other.deadline.responsibleUserId).toBe("user-price");
            expect(other.requirements[0].productIds).toEqual(["product-cream"]);
            await command(result.ids[0], "publish");
            await command(result.ids[0], "accept", {}, brand);
            const old = await repo.get("task", result.ids[0]);
            const copy = await command(result.ids[0], "duplicate", { cycle: { label: "10월 회차", start: "2026-10-01", end: "2026-10-31" } });
            expect(await repo.get("task", result.ids[0])).toEqual(old);
            expect((await repo.get("task", copy.ids[0]))!.data).toMatchObject({ status: "draft", currentRequestId: null, cycle: { sourceTaskId: result.ids[0] } });
        ...
        it("G04-V03 AC-04-02 A19 stored nested extensions never enter current/history/preview/draft/template/activity/project output", async () => {
            await setup();
            const marker = "V03_PRIVATE_STORED_EXTENSION";
            function poison<T>(value: T): T { if (Array.isArray(value))
                return value.map(poison) as T; if (value && typeof value === "object")
                return Object.assign(Object.fromEntries(Object.entries(value).map(([k, v]) => [k, poison(v)])), { privateExtension: { deep: marker } }) as T; return value; }
            const clean = payload();
            clean.links = [{ url: "https://example.test/guide", description: "공개 링크", contentFixed: false }];
            ...
            const c = Object.assign(poison(clean), { internalSupplyPrice: marker, unknownObject: { deep: marker } });
            const id = await create(clean);
            await repo.transaction(async (s) => {
                (await s.create("requestVersion", { id: "v03-old", contextId: ctx, data: { taskId: id, sequence: 1, previousId: null, templateVersionId: null, content: c, publishedBy: "user-admin", publishedAt: identity.clock(), changedKeys: ["description", { private: marker }] as unknown as string[] } }));
                const t = (await s.get("task", id))!;
                (await s.update("task", id, t.revision, { ...t.data, draft: c, currentRequestId: "v03-old", visibility: "public", status: "requested" }));
                (await s.create("templateVersion", { id: "v03-template", contextId: ctx, data: Object.assign({ templateId: "v03-template", name: "공개 템플릿", sequence: 1, previousId: null, content: c, createdBy: "user-admin", builtin: false }, { privateExtension: marker }) }));
                (await s.create("taskActivity", { id: "v03-activity", contextId: ctx, data: Object.assign({ taskId: id, requestId: "v03-old", userId: "user-luna", kind: "schedule" as const, at: identity.clock(), sequence: 1, reason: "공개 조정 사유", proposedDeadline: poison(clean.deadline), respondsTo: null, decision: null, resultingRequestId: null }, { privateExtension: marker }) }));
                (await s.create("project", { id: "v03-project", contextId: ctx, data: Object.assign({ title: "공개 프로젝트", taskIds: [id, "task-onboarding"], dependencies: [Object.assign({ before: id, after: "task-onboarding" }, { privateExtension: marker })], status: "active" as const, createdBy: "user-admin" }, { privateExtension: marker }) }));
            });
            await command(id, "publish");
            ...
            expect(b.request).toEqual(publicContent);
            expect(b.versions[1].content).toEqual(publicContent);
            expect(b.versions[1].changedKeys).toEqual(["description"]);
            expect(preview.content).toEqual(publicContent);
            expect(g.draft).toEqual(clean);
            expect(g.request).toEqual(clean);
            expect(catalog.templates.find(t => t.id === "v03-template")!.content).toEqual(clean);
            expect(b.activities[0].data.proposedDeadline).toEqual(clean.deadline);
            expect(b.activities[0].data.reason).toBe("공개 조정 사유");
            expect(project.data.dependencies).toEqual([{ before: id, after: "task-onboarding" }]);
        ...
        it("G04-V02 AC-04-05 SA-15 requested/accepted nested pause restores progress without duplicate acceptance or state events", async () => {
            ...
            await command(id, "accept", {}, brand);
            const history = await repo.list("taskActivity");
            const versions = await repo.list("requestVersion");
            const hold = { command: "hold", expectedRevision: (await repo.get("task", id))!.revision, reason: "accepted hold", idempotencyKey: randomUUID() };
            await tasks.command(admin, id, hold);
            const paused = await repo.get("task", id);
            const stateEvents = await repo.list("domainEvent");
            await tasks.command(admin, id, hold);
            await command(id, "hold", { reason: "same hold" });
            expect(await repo.get("task", id)).toEqual(paused);
            expect(await repo.list("domainEvent")).toEqual(stateEvents);
            await command(id, "cancel", { reason: "nested cancellation" });
            await command(id, "hold", { reason: "nested hold" });
            ...
            await tasks.command(admin, id, resume);
            expect(await repo.get("task", id)).toEqual(restored);
            expect(restored!.data).toMatchObject({ status: "in_progress", resumeStatus: null });
            await command(id, "accept", {}, brand);
            expect((await repo.get("task", id))!.data.status).toBe("in_progress");
            expect(await repo.list("taskActivity")).toEqual(history);
            expect(await repo.list("requestVersion")).toEqual(versions);
            expect((await repo.list("domainEvent")).filter(e => e.data.eventType === "TASK_ACCEPTED")).toHaveLength(1);
        });
        it("G04-V02 SA-15 partial state and prior answers survive pause and new-public-version is not auto-accepted", async () => {
            ...
            const id = await publish();
            await command(id, "accept", {}, brand);
            const v1 = (await tasks.detail(admin, id)).versions[0];
            await repo.transaction(async (s) => { const row = (await s.get("task", id))!; (await s.update("task", id, row.revision, { ...row.data, status: "partial" })); (await s.create("priorSubmission", { id: "state-prior", contextId: ctx, data: { taskId: id, requestId: v1.id, authorId: "user-luna", answers: [{ requirementKey: "description", productId: null, value: "previous partial answer", fileVersionIds: [] }] } })); });
            const prior = await repo.get("priorSubmission", "state-prior");
            await command(id, "hold", { reason: "partial hold" });
            await command(id, "save", { content: { ...payload(), description: "new request version" } });
            await command(id, "publish");
            const v2 = (await tasks.detail(admin, id)).versions[0];
            expect(v2.id).not.toBe(v1.id);
            await command(id, "cancel", { reason: "nested" });
            await command(id, "resume", { reason: "restore partial" });
            expect((await repo.get("task", id))!.data.status).toBe("partial");
            expect(await repo.get("priorSubmission", "state-prior")).toEqual(prior);
            expect((await repo.list("taskActivity")).filter(a => a.data.kind === "accept" && a.data.requestId === v2.id)).toHaveLength(0);
            await command(id, "accept", {}, brand);
            await command(id, "accept", {}, brand);
            expect((await repo.get("task", id))!.data.status).toBe("partial");
            expect((await repo.list("taskActivity")).filter(a => a.data.kind === "accept")).toHaveLength(2);
            expect((await repo.list("domainEvent")).filter(e => e.data.eventType === "TASK_ACCEPTED")).toHaveLength(2);
        });
        it("G04-V02 legacy paused fallback uses current request acceptance only and repairs previously inconsistent requested state", async () => {
            ...
            const id = await publish();
            await command(id, "accept", {}, brand);
            async function legacy(status: "on_hold" | "cancelled" | "requested") { await repo.transaction(async (s) => { const row = (await s.get("task", id))!; const data = { ...row.data, status }; delete data.resumeStatus; (await s.update("task", id, row.revision, data)); }); }
            await legacy("on_hold");
            await command(id, "resume", { reason: "legacy accepted" });
            expect((await repo.get("task", id))!.data.status).toBe("in_progress");
            await legacy("requested");
            const accepted = await repo.list("taskActivity"), events = await repo.list("domainEvent");
            await command(id, "accept", {}, brand);
            expect((await repo.get("task", id))!.data.status).toBe("in_progress");
            expect(await repo.list("taskActivity")).toEqual(accepted);
            expect(await repo.list("domainEvent")).toEqual(events);
            await command(id, "save", { content: { ...payload(), description: "v2 without acceptance" } });
            await command(id, "publish");
            await legacy("cancelled");
            await command(id, "resume", { reason: "legacy unaccepted latest" });
            expect((await repo.get("task", id))!.data.status).toBe("requested");
            expect((await repo.list("taskActivity")).filter(a => a.data.kind === "accept")).toHaveLength(1);
        ...
        it("A04/A19 SA-04 exact reference bytes stay private before publish, survive revisions and revoke with membership", async () => {
            await setup();
            const directory = await mkdtemp(path.join(os.tmpdir(), "g04-file-"));
            directories.push(directory);
            const files = new FileService(identity, directory);
            ...
            const preview = await tasks.preview(admin, id);
            await command(id, "publish");
            expect((await tasks.detail(brand, id)).request).toEqual(preview.content);
            expect((await files.download(brand, fid, id, "download")).bytes).toEqual(bytes);
            expect((await files.download(brand, fid, id, "preview")).metadata.preview).toBe(true);
            await command(id, "save", { content: payload() });
            await command(id, "publish");
...
describe("G04 declarative validation and evaluator boundaries", () => {
    ...
    it("CR06 rejects impossible date/date-time, preserves timezone/date-only precision and conflicting raw source", () => { for (const value of ["2026-02-30", "2026-13-01"])
        expect(() => deadline({ ...payload().deadline, value })).toThrow(); for (const value of ["2026-02-30T09:00:00+09:00", "2026-10-01T09:00:00", "2026-10-01T24:00:00+09:00", "2026-10-01T09:61:00+09:00"])
        expect(() => deadline({ ...payload().deadline, value, precision: "datetime" })).toThrow(); expect(deadline({ ...payload().deadline, value: "2026-10-01T09:00:00+09:00", precision: "datetime", raw: "10/1 수요일 원문" })).toMatchObject({ value: "2026-10-01T09:00:00+09:00", raw: "10/1 수요일 원문", timezone: "Asia/Seoul" }); });
    it("A04 partial-SA-19 file formats, bounded names/mime/size and signature checks reject traversal or disguised content", () => { const pdf = Buffer.from("%PDF-1.4\nsynthetic"); for (const [name, type, bytes] of [["../bad.pdf", "application/pdf", pdf], ["x.exe", "application/octet-stream", pdf], ["x.pdf", "image/png", pdf], ["x.pdf", "application/pdf", Buffer.from("fake")], ["x.pdf", "application/pdf", Buffer.alloc(MAX_FILE_BYTES + 1)]] as const)
        expect(() => validateFile(name, type, bytes)).toThrow(); const exact = Buffer.alloc(MAX_FILE_BYTES); pdf.copy(exact); expect(validateFile("limit.pdf", "application/pdf", exact).preview).toBe(true); });
});
