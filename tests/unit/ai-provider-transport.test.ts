...
describe('G17-V01 actual response discriminator and durable safe projection',()=>{
 for(const f of malformedOutputs)it(`G17-V01 ${f.kind} received parseable response retains safe metadata`,async()=>{let calls=0;vi.stubGlobal('fetch',async()=>{calls++;return response(envelope(f.output));});const r=await openaiTransport(config,request,async()=>{},async()=>{});expect(r).toMatchObject({issue:'PARSE_ERROR',responseId:'resp_real_envelope',requestId:'req_real_envelope',responseModel:'gpt-6-astra',serviceTier:'default',providerStatus:'completed',remoteOutcomeUnknown:false,rawCandidate:null,usage:{inputTokens:11,cachedTokens:0,cacheWriteTokens:2,outputTokens:7,reasoningTokens:3,totalTokens:18}});expect(r.responseHash).toMatch(/^[a-f0-9]{64}$/);expect(JSON.stringify(r)).not.toContain('PRIVATE_PROVIDER_BODY_NOT_TO_STORE');expect(calls).toBe(1);});
 for(const mode of ['mock','sqlite'] as const)for(const f of malformedOutputs)it(`G17-V01 ${mode} ${f.kind} persists provided usage and nullable cost without a result`,async()=>{const directory=await mkdtemp(path.join(os.tmpdir(),'g17-v01-'));const repo=mode==='mock'?createMockRepository(()=>NOW):(()=>{const db=openDatabase(':memory:',true);migrate(db);return createSqliteRepository(db,()=>NOW);})();try{const identity=await policyFixture(repo),input=await readyInput(identity,directory,inputContent(SYNTHETIC_TEXT));let calls=0;const usage=f.kind==='output-object'?usageFixture:{...usageFixture,input_tokens_details:{cached_tokens:0}};vi.stubGlobal('fetch',async()=>{calls++;return response(envelope(f.output,usage));});const service=new AiProviderService(identity,directory,{config:()=>({config,issue:null})});const result=await service.start(staff,input.input.id,{...input.body,engine:'provider'});expect(result.detail).toMatchObject({state:'failed',providerCalled:true,result:null,provider:{attempts:[{issue:'PARSE_ERROR',remoteOutcomeUnknown:false,responseId:'resp_real_envelope',requestId:'req_real_envelope',usage:{inputTokens:11,cacheWriteTokens:f.kind==='output-object'?2:null},cost:{amountUsd:f.kind==='output-object'?0.000465:null}}]}});const outcomes=await repo.list('aiProviderOutcome');expect(outcomes).toHaveLength(1);expect(outcomes[0].data.rawCandidate).toBeNull();expect(outcomes[0].data.responseHash).toMatch(/^[a-f0-9]{64}$/);expect(JSON.stringify(outcomes)).not.toContain('PRIVATE_PROVIDER_BODY_NOT_TO_STORE');expect(await repo.list('aiAnalysisResult')).toHaveLength(0);expect(calls).toBe(1);}finally{repo.close();await rm(directory,{recursive:true,force:true});}});
...
async function bodyDeadlineProbe(boundary:'transport'|'sdk-control',headerDelay:number,bodyDelay:number|null) {
 vi.useFakeTimers();let calls=0,before=0,dispatched=0,bodyRead=false,aborted=false;
 let stream:ReadableStreamDefaultController<Uint8Array>|undefined;
 vi.stubGlobal('fetch',async(_input:unknown,init:RequestInit)=>{
  calls++;if(headerDelay)await new Promise(resolve=>setTimeout(resolve,headerDelay));
  ...
  if(bodyDelay!==null)setTimeout(()=>{stream!.enqueue(new TextEncoder().encode(JSON.stringify(envelope([{type:'message',content:[{type:'output_text',text:'{"findings":[]}'}]}]))));stream!.close();},bodyDelay);
 ...
 await vi.advanceTimersByTimeAsync(44999);const beforeDeadline={settled,aborted};
 await vi.advanceTimersByTimeAsync(2);const atDeadline={settled,aborted,calls,before,dispatched,bodyRead};
 if(!settled)stream!.error(new Error('bounded fixture cleanup after deadline observation'));
 const result=await value;return {beforeDeadline,atDeadline,result};
}
describe('G17-V02 SDK original deadline includes response body',()=>{
 for(const boundary of ['transport','sdk-control'] as const)for(const delay of [0,20000])it(`G17-V02 ${boundary} headers ${delay}ms then stalled body expires at original45s`,async()=>{
  ...
  if(boundary==='transport'){expect(p.atDeadline).toMatchObject({before:1,dispatched:1});expect(p.result).toMatchObject({remoteOutcomeUnknown:true,requestId:'req_body_deadline',rawCandidate:null,responseId:null,usage:{inputTokens:null}});}
 });
 for(const boundary of ['transport','sdk-control'] as const)it(`G17-V02 ${boundary} headers20s and completed body10s succeeds within deadline`,async()=>{
  const p=await bodyDeadlineProbe(boundary,20000,10000);expect(p.atDeadline).toMatchObject({settled:true,aborted:false,calls:1});expect(p.result.issue).toBeNull();
  if(boundary==='transport')expect(p.result).toMatchObject({requestId:'req_body_deadline',responseId:'resp_real_envelope',remoteOutcomeUnknown:false,usage:{inputTokens:11,cacheWriteTokens:2}});
 });
});


it('G17-V02 public SDK request preserves Responses.create wire options and single dispatch callbacks',async()=>{
 const wire:unknown[]=[];const callbacks:string[]=[];
 ...
 expect(result.issue).toBeNull();expect(wire).toHaveLength(2);expect(wire[1]).toEqual(wire[0]);expect(wire[1]).toMatchObject({url:'https://api.openai.com/v1/responses',method:'POST',authMatches:true,retryCount:'0',timeout:null,body:request});expect(callbacks).toEqual(['before','dispatched']);
});
