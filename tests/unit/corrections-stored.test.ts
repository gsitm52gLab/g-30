...
for (const mode of ['mock', 'sqlite'] as const)
    describe(`${mode} G10 stored public boundary`, () => {
        it('A19 malformed known immutable public batch content fails controlled503 without rewriting raw snapshot', async () => {
            ...
            try {
                ...
                await expect(service.detail(tokenFor('user-luna'), 'poison-batch')).rejects.toMatchObject({ status: 503, code: 'STORAGE_UNAVAILABLE' });
                await expect(service.workspace(tokenFor('user-luna'), 'task-onboarding')).rejects.toMatchObject({ status: 503 });
