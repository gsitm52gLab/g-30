...
for(const mode of ['mock','sqlite'] as const)describe(`${mode} G17 frozen ac53 recovery edges`,()=>{
 ...
 it('R17-02 expired observed call needs explicit bounded recovery; late old response/replayed intent cannot overwrite or resend',async()=>{await setup();let now=NOW,enter!:()=>void,release!:()=>void,calls=0;const identity=new IdentityService(repo,()=>now),gate=new Promise<void>(r=>release=r),entered=new Promise<void>(r=>enter=r),cfg=()=>({config:{apiKey:'synthetic',model:'gpt-6-astra',baseURL:'https://api.openai.com/v1'},issue:null});const service=new AiProviderService(identity,directory,{config:cfg,transport:async(...args)=>{await args[2]();await args[3]();const callNumber=++calls;if(callNumber===1){enter();await gate;}return {...await success(args[0],args[1],async()=>{},async()=>{}),responseId:callNumber===1?'resp_old':'resp_new'};}});const f=await readyInput(identity,directory,inputContent(SYNTHETIC_TEXT)),pending=service.start(staff,f.input.id,{...f.body,engine:'provider'});await entered;try{now=new Date(Date.parse(NOW)+100000).toISOString();const row=(await repo.list('aiAnalysisRun'))[0],before=await new AiReviewService(identity,directory).detail(staff,row.id);expect(before.state).toBe('interrupted');expect(before.provider!.attempts[0].remoteOutcomeUnknown).toBe(true);await expect(service.retry(staff,row.id,{expectedRevision:before.revision,idempotencyKey:randomUUID()})).rejects.toMatchObject({code:'RESPONSE_UNKNOWN'});expect(calls).toBe(1);const command={expectedRevision:before.revision,acknowledgeUnknown:true,idempotencyKey:randomUUID()},recovered=await service.retry(staff,row.id,command);expect(recovered.runId).toBe(row.id);expect(recovered.detail.state).toBe('finished');expect(recovered.detail.provider!.attempts).toHaveLength(2);expect(recovered.detail.provider!.attempts[0]).toMatchObject({issue:'RESPONSE_UNKNOWN',remoteOutcomeUnknown:true});await service.retry(staff,row.id,command);expect(calls).toBe(2);const resultId=recovered.detail.result!.id;release();await pending;expect((await new AiReviewService(identity,directory).detail(staff,row.id)).result!.id).toBe(resultId);expect(await repo.list('aiAnalysisResult')).toHaveLength(1);}finally{release();await pending.catch(()=>{});}});
});
