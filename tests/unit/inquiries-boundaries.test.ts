...
for (const mode of ['mock', 'sqlite'] as const)
    describe(`${mode} inquiry current authorization boundaries`, () => {
        ...
        it('A19 same brand other retailer and selected GSG scope never imply participation', async () => {
            ...
            for (const token of [brand, gsg, tokenFor('user-selected-admin'), tokenFor('user-wave')])
                await expect(service.detail(token, id)).rejects.toMatchObject({ status: 404 });
            expect((await service.detail(admin, id)).phase).toBe('active');
            const same = await active();
            await expect(service.detail(tokenFor('user-co'), same)).rejects.toMatchObject({ status: 404 });
        ...
        it('D02 actual inquiry download rechecks fresh membership after asynchronous byte IO', async () => {
            ...
            await expect(new FileService(new IdentityService(wrapped, () => NOW), dir).download(gsg, f.file.id, { kind: 'inquiry', conversationId: id, messageId: m.messageId! }, 'download')).rejects.toMatchObject({ status: 401 });
            expect(calls).toBe(1);
        });
        it('D04 cursor delivery callback is never called after session revocation', async () => {
            ...
            let delivered = false;
            await expect(service.events(brand, id, new URLSearchParams({ after: d.cursor }), () => { delivered = true; })).rejects.toMatchObject({ status: 401 });
