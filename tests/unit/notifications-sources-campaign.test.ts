...
for (const mode of ['mock', 'sqlite'] as const) describe(`${mode} G13 campaign request recipient repair`, () => {
    ...
    it.each(['selection', 'external_fact'] as const)('S13-R01 %s actual new request preserves primary/co brand candidates independently of GSG event', async cause => {
        ...
        if (cause === 'external_fact') await fixture.select();
        ...
        const matrix = await repo.transaction(s => recipients.map(({ id, token }) => {
            const p = identity.principal(s, token), revised = notificationEventSource(s, p, revision.id, () => NOW), original = notificationEventSource(s, p, origin.id, () => NOW);
