...
it('G08 migration adds constraints to populated accepted G05 DB without changing edited business records', async () => {
    ...
    try {
        for (const name of readdirSync(source).filter(n => /^000[1-5]-.*\.sql$/.test(n))) copyFileSync(path.join(source, name), path.join(directory, name));
        expect(migrate(db, directory).total).toBe(5); repo = createSqliteRepository(db); await seed(repo);
        await repo.transaction(s => { const t = s.get('task', 'task-pop')!; s.update('task', t.id, t.revision, { ...t.data, title: '기존 사용자가 수정한 실제 데이터' }); });
        const before = db.prepare('SELECT * FROM records ORDER BY kind,id').all();
        expect(migrate(db)).toEqual({ applied: 6, total: 11 });
        expect(db.prepare('SELECT * FROM records ORDER BY kind,id').all()).toEqual(before);
        expect(migrate(db)).toEqual({ applied: 0, total: 11 });
        expect(db.prepare('SELECT * FROM records ORDER BY kind,id').all()).toEqual(before);
    } finally { if (repo) repo.close(); else db.close(); rmSync(directory, { recursive: true, force: true }); }
});
