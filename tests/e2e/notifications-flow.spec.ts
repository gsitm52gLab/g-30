...
journeys();
test('G13 UI-F normal task open focus self sync badge clicked RSC and notification read separate from task read', async ({ page }, info) => { test.setTimeout(120000); const x = await setup(page), admin = await page.context().browser()!.newContext({ baseURL: `http://127.0.0.1:${process.env.E2E_PORT}`, storageState: await page.context().storageState() }); try {
    ...
    page.on('response', r => { if (r.request().headers()['rsc'] === '1')
        captured.push((async () => { try {
            const b = await r.body();
            await writeFile(info.outputPath(`native-rsc-${createHash('sha256').update(b).digest('hex')}.txt`), b);
        }
        catch { /* Prefetch bodies discarded by browser are not claimed captured. */ } })()); });
