...
finally{for(const p of [...children.keys()])await stop(p);const report={candidate_commit:candidate,session_uuid:'01a0c307-c975-75b1-b96a-5a5c4e448aec',runner_sha256:hash(readFileSync('scripts/verify-campaigns-http.ts','utf8')),fixture_runner_sha256:hash(readFileSync('scripts/verify-campaigns-fixtures.ts','utf8')),status:failure?'FAIL':'PASS',mode,cwd:process.cwd(),startedAt,finishedAt:new Date().toISOString(),failure,count_unit:'assertion',pass:checks.filter(c=>c.status==='PASS').length,fail:checks.filter(c=>c.status==='FAIL').length+(failure&&!checks.some(c=>c.status==='FAIL')?1:0),skip:skippedIds.length,skipped_ids:skippedIds,not_run:reachedEnd?0:32-checks.length-skippedIds.length,reachedEnd,checks,transcript,processes,resources:{port,auxPort,database,files},fixture_boundary:'Actual task/submission/product/campaign/file HTTP producers; private mock IPC observer and explicit stored-corruption fixtures only. SQLite normal Next start. No product test endpoint/auth bypass. G12 UI and independent verification NOT_RUN.'};writeFileSync(reportFile,JSON.stringify(report,null,2),{mode:0o600});console.log(JSON.stringify({status:report.status,pass:report.pass,fail:report.fail,reportFile}));}
