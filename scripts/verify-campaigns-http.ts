...
async function actualSubmission(taskId: string) {
    ...
    body.append('files', new Blob([new Uint8Array(png)], { type: 'image/png' }), '합성실행.png');
    const upload = await fetch(`${brand.origin}/api/tasks/${taskId}/submission-files?requestId=${w.request.id}`, { method: 'POST', headers: { Cookie: brand.cookie, Origin: brand.origin, 'X-CSRF-Token': csrf.csrfToken }, body });
    await capture(upload, 'POST multipart', `/api/tasks/${taskId}/submission-files?requestId=${w.request.id}`);
    ...
    if (item.state !== 'ready')
        throw Error('submission upload failed');
    ...
    const r = await brand.mutate(`/api/tasks/${taskId}/submissions`, { baseRequestId: w.request.id, expectedDraftRevision: w.draft!.revision, expectedTaskRevision: w.taskRevision, mode: 'partial', idempotencyKey: randomUUID() });
    assert.equal(r.status, 201, await r.clone().text());
    const sid = (await r.json()).ids[0], snapshot = await brand.get<SubmissionSnapshot>(`/api/submissions/${sid}`), reference: SubmittedReference = { taskId, requestId: snapshot.requestId, submissionId: sid, contentHash: snapshot.contentHash, answer: { requirementKey: 'proof', productId: null }, fileVersionIds: [item.file.id], productUseIds: snapshot.products.map(p => p.id) };
    return { snapshot, reference, fileId: item.file.id };
}
async function banner(taskId: string, viewport: {
    width: number;
    height: number;
}, label: string) { const browser = await chromium.launch(); const context = await browser.newContext({ viewport }); await context.addCookies([{ name: brand.cookie.slice(0, brand.cookie.indexOf('=')), value: brand.token, url: brand.origin, httpOnly: true, sameSite: 'Lax' }]); await context.tracing.start({ screenshots: true, snapshots: true, sources: true }); const page = await context.newPage(); try {
    await page.goto(`${brand.origin}/tasks/${taskId}?context=${A}`);
    await expect(page.getByRole('note', { name: '메뉴 선택 반영 요청', exact: true }).first()).toBeVisible();
    await expect(page.getByText('자료 요청 없음 · 이 요청 버전의 자동 제출 대상 항목은 없습니다.', { exact: true }).first()).toBeVisible();
    await expect(page.getByText(/취소 협의 중 · 이전 이행 범위 조정 확인 필요/).first()).toBeVisible();
    await page.screenshot({ path: `${reportFile}.${label}.png`, fullPage: true });
}
catch (error) {
    await page.screenshot({ path: `${reportFile}.${label}-failure.png`, fullPage: true }).catch(() => { });
    writeFileSync(`${reportFile}.${label}-failure.html`, await page.content(), { mode: 0o600 });
    throw error;
}
finally {
    await context.tracing.stop({ path: `${reportFile}.${label}.trace.zip` });
    await browser.close();
} }
...
try {
    ...
    const created = await admin.mutate('/api/tasks', { category: 'spot', content: c, targets: [{ contextId: A, ownerId: 'user-gsg', assigneeId: 'user-luna', coAssigneeIds: ['user-co'], productIds: ['product-serum'] }], idempotencyKey: randomUUID() });
    assert.equal(created.status, 201, await created.clone().text());
    const taskId = (await created.json()).ids[0] as string;
    await taskCommand(taskId, 'publish');
    ...
    const secondCreated = await admin.mutate('/api/products', { contextId: A, brandId: p.context.data.brandId, common: { ...blankCommon(), name: 'HTTP 미선택 메뉴 상품', code: 'G12-PRE01-HTTP' }, fields: blankContext(), idempotencyKey: randomUUID() });
    assert.equal(secondCreated.status, 201, await secondCreated.clone().text());
    const p2Id = (await secondCreated.json()).ids[0] as string, taskForLink = await admin.get<TaskDetail>(`/api/tasks/${taskId}`);
    const secondLinked = await admin.mutate(`/api/products/${p2Id}`, { command: 'link_task', contextId: A, taskId, expectedTaskRevision: taskForLink.task.revision, idempotencyKey: randomUUID() });
    assert.equal(secondLinked.status, 200, await secondLinked.clone().text());
    const second = await admin.get<ProductDetail>(`/api/products/${p2Id}?context=${A}`), secondTarget = { productId: p2Id, productVersionId: second.commonVersionId, contextProductVersionId: second.contextVersionId, productUseId: null, sampleVariant: '미선택 상품' };
    const draft: CampaignDraft = { title: 'HTTP 합성 행사', menus: ['one', 'two', 'three'].map(menu) };
    draft.menus[1].products = [secondTarget];
    draft.menus[2].products = [secondTarget];
    const id = (await mutate(admin, { command: 'save', contextId: A, taskId, campaignId: null, expectedRevision: 0, idempotencyKey: randomUUID(), draft }))[0];
    ...
    const preview = await admin.get(`/api/campaigns/${id}/preview`);
    check('G12-H05 preview excludes originals and conflict values', !JSON.stringify(preview).includes('PRIVATE_') && !JSON.stringify(preview).includes('12000円'), ['AC-12-05', 'SA-44']);
    const pubInput = { command: 'publish', contextId: A, taskId, campaignId: id, expectedRevision: (await detail(admin, id)).revision, idempotencyKey: randomUUID() }, publication = await mutate(admin, pubInput), versionId = publication[1];
    check('G12-H06 publication same intent returns original version', hash(await mutate(admin, pubInput)) === hash(publication), ['A20']);
    const publicV1 = await detail(brand, id), staffV1 = await detail(admin, id);
    check('G12-H07 confirmed unknown price and original conflicts distinct', publicV1.selected!.menus[0].conditions.cost.amount === null && publicV1.selected!.menus[0].confirmationIssues.map(c => c.field).sort().join(',') === 'date,menu_number,price' && staffV1.staff!.publishedOriginal!.menus[0].sourceStatements.filter(s => s.field === 'menu_number').map(s => s.rawValue).join(',') === '3,4' && staffV1.staff!.publishedOriginal!.menus[0].sourceStatements[1].rawValue === '12000円' && !JSON.stringify(publicV1).includes('PRIVATE_'), ['AC-12-05']);
    ...
    check('G12-H08 team readonly and foreign inquiry denied', (await team.mutate('/api/campaigns', selectInput)).status === 403 && (await foreign.send(`/api/campaigns/${id}`)).status === 404, ['A19']);
    await mutate(brand, selectInput);
    let d = await detail(brand, id);
    const chosenWorkspace = await brand.get<SubmissionWorkspace>(`/api/tasks/${taskId}/submissions`), chosenTask = await brand.get<TaskDetail>(`/api/tasks/${taskId}`);
    check('G12-H09 three offered one selected activates only two chosen requirements', chosenWorkspace.request.content.requirements.map(q => q.key).join(',') === 'proof,url' && chosenWorkspace.draftEvaluation?.required === 2 && chosenTask.requirementStatus.length === 2 && chosenWorkspace.request.source?.actorId === 'user-luna' && d.progress.filter(p => p.active).length === 1 && d.progress.map(p => p.missingRequired).join(',') === '2,0,0' && d.progress.every(p => p.state.application === 'not_applied' && p.state.selection === 'pending'), ['AC-12-01', 'AC-12-02']);
    const g07 = await brand.get<MaterialTable>(`/api/evidence/table?context=${A}`), g07p2 = g07.rows.find(r => r.productId === p2Id)!, p2detail = await brand.get<ProductDetail>(`/api/products/${p2Id}?context=${A}`), productList = await brand.get<ProductList>(`/api/products?context=${A}`);
    check('G12-H33 PRE01 actual G07 table product list detail exclude unselected P2 global requirements', g07p2.cells.filter(c => c.taskId === taskId).every(c => !c.applicable && c.status === 'not_applicable') && g07p2.counts.requested === 0 && g07p2.counts.missing === 0 && p2detail.materialCounts.requested === 0 && p2detail.materialCounts.missing === 0 && productList.items.find(p => p.productId === p2Id)!.materialCounts.missing === 0 && chosenWorkspace.products.map(p => p.productId).join(',') === 'product-serum', ['G12-PRE01', 'AC-12-01']);
    const actual = await actualSubmission(taskId), savedSubmission = hash(actual.snapshot);
    d = await detail(brand, id);
    check('G12-H10 actual partial G05 exact file/productUse reduces selected missing only', d.progress[0].missingRequired === 1 && d.progress[1].missingRequired === 0 && actual.snapshot.products.length === 1, ['AC-12-01', 'SA-46']);
    const fact = { axis: 'application', value: 'applied', requester: person, performedBy: external, occurredAt: null, source: source(), note: '신청 확인' };
    await command(admin, id, 'external', { campaignVersionId: versionId, menu: draft.menus[0].identity, fact });
    await command(admin, id, 'external', { campaignVersionId: versionId, menu: draft.menus[1].identity, fact: { ...fact, axis: 'selection', value: 'not_selected' } });
    await command(brand, id, 'participate', { campaignVersionId: versionId, response: 'decline', selectedMenus: [draft.menus[0].identity], providedBy: person, note: '불참 협의' });
    d = await detail(brand, id);
    ...
    const declinedWorkspace = await brand.get<SubmissionWorkspace>(`/api/tasks/${taskId}/submissions`);
    check('G12-H12 decline missing reminder0 preserves exact previous submission', declinedWorkspace.request.source?.noMaterials === true && declinedWorkspace.request.source.retainedRequirementKeys.join(',') === 'proof,url' && declinedWorkspace.requiresRebase && declinedWorkspace.draftEvaluation?.missing === 0 && declinedWorkspace.draftEvaluation.invalid === 0 && declinedWorkspace.products.length === 0 && d.progress.every(p => !p.reminderEligible && p.missingRequired === 0) && hash(await brand.get(`/api/submissions/${actual.snapshot.id}`)) === savedSubmission, ['AC-12-01']);
    const emptyTable = await brand.get<MaterialTable>(`/api/evidence/table?context=${A}`);
    check('G12-H34 approved zero request leaves G07 source counts and both products empty', emptyTable.columns.every(c => c.taskId !== taskId) && emptyTable.rows.filter(r => ['product-serum', p2Id].includes(r.productId)).every(r => r.counts.missing === 0), ['G12-PRE01', 'AC-12-01']);
    await banner(taskId, { width: 1440, height: 960 }, 'decline-desktop');
    check('G12-H28 actual desktop G05 generated-request and cancellation banner', true, ['AC-12-01', 'AC-12-02'], 'UI');
    await banner(taskId, { width: 390, height: 844 }, 'decline-mobile');
    check('G12-H29 actual 390px G05 generated-request and cancellation banner', true, ['AC-12-01', 'AC-12-02'], 'UI');
    const zeroRebase = await brand.mutate(`/api/tasks/${taskId}/submission-draft`, { command: 'rebase_apply', baseRequestId: declinedWorkspace.draft!.baseRequestId, targetRequestId: declinedWorkspace.request.id, expectedDraftRevision: declinedWorkspace.draft!.revision, carryAnswers: [], idempotencyKey: randomUUID() });
    assert.equal(zeroRebase.status, 200, await zeroRebase.clone().text());
    const zero = await brand.get<SubmissionWorkspace>(`/api/tasks/${taskId}/submissions`);
    check('G12-H30 explicit zero-request rebase removes only current answer and product selections', zero.draft!.content.answers.length === 0 && zero.draft!.content.productSelections.length === 0 && !zero.requiresRebase && hash(await brand.get(`/api/submissions/${actual.snapshot.id}`)) === savedSubmission, ['AC-12-01', 'SA-46']);
    await command(brand, id, 'participate', { campaignVersionId: versionId, response: 'participate', selectedMenus: [draft.menus[0].identity], providedBy: person, note: '다시 선택' });
    let selectedAgain = await brand.get<SubmissionWorkspace>(`/api/tasks/${taskId}/submissions`);
    check('G12-H31 reselect requires explicit request/product rebase', selectedAgain.requiresRebase && selectedAgain.products.length === 1 && selectedAgain.draft!.content.answers.length === 0, ['SA-46']);
    const rebaseBack = await brand.mutate(`/api/tasks/${taskId}/submission-draft`, { command: 'rebase_apply', baseRequestId: selectedAgain.draft!.baseRequestId, targetRequestId: selectedAgain.request.id, expectedDraftRevision: selectedAgain.draft!.revision, carryAnswers: [], idempotencyKey: randomUUID() });
    assert.equal(rebaseBack.status, 200, await rebaseBack.clone().text());
    selectedAgain = await brand.get<SubmissionWorkspace>(`/api/tasks/${taskId}/submissions`);
    const fullContent = { ...actual.snapshot.content, answers: [...actual.snapshot.content.answers.map(a => ({ ...a, requestId: selectedAgain.request.id })), { requestId: selectedAgain.request.id, requirementKey: 'url', productId: null, type: 'link', input: { url: 'https://example.test/published', description: '게시 URL', contentFixed: false, fixedReference: null } }] };
    const saveFull = await brand.mutate(`/api/tasks/${taskId}/submission-draft`, { command: 'save', baseRequestId: selectedAgain.request.id, expectedDraftRevision: selectedAgain.draft!.revision, content: fullContent, idempotencyKey: randomUUID() });
    assert.equal(saveFull.status, 200, await saveFull.clone().text());
    selectedAgain = await brand.get<SubmissionWorkspace>(`/api/tasks/${taskId}/submissions`);
    const full = await brand.mutate(`/api/tasks/${taskId}/submissions`, { baseRequestId: selectedAgain.request.id, expectedDraftRevision: selectedAgain.draft!.revision, expectedTaskRevision: selectedAgain.taskRevision, mode: 'full', idempotencyKey: randomUUID() });
    assert.equal(full.status, 201, await full.clone().text());
    const fullSnapshot = await brand.get<SubmissionSnapshot>(`/api/submissions/${(await full.json()).ids[0]}`);
    check('G12-H32 full actual submission requires selected menu only and keeps old exact file/productUse', fullSnapshot.evaluation.required === 2 && fullSnapshot.evaluation.missing === 0 && hash(await brand.get(`/api/submissions/${actual.snapshot.id}`)) === savedSubmission, ['AC-12-01', 'SA-46']);
    const physical = { campaignVersionId: versionId, menu: draft.menus[0].identity, physicalKey: 'shoot' }, provenance = { performedBy: external, occurredAt: null, evidence: [actual.reference], note: '합성 증빙' };
    await command(brand, id, 'physical', { ...physical, fact: { ...provenance, kind: 'tracking', carrier: '합성택배', trackingNumber: '0000123', trackingUrl: 'https://example.test/tracking' } });
    d = await detail(brand, id);
    check('G12-H13 uploaded proof and tracking never dispatch receipt', d.progress[0].physical.every(p => p.receiptFacts === 0 && p.dispatchFacts === 0), ['AC-12-04']);
    const dispatch = (await command(brand, id, 'physical', { ...physical, fact: { ...provenance, kind: 'dispatch', quantity: '1', unit: '개', carrier: '합성택배', trackingNumber: '0000123' } }))[1];
    await command(brand, id, 'physical', { ...physical, fact: { ...provenance, kind: 'receipt', quantity: '0', unit: '개', dispatchFactIds: [dispatch] } });
    d = await detail(brand, id);
    check('G12-H14 shooting receipt0 is explicit observation not inferred fulfillment; distribution100 unchanged', d.progress[0].physical[0].receiptFacts === 1 && d.progress[0].physical[0].fulfillment === 'not_inferred' && d.progress[0].physical[1].receiptFacts === 0 && d.progress[0].physical[1].definition.requestedQuantity === '100', ['AC-12-03', 'AC-12-04']);
    const followup = { campaignVersionId: versionId, menu: draft.menus[0].identity, followupKey: 'photo', source: actual.reference, receivedBy: external, occurredAt: null, note: '외부 수령 기록' };
    await command(brand, id, 'followup', followup);
    d = await detail(brand, id);
    const fileUrl = d.progress[0].followups[0].facts[0].source.files[0].originalUrl;
    check('G12-H15 followup photo received while separate URL deadline remains pending', d.progress[0].followups.map(f => f.status).join(',') === 'received,pending' && d.progress[0].followups[1].definition.deadline.value === '2026-11-01', ['SA-49']);
    check('G12-H16 exact current-auth original bytes through campaign reference', byteHash(Buffer.from(await (await brand.send(fileUrl)).arrayBuffer())) === byteHash(png), ['D02']);
    const beforeBad = await fixture<CampaignFixtureSnapshot>({ campaignId: id }), bad = { command: 'followup', contextId: A, taskId, campaignId: id, expectedRevision: d.revision, idempotencyKey: randomUUID(), ...followup, source: { ...actual.reference, contentHash: 'f'.repeat(64) } };
    check('G12-H17 wrong exact target returns404 and writes no partial facts', (await brand.mutate('/api/campaigns', bad)).status === 404 && (await fixture<CampaignFixtureSnapshot>({ campaignId: id })).rowsSha256 === beforeBad.rowsSha256, ['A19', 'A20']);
    const publicBefore = await detail(brand, id);
    draft.title = 'PRIVATE_EDIT';
    await command(admin, id, 'save', { draft });
    check('G12-H18 internal draft edits leave every brand body field and publicCAS unchanged', hash(await detail(brand, id)) === hash(publicBefore), ['A19']);
    const racer = new Client(mode === 'sqlite' ? auxPort : port);
    if (mode === 'sqlite')
        await start(auxPort);
    await racer.login('admin@example.test');
    const rev = (await detail(admin, id)).revision;
    const race = await Promise.all([admin, racer].map((c, i) => c.mutate('/api/campaigns', { command: 'save', contextId: A, taskId, campaignId: id, expectedRevision: rev, idempotencyKey: randomUUID(), draft: { ...draft, title: '경합 ' + i } })));
    check('G12-H19 two-session or actual twoPID CAS exactly one winner', race.map(r => r.status).sort().join(',') === '200,409', ['A20']);
    ...
    const concurrent = { ...pubInput, expectedRevision: (await detail(admin, id)).revision, idempotencyKey: randomUUID() }, replies = await Promise.all([admin, racer].map(c => c.mutate('/api/campaigns', concurrent))), ids = await Promise.all(replies.map(r => r.json()));
    const observed = await fixture<CampaignFixtureSnapshot>({ campaignId: id });
    check('G12-H20 same-intent publication one version event receipt', replies.every(r => r.status === 200) && hash(ids[0]) === hash(ids[1]) && observed.rows.find(r => r.kind === 'domainEvent')!.rows.filter(r => 'sourceVersionId' in r.data && r.data.sourceVersionId === ids[0].ids[1]).length === 1 && observed.rows.find(r => r.kind === 'commandReceipt')!.rows.filter(r => 'key' in r.data && r.data.key === hash(`user-admin:${A}:campaign.publish:${id}:${concurrent.idempotencyKey}`)).length === 1, ['A20']);
    const extension = await fixture<CampaignFixtureSnapshot>({ campaignId: id, action: 'extension' });
    persist('extension', extension);
    check('G12-H21 stored valid nested extras retained but omitted from staff/brand DTOs', JSON.stringify(extension).includes('G12_STORED_CANARY') && !JSON.stringify(await detail(brand, id)).includes('CANARY') && !JSON.stringify(await detail(admin, id)).includes('CANARY'), ['A19']);
    const beforeRestart = await detail(brand, id), staffBefore = await detail(admin, id), rowsBefore = await fixture<CampaignFixtureSnapshot>({ campaignId: id });
    persist('before-restart', rowsBefore);
    if (mode === 'sqlite') {
        await stop(auxPort);
        await stop(port);
        await start();
        const again = new Client(), againAdmin = new Client();
        await again.login('luna@example.test');
        await againAdmin.login('admin@example.test');
        check('G12-H22 actual newPID restart and real relogin', processes[0].pid !== processes.at(-1)!.pid, ['D10'], 'PROCESS');
        check('G12-H23 private/public versions/facts/productUse/receipt persisted byte-exact', hash(await detail(again, id)) === hash(beforeRestart) && hash(await detail(againAdmin, id)) === hash(staffBefore) && (await fixture<CampaignFixtureSnapshot>({ campaignId: id })).rowsSha256 === rowsBefore.rowsSha256, ['D10']);
        check('G12-H24 historical source file bytes survive restart', byteHash(Buffer.from(await (await again.send(fileUrl)).arrayBuffer())) === byteHash(png), ['D02', 'D10']);
    }
    else
        skippedIds.push('G12-H22', 'G12-H23', 'G12-H24');
    ...
    check('G12-H25 current membership revoke blocks history/file/successful receipt replay', revoked.status === 200 && (await brand.send(`/api/campaigns/${id}`)).status === 404 && (await brand.send(fileUrl)).status === 404 && (await brand.mutate('/api/campaigns', selectInput)).status === 404, ['A19', 'D02']);
    check('G12-H26 revoke does not erase immutable public or fact history', (await fixture<CampaignFixtureSnapshot>({ campaignId: id })).rowsSha256 === rowsBefore.rowsSha256, ['D10'], 'DB_FIXTURE');
    const corrupt = await fixture<CampaignFixtureSnapshot>({ campaignId: id, action: 'corrupt' });
    persist('corrupt', corrupt);
    const corrupted = await admin.send(`/api/campaigns/${id}`);
    check('G12-H27 stored malformed known nested amount fails503 without rawcanary', corrupted.status === 503 && !(await corrupted.text()).includes('KNOWN_CORRUPTION'), ['A19']);
    reachedEnd = true;
}
catch (error) {
