...
const root = path.resolve(process.env.AI_REVIEW_HISTORY_ROOT ?? '.local/g16-history');
...
const report = path.resolve(process.env.AI_REVIEW_HISTORY_REPORT ?? path.join(directory, 'report.json'));
...
try {
    ...
    const helperPath = path.join(oldSource, 'scripts', 'g16-history-producer.ts');
    ...
    check('P08 actual accepted G15 snapshot feeds new analysis preserving exact old immutable rows', result.detail.snapshotHash === prior.snapshotHash && result.detail.state === 'finished' && prior.rows.every((r: {
        kind: string;
        id: string;
