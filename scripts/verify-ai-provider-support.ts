...
export class ProviderHarness {
 readonly cwd=process.cwd();readonly candidate=execFileSync('git',['rev-parse','HEAD'],{encoding:'utf8'}).trim();readonly directory:string;readonly database:string;readonly files:string;readonly control:string;readonly calls:string;readonly loader:string;
 readonly processes:{pid?:number;port:number;argv:string[];log:string;stopped?:boolean}[]=[];readonly transcript:{url:string;method:string;status:number;path:string;sha256:string}[]=[];private children=new Map<number,ChildProcess>();
 ...
 async setup(){if(this.mode==='sqlite'){const db=openDatabase(this.database,true);migrate(db);const repo=createSqliteRepository(db);await seed(repo);repo.close();}}
 async start(kind:'fault'|'missing'|'live'='fault',port=this.port){await free(port);const origin=`http://127.0.0.1:${port}`;const env:NodeJS.ProcessEnv={PATH:process.env.PATH,NODE_ENV:'production',LANG:process.env.LANG,TZ:'Asia/Seoul',DATA_SOURCE:this.mode,DATABASE_FILE:this.database,FILE_STORAGE_DIR:this.files,IMPORT_STORAGE_DIR:path.join(this.directory,'imports'),APP_ORIGIN:origin,SESSION_COOKIE_NAME:`gs_hale_g17_${port}`,NEXT_TELEMETRY_DISABLED:'1',OPENAI_API_KEY:kind==='fault'?'SYNTHETIC_FAULT_KEY_MUST_NOT_LEAK':'',OPENAI_MODEL:'gpt-6-astra',OPENAI_BASE_URL:'https://api.openai.com/v1',G17_FAULT_CONTROL:this.control,G17_FAULT_CALLS:this.calls};
 if(kind==='live'){const values=parseEnv(readFileSync(path.resolve(process.env.AI_PROVIDER_ENV_FILE??'.env'),'utf8')),configured=parseAiConfig(values);assert(configured.apiKey&&configured.model,'live configuration incomplete');env.OPENAI_API_KEY=configured.apiKey;env.OPENAI_MODEL=configured.model;env.OPENAI_BASE_URL=configured.baseURL;writeFileSync(path.join(this.directory,'safe-live-config.json'),JSON.stringify({model:configured.model,baseURL:configured.baseURL,keyPresent:true,at:new Date().toISOString()}),{mode:0o600});}
 const args=[...(kind==='fault'?['--require',this.loader]:[]),'node_modules/next/dist/bin/next','start','--hostname','127.0.0.1','--port',String(port)],log=path.join(this.directory,`server-${this.processes.length+1}.log`),out=createWriteStream(log,{mode:0o600}),child=spawn(process.execPath,args,{cwd:this.cwd,env,stdio:['ignore','pipe','pipe']});this.children.set(port,child);this.processes.push({pid:child.pid,port,argv:[process.execPath,...args],log});child.stdout!.pipe(out,{end:false});child.stderr!.pipe(out,{end:false});child.once('exit',()=>out.end());for(let i=0;i<200;i++){if(child.exitCode!==null)throw Error('owned server exited');try{if((await fetch(origin+'/api/health')).ok)return;}catch{}await sleep(50);}throw Error('server readiness');}
 async stop(port=this.port){const child=this.children.get(port);if(!child)return;const done=new Promise<void>(r=>child.once('exit',()=>r()));child.kill('SIGTERM');if(child.exitCode===null&&child.signalCode===null)await done;this.processes.find(p=>p.pid===child.pid)!.stopped=true;this.children.delete(port);await free(port);}
 async stopAll(){for(const p of [...this.children.keys()])await this.stop(p);}
...
export class ProviderClient {
 cookie='';constructor(readonly harness:ProviderHarness,readonly port:number){}get origin(){return `http://127.0.0.1:${this.port}`;}
 async send(url:string,method='GET',body?:unknown,csrf=''){const r=await fetch(this.origin+url,{method,redirect:'manual',headers:{Cookie:this.cookie,...method!=='GET'?{'Content-Type':'application/json',Origin:this.origin,'X-CSRF-Token':csrf}:{}},body:body===undefined?undefined:JSON.stringify(body)});if(r.headers.get('set-cookie'))this.cookie=r.headers.get('set-cookie')!.split(';')[0];await this.harness.capture(r,method,url);return r;}
 ...
 async mutate(url:string,body:unknown,method='POST'){const {csrfToken}=await this.get<{csrfToken:string}>('/api/auth/csrf');return this.send(url,method,body,csrfToken);}
...
export async function prepare(brand:ProviderClient,gsg:ProviderClient,c=content()){const input=await brand.ok<AiInputDetail>('/api/ai-input',{contextId:ctx,visibility:'context',content:c,idempotencyKey:randomUUID()}),extraction=await brand.ok<{runId:string}>(`/api/ai-input/${input.id}/extract`,{versionId:input.version.id,expectedRunId:null,idempotencyKey:randomUUID()}),w=await gsg.get<AiReviewWorkspace>(`/api/ai-review/inputs/${input.id}`);return {input,body:{inputVersionId:input.version.id,extractionRunId:extraction.runId,expectedRunId:w.runs[0]?.id??null,corpusReleaseId:w.corpus.id,corpusManifestHash:w.corpus.manifestHash,engine:'provider',idempotencyKey:randomUUID()}};}
export function providerStart(c:ProviderClient,f:Awaited<ReturnType<typeof prepare>>){return c.ok<{runId:string;detail:AiReviewDetail}>(`/api/ai-review/inputs/${f.input.id}/start`,f.body);}
