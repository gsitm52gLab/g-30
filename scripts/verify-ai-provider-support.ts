...
export class ProviderHarness {
 ...
 if(kind==='live'){const values=parseEnv(readFileSync('/Users/evan/workspace/gs-hale/.env','utf8')),configured=parseAiConfig(values);assert(configured.apiKey&&configured.model,'live configuration incomplete');env.OPENAI_API_KEY=configured.apiKey;env.OPENAI_MODEL=configured.model;env.OPENAI_BASE_URL=configured.baseURL;writeFileSync(path.join(this.directory,'safe-live-config.json'),JSON.stringify({model:configured.model,baseURL:configured.baseURL,keyPresent:true,at:new Date().toISOString()}),{mode:0o600});}
