...
export async function completionFixture(repo: RecordRepository, input: CompletionFixtureInput) {
    ...
    return repo.transaction(async (s) => {
        ...
        if (input.action === 'extension' || input.action === 'corruption') {
            const original = (await s.get('completionSnapshot', input.snapshotId!))!;
            if (!original)
                throw Error('own completion missing');
            const sequence = Math.max(...(await s.list('completionSnapshot', task.contextId!)).filter(r => r.data.taskId === task.id).map(r => r.data.sequence)) + 1;
            ...
            fixtureSnapshotId = (await s.create('completionSnapshot', { id: randomUUID(), contextId: task.contextId, data: data as unknown as CompletionRecords['completionSnapshot'] })).id;
        }
        ...
        const rows = (await asyncMap(kinds, async (kind) => ({ kind, rows: (await s.list(kind, task.contextId!)) })));
