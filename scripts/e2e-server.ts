...
if (!["mock", "sqlite"].includes(mode)) throw new Error("Invalid E2E_DATA_SOURCE");
mkdirSync(".local", { recursive: true });
const directory = mkdtempSync(path.resolve(".local/e2e-"));
const filename = path.join(directory, "fixture.db");
if (mode === "sqlite") { const db = openDatabase(filename, true); migrate(db); const repository = createSqliteRepository(db); await seed(repository); repository.close(); }
const server = spawn(process.execPath, ["node_modules/next/dist/bin/next", "start", "--hostname", "127.0.0.1", "--port", process.env.E2E_PORT || "4101"], {
  stdio: "inherit", env: { ...process.env, DATA_SOURCE: mode, DATABASE_FILE: filename, OPENAI_API_KEY: "", OPENAI_MODEL: "", OPENAI_BASE_URL: "https://api.openai.com/v1", NEXT_TELEMETRY_DISABLED: "1" },
});
