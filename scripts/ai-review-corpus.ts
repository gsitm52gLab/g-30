...
if (!filename || extra.length || operation !== 'inspect' && operation !== 'publish' || operation === 'inspect' && expected !== undefined || operation === 'publish' && !expected) {
    console.error('Usage: tsx scripts/ai-review-corpus.ts inspect DB_FILE | publish DB_FILE EXPECTED_RELEASE_ID_OR_NONE [VALIDATED_RELEASE_JSON]');
    process.exit(1);
}
...
try {
    ...
    if (operation === 'inspect') {
        const release = await repository.transaction(async (s) => (await loadCorpus(s)));
        ...
        if (bytes && bytes.length > 1024 * 1024)
            throw new Error('Manifest exceeds technical input limit');
        const release: unknown = bytes ? JSON.parse(bytes.toString('utf8')) : curatedRelease();
        console.log(JSON.stringify({ operation, ...await repository.transaction(async (s) => (await publishCorpus(s, release, expected === 'NONE' ? null : expected))) }));
    }
}
catch (error) {
    console.error(JSON.stringify({ operation, status: 'failed', code: error instanceof AuthError || error instanceof StoreError ? error.code : 'CORPUS_IMPORT_FAILED' }));
    process.exitCode = 1;
}
finally {
    repository?.close();
