...
async function upload(id: string, items: FileItem[], client = brand) {
    const w = await workspace(client, id), csrf = await client.get<{
        csrfToken: string;
    ...
    const response = await fetch(client.origin + url, { method: 'POST', headers: { Cookie: client.cookie, Origin: client.origin, 'X-CSRF-Token': csrf.csrfToken }, body: form });
    transcript.push({ method: 'POST multipart', path: url, status: response.status });
    return response;
}
...
try {
    ...
    if (process.env.SUBMISSIONS_PROJECTION_ONLY === '1') {
        const observed = await fixture<SubmissionFixtureSnapshot>({ taskId: tid, action: 'poison_request' });
        const response = await brand.send(`/api/tasks/${tid}/submissions`), raw = await response.text();
        writeFileSync(`${reportFile}.projection-response.json`, raw, { mode: 0o600 });
        check('poisoned immutable request remains stored', observed.request.canaryStored, ['G05-SRV-01'], 'DB_FIXTURE');
        check('actual HTTP evaluation drops stored malformed nested values', response.status === 200 && !raw.includes('G05_HTTP_EVALUATION_CANARY'), ['G05-SRV-01', 'AC-05-05', 'A19']);
        const parsed = JSON.parse(raw) as SubmissionWorkspace;
        check('malformed request remains confirmation-needed not structurally complete', parsed.draftEvaluation?.canSubmitFull === false && Number(parsed.draftEvaluation?.invalid) > 0, ['G05-SRV-01']);
        const attempt = await brand.mutate(`/api/tasks/${tid}/submission-draft`, { command: 'save', baseRequestId: parsed.request.id, expectedDraftRevision: 0, content: blankDraft(), idempotencyKey: randomUUID() });
        check('malformed authoritative scope blocks writes instead of becoming common', attempt.status === 409 && (await attempt.json()).error.code === 'REQUEST_INVALID', ['G05-SRV-01']);
    ...
    for (const p of [...children.keys()])
        await stop(p);
    const report = { candidate_commit: candidate, runner_sha256: hash(readFileSync('scripts/verify-submissions-http.ts', 'utf8')), fixture_runner_sha256: hash(readFileSync('scripts/verify-submissions-fixtures.ts', 'utf8')), status: failure ? 'FAIL' : 'PASS', mode, cwd: process.cwd(), startedAt, finishedAt: new Date().toISOString(), failure, unit: 'assertion', pass: checks.filter(c => c.status === 'PASS').length, fail: checks.filter(c => c.status === 'FAIL').length + (failure && !checks.some(c => c.status === 'FAIL') ? 1 : 0), skip: 0, not_run: checksReachedEnd ? 'Browser/UI/RSC and independent verification remain NOT_RUN' : 'remaining assertions interrupted; browser/UI/RSC NOT_RUN', checks, transcript, processes, resources: { primaryPort: port, auxPort, database, files }, fixture_boundary: process.env.SUBMISSIONS_PROJECTION_ONLY === '1' ? 'Synthetic immutable request appended through private fixture IPC/repository for output-canary probe; no public test endpoint' : 'Actual HTTP producer; private repository read-only snapshots and synthetic mock IPC only; no credentials/cookies logged' };
