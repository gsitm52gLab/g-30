...
try {
    ...
    check('C04 actual tracking stales basis409 while task revision unchanged and no completion written', staleReply.status === 409 && (await staleReply.json()).code === 'BASIS_CHANGED' && w.taskRevision === taskBefore.revision && w.history.length === 0, ['AC-11-02', 'AC-11-05']);
