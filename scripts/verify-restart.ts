...
const evidence = { requirement: "AC-00-02", status: "PASS", passed: 3, failed: 0, skipped: 0, processRestart: true, authentication: "NOT_IMPLEMENTED_G01", results };
const report = process.env.RESTART_REPORT || path.join(directory, "restart.json");
mkdirSync(path.dirname(report), { recursive: true }); writeFileSync(report, JSON.stringify(evidence, null, 2));
