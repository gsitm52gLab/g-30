...
async function draft(c = brand) { const response = await c.mutate('/api/inquiries', { contextId: A, taskId: null, idempotencyKey: randomUUID() }); assert.equal(response.status, 201, await response.clone().text()); return (await response.json()) as {
...
async function upload(c: Client, id: string, items: {
    key: string;
    name: string;
    bytes: Buffer;
}[], visibility = 'public') {
    const csrf = await c.get<{
        csrfToken: string;
    }>('/api/auth/csrf'), form = new FormData();
    for (const item of items) {
        form.append('files', new Blob([new Uint8Array(item.bytes)], { type: 'image/png' }), item.name);
        form.append('clientItemIds', item.key);
    }
    const path = `${url(id)}/files?visibility=${visibility}`, r = await fetch(c.origin + path, { method: 'POST', headers: { Cookie: c.cookie, Origin: c.origin, 'X-CSRF-Token': csrf.csrfToken }, body: form });
    await capture(r, 'POST multipart', path);
    return r;
}
async function ready(c: Client, id: string, visibility = 'public', key = randomUUID()) {
    const r = await upload(c, id, [{ key, name: 'source.png', bytes: png }], visibility);
    assert.equal(r.status, 200, await r.clone().text());
    const item = (await r.json()).items[0] as InquiryUploadResult;
    assert.equal(item.state, 'ready');
    if (item.state !== 'ready')
        throw Error('ready expected');
    return item.file;
}
const streams: {
    path: string;
...
async function stream(c: Client, id: string, cursor: string) {
    const abort = new AbortController(), path = `${url(id)}/stream?after=${encodeURIComponent(cursor)}`, response = await fetch(c.origin + path, { headers: { Cookie: c.cookie }, signal: abort.signal });
    assert.equal(response.status, 200);
    const record = { path, raw: '', frames: [] as unknown[], closed: false };
    streams.push(record);
    ...
    const done = (async () => { try {
        while (true) {
            ...
            while ((end = pending.indexOf('\n\n')) >= 0) {
                ...
                if (data)
                    record.frames.push(JSON.parse(data.slice(6)));
            }
        }
    }
    catch (error) {
    ...
    return { record, close: async () => { abort.abort(); await done; }, wait: async (predicate: () => boolean) => { for (let n = 0; n < 120 && !predicate(); n++)
            await new Promise(r => setTimeout(r, 50)); assert(predicate(), 'expected live stream frame'); } };
}
...
try {
    ...
    await start();
    for (const [c, name] of [[brand, 'team'], [gsg, 'operator'], [admin, 'admin'], [peer, 'luna'], [foreign, 'wave']] as const)
        await c.login(`${name}@example.test`);
    ...
    const item = { key: randomUUID(), name: 'selected.png', bytes: png }, bad = { key: randomUUID(), name: 'bad.png', bytes: Buffer.from('not image') }, partial = await upload(brand, id, [item, bad]);
    assert.equal(partial.status, 200);
    const items = (await partial.json()).items as InquiryUploadResult[];
    ...
    if (items[0].state !== 'ready')
        throw Error('ready selected');
    const selected = items[0].file, unused = await ready(brand, id);
    const replayUpload = await upload(brand, id, [item]);
    check('G09-H06 same upload intent returns same file', (await replayUpload.json()).items[0].file.id === selected.id, ['D02', 'A20']);
    const changed = await upload(brand, id, [{ ...item, name: 'changed.png' }]);
    check('G09-H07 changed upload intent conflicts', (await changed.json()).items[0].error.code === 'CONFLICT', ['D02', 'A20']);
    ...
    for (let i = 1; i < 5; i++)
        await command(brand, id, { command: 'question', expectedRevision: (await active(brand, id)).revision, content: content('질문 ' + i) });
    let current = await active(gsg, id);
    for (const q of current.questions.slice(0, 4))
        await command(gsg, id, { command: 'answer', questionId: q.id, expectedQuestionRevision: q.revision, content: content('명시 답변 ' + q.id) });
    current = await active(gsg, id);
    const waiting = current.questions.find(q => q.state !== 'resolved')!;
    ...
    for (const [c, name] of [[brand, 'brand'], [gsg, 'gsg']] as const) {
        const d = await active(c, id), l = await list(c);
        check(`G09-H13 ${name} five four one with actual summary`, hash(d.counts) === hash({ questions: 5, answered: 4, unresolved: 1, waitingGsg: 0, waitingBrand: 0, externalWaiting: 1 }) && hash(d.counts) === hash(l.counts) && d.nextChecks[0].date === '2026-10-05', ['AC-09-01']);
    }
    const beforeInternal = await active(brand, id), internal = await ready(gsg, id, 'internal');
    await command(gsg, id, { command: 'internal_note', questionId: null, content: content('G09_INTERNAL_MARKER', [internal.id]) });
    ...
    const staffInternal = await detail(gsg, id);
    assert('internalMessages' in staffInternal);
    const internalMessage = staffInternal.internalMessages[0];
    ...
    const has = (frames: unknown[], id: string | null) => frames.some(f => (f as StaffInquiryEvent).type === 'message' && 'message' in (f as StaffInquiryEvent) && (f as {
        message: {
            id: string;
        };
    }).message.id === id);
    await live.wait(() => has(live.record.frames, m1.messageId) && has(live.record.frames, m2.messageId));
    await liveStaff.wait(() => has(liveStaff.record.frames, m1.messageId) && has(liveStaff.record.frames, m2.messageId));
    ...
    await live.close();
    await liveStaff.close();
    const offline = await active(brand, id);
    const missed: InquiryCommandResult[] = [];
    for (let i = 0; i < 2; i++)
        missed.push(await command(gsg, id, { command: 'message', kind: 'comment', questionId: null, content: content('MISSED_' + i) }));
    ...
    const reconnected = await stream(brand, id, offline.cursor);
    await reconnected.wait(() => missed.every(m => has(reconnected.record.frames, m.messageId)));
    check('G09-H20 reconnect SSE exact missing two', reconnected.record.frames.length === 2, ['AC-09-03'], 'SSE');
    ...
    const send = { command: 'message', kind: 'comment', questionId: null, content: content('LOST_RESPONSE'), idempotencyKey: randomUUID() };
    const sentLost = await command(brand, id, send), again = await command(brand, id, { ...send, idempotencyKey: randomUUID() });
    ...
    const read = { command: 'read', throughMessageId: sentLost.messageId, idempotencyKey: randomUUID() };
    await command(gsg, id, read);
    await command(gsg, id, { ...read, idempotencyKey: randomUUID() });
    ...
    const created = await admin.mutate('/api/tasks', { targets: [{ contextId: A, ownerId: 'user-gsg', assigneeId: 'user-team', coAssigneeIds: [], productIds: [] }], category: 'spot', content: c, idempotencyKey: randomUUID() });
    assert.equal(created.status, 201, await created.clone().text());
    const taskId = (await created.json()).ids[0] as string;
    const revision = (await active(gsg, id)).revision;
    check('G09-H25 failed stale association preserves created task', (await gsg.mutate(url(id), { command: 'link_task', taskId, expectedRevision: revision - 1, idempotencyKey: randomUUID() })).status === 409 && (await admin.send(`/api/tasks/${taskId}`)).status === 200, ['AC-09-04']);
    await command(gsg, id, { command: 'link_task', taskId, expectedRevision: revision });
    check('G09-H26 unpublished linked task hidden without changing original messages', (await active(brand, id)).task === null && hash((await fixture<InquiryFixtureSnapshot>({ conversationId: id })).messages) === hash(beforeLink.messages), ['AC-09-04']);
    const task = await admin.get<TaskDetail>(`/api/tasks/${taskId}`);
    const published = await admin.mutate(`/api/tasks/${taskId}`, { command: 'publish', expectedRevision: task.task.revision, idempotencyKey: randomUUID() });
    ...
    const remaining = (await active(gsg, id)).questions.find(q => q.id === waiting.id)!;
    await command(gsg, id, { command: 'answer', questionId: remaining.id, expectedQuestionRevision: remaining.revision, content: content('외부 일정 답변') });
    const resolved = await active(brand, id);
    await command(brand, id, { command: 'question', expectedRevision: resolved.revision, content: content('해결 후 새 질문') });
    const reopened = await active(brand, id);
    check('G09-H28 reopen preserves previous resolution and history', reopened.lastResolvedAt === resolved.lastResolvedAt && !!resolved.lastResolvedAt && reopened.counts.answered === 5 && reopened.counts.unresolved === 1 && reopened.history.length > resolved.history.length, ['AC-09-05']);
    const racer = new Client(mode === 'sqlite' ? auxPort : port);
    if (mode === 'sqlite')
        await start(auxPort);
    await racer.login('team@example.test');
    ...
    const extended = await fixture<InquiryFixtureSnapshot>({ conversationId: id, action: 'extend' });
    persist('stored-extension', extended);
    check('G09-H31 unknown stored extension stays out of API', JSON.stringify(extended).includes('G09_STORED_EXTENSION') && !JSON.stringify(await active(brand, id)).includes('G09_STORED_EXTENSION'), ['A19']);
    const beforeRestart = await active(brand, id), saved = await fixture<InquiryFixtureSnapshot>({ conversationId: id }), privateDraft = await draft(), privateFile = await ready(brand, privateDraft.conversationId);
    persist('before-restart', { beforeRestart, saved, privateDraft, privateFile });
    if (mode === 'sqlite') {
        await stop(auxPort);
        await stop(port);
        await start();
        const relog = new Client();
        await relog.login('team@example.test');
        ...
        brand.cookie = relog.cookie;
        await gsg.login('operator@example.test');
        await admin.login('admin@example.test');
    }
    const revokeStream = await stream(brand, id, (await active(brand, id)).cursor), members = await admin.get<{
        members: {
            id: string;
            revision: number;
            data: {
    ...
    await command(gsg, id, { command: 'message', kind: 'comment', questionId: null, content: content('AFTER_REVOKE_SECRET') });
    await revokeStream.wait(() => revokeStream.record.frames.some(f => !!(f as {
        error?: unknown;
    }).error));
    check('G09-H36 current revocation terminates actual stream without new protected message', !revokeStream.record.raw.includes('AFTER_REVOKE_SECRET') && revokeStream.record.frames.some(f => (f as {
        error?: {
            code: string;
        };
    }).error?.code === 'NOT_FOUND'), ['A19', 'D04', 'AC-09-03'], 'SSE');
    await revokeStream.close();
    ...
    const malformed = await fixture({ conversationId: id, action: 'malform' });
    persist('malformed-stored', malformed);
    const invalid = await gsg.send(url(id));
    check('G09-H38 malformed known store returns safe503', invalid.status === 503 && !JSON.stringify(await invalid.json()).includes('G09_MALFORMED_TITLE'), ['A19']);
    reachedEnd = true;
}
catch (error) {
    failure = error instanceof Error ? error.stack ?? error.message : 'unknown failure';
    process.exitCode = 1;
}
finally {
    for (const p of [...children.keys()])
        await stop(p);
    persist('stream-frames', streams);
