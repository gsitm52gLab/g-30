...
try {
  ...
  check('P04 only0015 applied with old rows and SQL hashes unchanged', JSON.stringify(migrate(db)) === JSON.stringify({ applied: 1, total: 14 }) && JSON.stringify(db.prepare('SELECT * FROM records ORDER BY kind,id').all()) === JSON.stringify(prior.rows) && JSON.stringify(db.prepare("SELECT * FROM schema_migrations WHERE name<'0015' ORDER BY name").all()) === JSON.stringify(prior.migrations));
  ...
  check('P06 repeat seed and migration exact all rows', JSON.stringify(migrate(db)) === JSON.stringify({ applied: 0, total: 14 }) && JSON.stringify(db.prepare('SELECT * FROM records ORDER BY kind,id').all()) === beforeRepeat);
