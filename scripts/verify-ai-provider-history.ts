...
try {
  ...
  check('P04 additive0014and0015 preserve old rows and exact prior SQL ledger', JSON.stringify(migrate(db)) === JSON.stringify({ applied: 2, total: 15 }) && JSON.stringify(db.prepare('SELECT * FROM records ORDER BY kind,id').all()) === JSON.stringify(prior.rows) && JSON.stringify((db.prepare('SELECT * FROM schema_migrations ORDER BY name').all() as {name:string}[]).filter(row => prior.migrations.some((old: {name:string}) => old.name === row.name))) === JSON.stringify(prior.migrations));
