...
const root = path.resolve(process.env.AI_PROVIDER_HISTORY_ROOT ?? '.local/g17-history'); mkdirSync(root, { recursive: true });
...
const report = path.resolve(process.env.AI_PROVIDER_HISTORY_REPORT ?? path.join(directory, 'report.json')); mkdirSync(path.dirname(report), { recursive: true }); mkdirSync(oldSource);
...
try {
  ...
  symlinkSync(path.join(cwd, 'node_modules'), path.join(oldSource, 'node_modules'), 'dir'); mkdirSync(path.join(oldSource, 'scripts')); const helperPath = path.join(oldSource, 'scripts', 'g17-history-producer.ts'); writeFileSync(helperPath, helper);
  ...
  check('P04 additive0014and0015and0016 preserve old rows and exact prior SQL ledger', JSON.stringify(migrate(db)) === JSON.stringify({ applied: 3, total: 16 }) && JSON.stringify(db.prepare('SELECT * FROM records ORDER BY kind,id').all()) === JSON.stringify(prior.rows) && JSON.stringify((db.prepare('SELECT * FROM schema_migrations ORDER BY name').all() as {name:string}[]).filter(row => prior.migrations.some((old: {name:string}) => old.name === row.name))) === JSON.stringify(prior.migrations));
  ...
  check('P06 repeat seed and migration exact all rows', JSON.stringify(migrate(db)) === JSON.stringify({ applied: 0, total: 16 }) && JSON.stringify(db.prepare('SELECT * FROM records ORDER BY kind,id').all()) === beforeRepeat);
  ...
  const historicalWithoutAdditive=JSON.parse(JSON.stringify(historical));delete historicalWithoutAdditive.provider;
  check('P08 exact historical result, human review and all stored immutable rows preserved',JSON.stringify(historicalWithoutAdditive)===JSON.stringify(prior.analysisSaved)&&prior.rows.every((r:{kind:string;id:string})=>JSON.stringify(db.prepare('SELECT * FROM records WHERE kind=? AND id=?').get(r.kind,r.id))===JSON.stringify(r)));
