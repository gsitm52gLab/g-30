...
const manifest = JSON.parse(readFileSync(input, 'utf8')) as { original_database_opened: boolean; any_sqlite_open_before_copy: boolean; copies: { label: string; copied_database: string; files: { source: string; destination: string; sha256: string }[] }[] };
assert.equal(manifest.original_database_opened, false);
assert.equal(manifest.any_sqlite_open_before_copy, false);
const checks: { id: string; status: 'PASS' | 'FAIL'; unit: 'assertion'; requirements: string[] }[] = [];
...
try {
    for (const copy of manifest.copies) {
        ...
        try {
            ...
            const migrations = db.prepare('SELECT * FROM schema_migrations ORDER BY name').all() as { name: string; sha256: string }[];
            ...
            const afterMigrations = db.prepare('SELECT * FROM schema_migrations ORDER BY name').all() as { name: string; sha256: string }[];
            ...
            for (const kind of immutableKinds) {
                const row = db.prepare('SELECT id FROM records WHERE kind=? LIMIT 1').get(kind) as { id: string } | undefined;
                if (!row) continue;
