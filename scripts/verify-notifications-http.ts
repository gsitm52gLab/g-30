...
async function createTask(publish = true) { const taskId = (await ids(await admin.mutate('/api/tasks', { category: 'spot', content: publicContent(), targets: [{ contextId, ownerId: 'user-gsg', assigneeId: 'user-luna', coAssigneeIds: ['user-co'], productIds: ['product-serum'] }], idempotencyKey: randomUUID() }), 201))[0]; if (publish) await ids(await admin.mutate(`/api/tasks/${taskId}`, { command: 'publish', expectedRevision: 1, idempotencyKey: randomUUID() })); return taskId; }
try {
    ...
    const first = await sync(); check('H05', first.items.length === 2 && first.items.filter(n=>n.source.kind==='event').length === 1 && first.items.filter(n=>n.source.kind==='reminder').length === 1 && first.items.every(n=>n.email==='not_connected'), ['AC-13-03','AC-13-05']);
    await Promise.all(Array.from({length:6},()=>sync())); check('H06', (await brand.get<NotificationList>('/api/notifications'+q)).items.length === 2, ['AC-13-03']);
    const material = first.items[0], readIntent = {read:true,expectedRevision:material.revision,idempotencyKey:randomUUID()}; await ids(await brand.mutate(`/api/notifications/${material.id}/read`,readIntent)); await ids(await brand.mutate(`/api/notifications/${material.id}/read`,readIntent));
    ...
    await sync();check('H12',(await brand.get<NotificationList>('/api/notifications'+q)).items.filter(n=>n.source.kind==='reminder'&&n.source.logicalKey===`manual:${id}`).length===1,['SA-52']);
    let detail=await schedule(id);const firstVersion=detail.versions[0];
    ...
    await ids(await admin.mutate('/api/schedule',{...intent,scheduleId:id,expectedRevision:detail.revision,content:conflict,idempotencyKey:randomUUID()}));detail=await schedule(id);
    ...
    await ids(await admin.mutate('/api/schedule',{command:'cancel',contextId,scheduleId:id,expectedRevision:detail.revision,reason:'일정 취소',idempotencyKey:randomUUID()}));detail=await schedule(id);
    check('H15',detail.current.state==='cancelled'&&!detail.calendar.reminder.eligible,['AC-13-04']);
    await ids(await admin.mutate('/api/schedule',{command:'reopen',contextId,scheduleId:id,expectedRevision:detail.revision,reason:'다시 확인',idempotencyKey:randomUUID()}));detail=await schedule(id);
    ...
    if(mode==='sqlite'){
        const raceTask=await createTask();await start(auxPort);const other=new Client(auxPort);await other.login('luna@example.test');
        const replies=await Promise.all(Array.from({length:8},(_,i)=>sync(i%2?brand:other)));check('H19',replies.every(r=>r.failed===0),['AC-13-03'],'PROCESS');
        let db=openDatabase(database);const stored=db.prepare("SELECT kind,data FROM records WHERE kind IN ('notification','notificationReceipt')").all() as {kind:string;data:string}[];
        const events=db.prepare("SELECT id FROM records WHERE kind='domainEvent' AND json_extract(data,'$.targetId')=? AND json_extract(data,'$.eventType')='TASK_PUBLISHED'").all(raceTask) as {id:string}[];
        check('H20',events.length===1&&stored.filter(r=>r.kind==='notification'&&JSON.parse(r.data).source.eventId===events[0].id&&JSON.parse(r.data).recipientId==='user-luna').length===1&&stored.filter(r=>r.kind==='notificationReceipt'&&JSON.parse(r.data).source.eventId===events[0].id&&JSON.parse(r.data).recipientId==='user-luna').length===1,['AC-13-03'],'DB_FIXTURE');db.close();
        await stop(auxPort);const faultTask=await createTask();db=openDatabase(database);const before=db.prepare("SELECT count(*) n FROM records WHERE kind='notification'").get() as {n:number};db.exec("CREATE TRIGGER g13_test_notification_fault BEFORE INSERT ON records WHEN NEW.kind='notification' BEGIN SELECT RAISE(ABORT,'synthetic ownedDB delivery fault'); END;");db.close();
        const failed=await sync();check('H21',failed.failed===2&&failed.failures.length===2,['AC-13-03']);db=openDatabase(database);check('H22',(db.prepare("SELECT count(*) n FROM records WHERE kind='notification'").get() as {n:number}).n===before.n,['AC-13-03'],'DB_FIXTURE');db.exec('DROP TRIGGER g13_test_notification_fault');db.close();
        const retried=[];for(const f of failed.failures){const response=await brand.mutate(`/api/notifications/attempts/${f.id}/retry`,{});assert.equal(response.status,200);retried.push((await response.json()).state);const replay=await brand.mutate(`/api/notifications/attempts/${f.id}/retry`,{});assert.equal(replay.status,200);retried.push((await replay.json()).state);}check('H23',retried.join(',')==='delivered,reused,delivered,reused',['AC-13-03']);
        check('H24',(await brand.get<NotificationList>('/api/notifications'+q)).failures.length===0&&!!faultTask,['AC-13-03']);
        const prior={schedule:await schedule(id),notifications:await brand.get<NotificationList>('/api/notifications'+q)},beforePid=processes[0].pid;await stop(port);await start();await brand.login('luna@example.test');await admin.login('admin@example.test');
    ...
    }else skipped.push('H19','H20','H21','H22','H23','H24','H25');
    const members=await admin.get<{members:{id:string;revision:number;data:{userId:string}}[]}>(`/api/contexts/${contextId}/members`),member=members.members.find(m=>m.data.userId==='user-luna')!;
    ...
    check('H27',(await brand.mutate(`/api/notifications/${material.id}/read`,{read:true,expectedRevision:1,idempotencyKey:randomUUID()})).status===404,['A19']);reachedEnd=true;
    ...
    writeFileSync(reportFile,JSON.stringify(report,null,2),{mode:0o600});console.log(JSON.stringify({status:report.status,pass:report.pass,fail:report.fail,execution_failure:report.execution_failure,reportFile}));
}
