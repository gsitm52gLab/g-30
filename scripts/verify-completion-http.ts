...
try {
    ...
    check('G11-H08 two purposes exact file performer recorder separate and no task status effect', external.purpose === 'review_request' && other.purpose === 'final_use' && external.effect === 'record_only' && external.performer.kind === 'external' && external.performer.label === 'Actual outside performer' && external.recordedByLabel !== external.performer.label && hash(external.source) === hash(other.source) && (await workspace()).taskRevision === revision && (await workspace()).taskStatus === 'partial', ['AC-11-03']);
