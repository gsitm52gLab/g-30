...
export async function importFixture(repo: RecordRepository, input: ImportFixtureInput) {
    return repo.transaction(async (s) => {
        if (input.action === 'membership') {
            const member = (await s.list('membership', input.contextId)).find(m => m.data.userId === input.userId);
            if (!member)
                throw new Error('synthetic member missing');
            (await s.update('membership', member.id, member.revision, { ...member.data, status: input.active ? 'active' : 'suspended' }));
        }
        if (input.action === 'poison_batch') {
            const original = (await s.get('importBatch', input.batchId));
            if (!original)
            ...
            (await s.create('importBatch', { id: input.poisonedId, contextId: original.contextId, data }));
        }
        ...
        const hashes = Object.fromEntries((await asyncMap(kinds, async (k) => { const rows = (await s.list(k)).sort((a, b) => a.id.localeCompare(b.id)); return [k, { count: rows.length, sha256: createHash('sha256').update(JSON.stringify(rows)).digest('hex') }]; })));
        return hashes;
    });
}
