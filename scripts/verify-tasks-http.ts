...
if (mode === "sqlite") { const db = openDatabase(filename, true); migrate(db); const repo = createSqliteRepository(db); await seed(repo); repo.close(); }
...
async function start() {
    ...
    server = spawn(process.execPath, args, { stdio: "pipe", env: { ...process.env, DATA_SOURCE: mode, DATABASE_FILE: filename, FILE_STORAGE_DIR: files, APP_ORIGIN: origin, SESSION_COOKIE_NAME: `gs_hale_g04_http_${port}`, OPENAI_API_KEY: "", NEXT_TELEMETRY_DISABLED: "1" } });
