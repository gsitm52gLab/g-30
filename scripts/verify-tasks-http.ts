...
function poison<T>(value: T): T { if (Array.isArray(value))
    return value.map(poison) as T; if (value && typeof value === "object")
    return Object.assign(Object.fromEntries(Object.entries(value).map(([k, v]) => [k, poison(v)])), { privateExtension: { deep: projectionMarker } }) as T; return value; }
async function prepareProjectionFixture(repo: RecordRepository) {
    ...
    await repo.transaction(async (s) => {
        (await s.create("task", { id: projectionTask, contextId, data: { schemaVersion: 2, visibility: "public", title: c.title, description: c.description, category: "spot", status: "requested", ownerId: "user-gsg", assigneeId: "user-luna", coAssigneeIds: [], productIds: [], deadline: c.deadline.value, nextAction: c.nextAction, notes: [], authorId: "user-admin", draft: c, currentRequestId: null, projectId: null } }));
        (await s.create("requestVersion", { id: "v03-http-old", contextId, data: { taskId: projectionTask, sequence: 1, previousId: null, templateVersionId: null, content: c, publishedBy: "user-admin", publishedAt: at, changedKeys: ["v03-text", { private: projectionMarker }] as unknown as string[] } }));
        const row = (await s.get("task", projectionTask))!;
        (await s.update("task", row.id, row.revision, { ...row.data, currentRequestId: "v03-http-old" }));
        (await s.create("templateVersion", { id: "v03-http-template", contextId, data: Object.assign({ templateId: "v03-http-template", name: "V03 합성 템플릿", sequence: 1, previousId: null, content: c, createdBy: "user-admin", builtin: false }, { privateExtension: projectionMarker }) }));
        (await s.create("taskActivity", { id: "v03-http-activity", contextId, data: Object.assign({ taskId: projectionTask, requestId: "v03-http-old", userId: "user-luna", kind: "schedule" as const, at, sequence: 1, reason: "공개 조정 사유", proposedDeadline: poison(c.deadline), respondsTo: null, decision: null, resultingRequestId: null }, { privateExtension: projectionMarker }) }));
        (await s.create("project", { id: "v03-http-project", contextId, data: Object.assign({ title: "V03 공개 프로젝트", taskIds: [projectionTask, "task-onboarding"], dependencies: [Object.assign({ before: projectionTask, after: "task-onboarding" }, { privateExtension: projectionMarker })], status: "active" as const, createdBy: "user-admin" }, { privateExtension: projectionMarker }) }));
    });
}
async function projectionSnapshot(repo: RecordRepository) {
    ...
    return { sha256: createHash("sha256").update(serialized).digest("hex"), extensionStillStored: serialized.includes(projectionMarker) };
}
...
if (process.argv.includes("--projection-mock-server")) {
    ...
    process.on("message", async (message) => { const m = message as {
        kind?: string;
        id?: string;
    }; if (m.kind === "projection-snapshot")
        process.send?.({ id: m.id, ...await projectionSnapshot(repo) }); });
...
async function stop() { if (!server)
    return; const own = server, done = new Promise<void>(r => own.once("exit", () => r())); own.kill("SIGTERM"); if (own.exitCode === null)
    await done; processes.at(-1)!.exitCode = own.exitCode; server = undefined; }
async function storedProjectionSnapshot() {
    if (mode === "sqlite") {
        const repo = createSqliteRepository(openDatabase(filename));
        try {
    ...
    return new Promise<{
        sha256: string;
        extensionStillStored: boolean;
    }>((resolve, reject) => { const id = randomUUID(); const timer = setTimeout(() => reject(new Error("Mock snapshot IPC timeout")), 5000); server!.once("message", message => { clearTimeout(timer); const m = message as {
        id: string;
        sha256: string;
        extensionStillStored: boolean;
    }; if (m.id !== id)
        reject(new Error("Snapshot IPC identity mismatch"));
    else
        resolve(m); }); server!.send({ kind: "projection-snapshot", id }); });
}
class Client {
    cookie = "";
...
try {
    ...
    const created = await admin.mutate("/api/tasks", createBody);
    check("actual task created", created.status === 201, ["AC-04-01"]);
    const taskId = (await created.json()).ids[0];
    const retry = await admin.mutate("/api/tasks", createBody);
    check("same command replay returns same task", (await retry.json()).ids[0] === taskId, ["AC-04-01", "A20"]);
    ...
    const uploaded = await admin.upload(taskId, bytes);
    check("multipart reference uploaded", uploaded.status === 201, ["A04", "SA-04", "partial-SA-19"]);
    const file = (await uploaded.json()).files[0];
    const raw = await admin.send(`/api/files/${file.id}?taskId=${taskId}`);
    check("manager exact original hash", createHash("sha256").update(Buffer.from(await raw.arrayBuffer())).digest("hex") === file.sha256, ["A04", "SA-04"]);
    ...
    c.referenceFileIds = [file.id];
    await command(admin, taskId, "save", { content: c });
    const preview = await admin.json<{
        content: unknown;
    }>(`/api/tasks/${taskId}?preview=1`);
    await command(admin, taskId, "publish");
    const published = await detail(brand, taskId);
    check("preview equals actual public DTO", JSON.stringify(preview.content) === JSON.stringify(published.request), ["AC-04-02", "SA-12"]);
    ...
    const download = await brand.send(`/api/files/${file.id}?taskId=${taskId}&mode=preview`);
    check("authorized preview private no-store plus original bytes", download.status === 200 && download.headers.get("cache-control")!.includes("no-store") && createHash("sha256").update(Buffer.from(await download.arrayBuffer())).digest("hex") === file.sha256, ["A04", "SA-04"]);
    check("file forged reference rejected", (await brand.send(`/api/files/${file.id}?taskId=task-wave`)).status === 404, ["A19"]);
    await command(team, taskId, "read");
    const t = await detail(team, taskId);
    check("team read stays separate from acceptance", t.activities.some(a => a.data.kind === "read") && t.task.data.status === "requested", ["AC-04-05"]);
    ...
    await command(admin, taskId, "hold", { reason: "V02 requested hold" });
    await command(admin, taskId, "resume", { reason: "V02 requested restore" });
    check("V02 requested task resumes requested", (await detail(brand, taskId)).task.data.status === "requested", ["G04-V02", "AC-04-05", "SA-15"]);
    await command(brand, taskId, "accept");
    const acceptanceBeforePause = (await detail(brand, taskId)).activities;
    await command(admin, taskId, "hold", { reason: "V02 accepted hold" });
    await command(admin, taskId, "cancel", { reason: "V02 nested cancel" });
    await command(admin, taskId, "hold", { reason: "V02 nested hold" });
    await command(admin, taskId, "resume", { reason: "V02 restore accepted" });
    await command(brand, taskId, "accept");
    const resumed = await detail(brand, taskId);
    ...
    const requested = await detail(brand, taskId);
    check("schedule proposal does not alter deadline", requested.request!.deadline.value === c.deadline.value, ["AC-04-05"]);
    const v1 = published.versions[0].id, activityId = requested.activities.find(a => a.data.kind === "schedule")!.id;
    const draft = { ...c, description: "PRIVATE_DRAFT_NOT_PUBLISHED" };
    await command(admin, taskId, "save", { content: draft });
    await command(admin, taskId, "schedule_decide", { activityId, decision: "apply", reason: "기한만 반영" });
    ...
    const ordered = adjusted.activities.map(a => a.data.sequence);
    check("activities strictly ordered even when timestamps tie", ordered.every((n, i) => i === 0 || n > ordered[i - 1]), ["AC-04-05"]);
    if (mode === "sqlite") {
        ...
        await command(admin, taskId, "publish");
        const revised = await detail(brand, taskId);
        check("actual API prior-submission fixture retained and additions missing", revised.requirementStatus.some(q => q.requirementKey === "q-short_text" && q.status === "prior_received") && revised.requirementStatus.some(q => q.requirementKey === "added-file" && q.status === "missing"), ["AC-04-04", "D06"]);
        await command(admin, taskId, "hold", { reason: "V02 persist paused progress" });
        const paused = await detail(brand, taskId);
        const beforeHash = createHash("sha256").update(JSON.stringify(paused)).digest("hex");
        await stop();
        await start();
        check("actual different server process", processes[0].pid !== processes[1].pid, ["A20"]);
        const relogged = new Client();
        await relogged.login("luna@example.test");
        const after = await detail(relogged, taskId);
        check("all task versions activities and projection survive actual restart/relogin", createHash("sha256").update(JSON.stringify(after)).digest("hex") === beforeHash, ["A20", "AC-04-02", "AC-04-04", "AC-04-05"]);
        await command(admin, taskId, "resume", { reason: "V02 restart restore" });
        const afterResume = await detail(relogged, taskId);
        ...
        const restored = await relogged.send(`/api/files/${file.id}?taskId=${taskId}&mode=original`);
        check("immutable bytes survive restart", createHash("sha256").update(Buffer.from(await restored.arrayBuffer())).digest("hex") === file.sha256, ["A04", "SA-04"]);
    ...
    if (mode === "sqlite") {
        ...
        await command(brand, taskId, "accept");
        const accepted = (await detail(brand, taskId)).activities;
        await legacy("on_hold");
        await command(admin, taskId, "resume", { reason: "V02 legacy latest accepted" });
        ...
        await legacy("requested");
        await command(brand, taskId, "accept");
        const repaired = await detail(brand, taskId);
    ...
    const builtInApply = await admin.mutate("/api/templates", { command: "apply", contextId: ctx, versionId: "builtin-pop-v1", targets: [{ id: taskId, expectedRevision: (await detail(admin, taskId)).task.revision }], idempotencyKey: randomUUID() });
    check("builtin template applies with explicit owner fallback", builtInApply.status === 200, ["SA-14"]);
    const oldFile = await brand.send(`/api/files/${file.id}?taskId=${taskId}`);
    check("old-version reference remains available and metadata projected", oldFile.status === 200 && (await detail(brand, taskId)).files.some(f => f.id === file.id), ["A04", "SA-04", "AC-04-04"]);
    if (projectionEnabled) {
        await command(admin, projectionTask, "publish");
        const before = await storedProjectionSnapshot();
        const b = await detail(brand, projectionTask), g = await detail(admin, projectionTask), preview = await admin.json<{
            content: unknown;
        }>(`/api/tasks/${projectionTask}?preview=1`), cat = await admin.json<TaskCatalog>(`/api/tasks?context=${ctx}`), project = await brand.json<unknown>("/api/projects/v03-http-project");
        const gsg = new Client();
        await gsg.login("operator@example.test");
        const nonprice = await detail(gsg, projectionTask), nonpriceCatalog = await gsg.json<TaskCatalog>(`/api/tasks?context=${ctx}`);
        const objects = { b, g, preview, cat, project, nonprice, nonpriceCatalog };
        writeFileSync(`${reportFile}.projection-api-private.json`, JSON.stringify(objects, null, 2), { mode: 0o600 });
        ...
        for (const [label, client, url] of [["brand", brand, `/tasks/${projectionTask}?context=${ctx}`], ["gsg", admin, `/tasks/${projectionTask}?context=${ctx}`], ["templates", admin, `/tasks/templates?context=${ctx}`], ["project", brand, "/projects/v03-http-project?context=" + ctx]] as const) {
            const response = await client.send(url);
            const html = await response.text();
            writeFileSync(`${reportFile}.projection-${label}-private.html`, html, { mode: 0o600 });
            check(`V03 actual SSR ${label} excludes extension markers`, response.status === 200 && !html.includes(projectionMarker), ["G04-V03", "AC-02-02", "A19"]);
        }
        const after = await storedProjectionSnapshot();
        writeFileSync(`${reportFile}.projection-stored-hashes.json`, JSON.stringify({ before, after }, null, 2));
        check("V03 projection does not mutate stored history or extension originals", before.extensionStillStored && after.extensionStillStored && before.sha256 === after.sha256, ["G04-V03", "A20"]);
    ...
    await stop();
    const report = { status: failure ? "FAIL" : "PASS", mode, level: "implementer-self-check", unit: "assertion", passed: checks.filter(c => c.status === "PASS").length, failed: checks.filter(c => c.status === "FAIL").length, skipped: 0, failure, checks, processes, actualHttp: true, projectionFixture: projectionEnabled ? (mode === "sqlite" ? "normal Next start with private persisted extension fixture" : "test-only Next startServer bootstrap with preloaded existing global mock repository cache; no product endpoint") : "disabled; regular boot", processRestart: mode === "sqlite", sqliteFixture: mode === "sqlite" ? "synthetic prior submission, actual G05 producer remains NOT_RUN" : null, credentialsAndCookies: "memory only; not recorded" };
    writeFileSync(reportFile, JSON.stringify(report, null, 2), { mode: 0o600 });
    console.log(JSON.stringify({ status: report.status, passed: report.passed, failed: report.failed, reportFile }));
}
