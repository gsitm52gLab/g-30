...
async function stop() {
    if (!child)
        return;
    const own = child, done = new Promise<void>(r => own.once('exit', () => r()));
    if (own.exitCode === null && own.signalCode === null) {
        own.kill('SIGTERM');
        await done;
    }
    const record = processes.at(-1)!;
    record.stopped = true;
    record.exitCode = own.exitCode;
    record.signal = own.signalCode;
    child = undefined;
}
async function fixture(input: ImportFixtureInput): Promise<ImportFixtureSnapshot> {
    if (mode === 'sqlite') {
    ...
    return new Promise((resolve, reject) => {
        const id = randomUUID(), timer = setTimeout(() => reject(new Error('fixture timeout')), 5000);
...
class Client {
    ...
    async send(url: string, method = 'GET', body?: unknown, csrf?: string) {
        const response = await fetch(origin + url, { method, redirect: 'manual', headers: { Cookie: this.cookie, ...method !== 'GET' ? { Origin: origin, 'X-CSRF-Token': csrf ?? '', ...body instanceof FormData ? {} : { 'Content-Type': 'application/json' } } : {} }, body: body === undefined ? undefined : body instanceof FormData ? body : JSON.stringify(body) });
        const cookie = response.headers.get('set-cookie');
        if (cookie)
            this.cookie = cookie.split(';')[0];
        const bytes = Buffer.from(await response.clone().arrayBuffer()), artifact = `${report}.response-${transcript.length + 1}${response.headers.get('content-type')?.includes('json') ? '.json' : '.bin'}`;
        writeFileSync(artifact, bytes, { mode: 0o600 });
        artifacts.push(artifact);
        transcript.push({ method, url, status: response.status, artifact, sha256: hash(bytes) });
        return response;
    }
    ...
    async mutate(url: string, body: unknown) {
        const c = await this.get<{
            csrfToken: string;
        }>('/api/auth/csrf');
        return this.send(url, 'POST', body, c.csrfToken);
    }
...
async function workbook(rows: unknown[][], headers = ['contextKey', 'common.code', 'common.name', 'local.jan', 'retail.amount', 'retail.currency'], headerRow = 1) {
...
try {
    ...
    if (process.env.IMPORTS_NAMESPACE_ONLY === '1') {
        ...
        for (const name of ['standard', 'formula-cached', 'numeric-identifier']) {
            ...
            if (name === 'standard') {
                ...
                const result = await applied.json() as {
                    ids: string[];
                }, batch = await brand.get<ImportBatch>(`/api/imports/batches/${result.ids[0]}`);
        ...
        const [applied, concurrent] = await Promise.all([posted<{
                ids: string[];
            }>(brand, '/api/imports/apply', apply, 201), posted<{
                ids: string[];
