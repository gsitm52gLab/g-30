...
export async function submissionFixture(repo: RecordRepository, input: SubmissionFixtureInput) {
    return repo.transaction(s => {
        let task = s.get('task', input.taskId);
        if (!task)
            throw new Error('fixture task missing');
        if (input.action === 'poison_request') {
            const old = s.get('requestVersion', task.data.currentRequestId!)!;
            ...
            const next = s.create('requestVersion', { id: randomUUID(), contextId: task.contextId, data });
            task = s.update('task', task.id, task.revision, { ...task.data, currentRequestId: next.id });
        }
