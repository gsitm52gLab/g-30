...
async function actualSubmission(label: string): Promise<ReviewTarget> {
    ...
    const saved = await brand.mutate(`/api/tasks/${taskId}/submission-draft`, { command: 'save', baseRequestId: w.request.id, expectedDraftRevision: w.draft?.revision ?? 0, content, idempotencyKey: randomUUID() });
    assert.equal(saved.status, 200, await saved.clone().text());
    w = await brand.get<SubmissionWorkspace>(`/api/tasks/${taskId}/submissions`);
    const submitted = await brand.mutate(`/api/tasks/${taskId}/submissions`, { baseRequestId: w.request.id, expectedDraftRevision: w.draft!.revision, expectedTaskRevision: w.taskRevision, mode: 'full', idempotencyKey: randomUUID() });
    assert.equal(submitted.status, 201, await submitted.clone().text());
    const id = (await submitted.json()).ids[0] as string, snapshot = await brand.get<SubmissionSnapshot>(`/api/submissions/${id}`);
    return { taskId, submissionId: id, requestId: snapshot.requestId, submissionContentHash: snapshot.contentHash, answer: null, fileVersionIds: [item.file.id], productUseIds: snapshot.products.map(p => p.id), location: { page: '2', locator: 'lower emblem' } };
}
...
async function draft(target: ReviewTarget, previousBatchVersionId: string | null = null): Promise<BatchDraftInput> { const items = []; for (const key of ['one', 'two', 'three']) {
    const ids = await cmd(admin, { command: 'save_opinion', opinionId: null, expectedRevision: 0, opinion: opinion(target) });
    items.push({ key, target, internalOpinionVersionIds: [ids[1]], publicSource: 'Public source', change: 'Change ' + key, reason: 'Reason', publicDescription: 'Selected public explanation', priority: 'normal' as const, issue: 'correction' as const });
} return { title: 'Public change batch', summary: 'Three selected items', items, mode: 'normal', pendingScopes: [], previousBatchVersionId }; }
...
try {
    ...
    await start();
    check('G10-H01 anonymous request denied', (await new Client().send('/api/corrections?taskId=task-onboarding')).status === 401, ['A19']);
    ...
    const content = { ...blankContent(), title: 'G10 actual correction source', description: 'Actual public submission', deadline: { ...blankContent().deadline, responsibleUserId: 'user-gsg' }, requirements: [{ ...blankRequirement('answer'), label: 'Answer' }] }, created = await admin.mutate('/api/tasks', { targets: [{ contextId: A, ownerId: 'user-gsg', assigneeId: 'user-luna', coAssigneeIds: [], productIds: ['product-serum'] }], content, category: 'spot', idempotencyKey: randomUUID() });
    assert.equal(created.status, 201, await created.clone().text());
    taskId = (await created.json()).ids[0];
    const t = await admin.get<TaskDetail>(`/api/tasks/${taskId}`), published = await admin.mutate(`/api/tasks/${taskId}`, { command: 'publish', expectedRevision: t.task.revision, idempotencyKey: randomUUID() });
    assert.equal(published.status, 200, await published.clone().text());
    const v1 = await actualSubmission('v1'), internalResponse = await admin.upload(`taskId=${taskId}`, png, 'PRIVATE_G10_FILE.png', 'image/png', 'internal');
    assert.equal(internalResponse.status, 201);
    const internal = (await internalResponse.json()).files[0] as {
        id: string;
    };
    const opinions = await cmd(admin, { command: 'save_opinion', opinionId: null, expectedRevision: 0, opinion: opinion(v1, [internal.id]) }), d = await draft(v1);
    d.items[0].internalOpinionVersionIds = [opinions[1]];
    const draftId = (await save(d))[0], pre = await workspace(brand), facts = await fixture<CorrectionFixtureSnapshot>({ taskId });
    persist('private-before-publication', facts);
    ...
    let current = await batch(first);
    check('G10-H06 selected public fields retained internal source IDs names absent', current.items.length === 3 && current.items[0].publicSource === 'Public source' && !JSON.stringify(current).includes('PRIVATE_G10') && !JSON.stringify(current).includes(opinions[1]), ['AC-10-01', 'A19']);
    ...
    const stable = hash(current), root = (await workspace()).staff!.opinions.find(o => o.id === opinions[0])!;
    await cmd(admin, { command: 'save_opinion', opinionId: root.id, expectedRevision: root.revision, opinion: { ...opinion(v1, [internal.id]), originalText: 'PRIVATE_G10_LATE' } });
    ...
    const v2 = await actualSubmission('v2');
    check('G10-H10 upload and full new submission leave all correction items pending', (await batch(first)).items.every(i => i.state.status === 'pending'), ['AC-10-02']);
    ...
    const resolution = { command: 'resolve', batchVersionId: first, items: [{ itemKey: 'one', expectedItemRevision: 1, reflectionId: reflections[0], decision: 'resolved', reason: '' }, { itemKey: 'two', expectedItemRevision: 1, reflectionId: reflections[1], decision: 'not_reflected', reason: 'Wrong emblem file' }] };
    await cmd(admin, resolution);
    current = await batch(first);
    ...
    const nextSnapshot = await admin.get<SubmissionSnapshot>(`/api/submissions/${v2.submissionId}`);
    check('G10-H15 new submitted version has actual connected zero reviews no inherited approval', nextSnapshot.review.connected && nextSnapshot.review.status === 'pending' && nextSnapshot.review.reviews.length === 0, ['AC-10-04']);
    ...
    const follow = await draft(v1, first);
    follow.mode = 'urgent_partial';
    follow.pendingScopes = [{ agency: 'Pending agency', scope: 'Emblem', expectedOn: null }];
    follow.items[0].issue = 'conflicting_opinions';
    follow.items[0].internalOpinionVersionIds.push(follow.items[1].internalOpinionVersionIds[0]);
    follow.items[0].publicDescription = 'Conflicting opinions need confirmation';
    const followDraft = (await save(follow))[0], followId = (await cmd(admin, { command: 'publish', draftId: followDraft, expectedRevision: 1 }))[0];
    ...
    const badTargets = [{ ...v1, submissionContentHash: 'f'.repeat(64) }, { ...v1, requestId: 'missing-request' }, { ...v1, productUseIds: v2.productUseIds }, { ...v1, answer: { requirementKey: 'missing', productId: null } }];
    const beforeInvalid = await fixture<CorrectionFixtureSnapshot>({ taskId });
    ...
    check('G10-H20 existing G05 task HTML carries no internal source even with review summary connected', html.includes('G10 actual correction source') && !html.includes('PRIVATE_G10'), ['A19']);
    const raceDraft = (await save(await draft(v1, followId)))[0], racer = new Client(mode === 'sqlite' ? auxPort : port);
    if (mode === 'sqlite')
        await start(auxPort);
    await racer.login('admin@example.test');
    const raceBody = (await workspace()).staff!.drafts.find(d => d.id === raceDraft)!.draft;
    ...
    const raceFacts = await fixture<CorrectionFixtureSnapshot>({ taskId }), raceId = values[0].ids[0];
    persist('race-facts', raceFacts);
    ...
    const extension = await fixture<CorrectionFixtureSnapshot>({ taskId, action: 'extension', batchId: first });
    persist('stored-extension', extension);
    check('G10-H23 stored unknown nested extension remains original but absent from legitimate public DTO', JSON.stringify(extension).includes('G10_STORED_PRIVATE') && !JSON.stringify(await batch(extension.fixtureBatchId!)).includes('G10_STORED_PRIVATE') && (await batch(extension.fixtureBatchId!)).items[0].publicSource === 'Public source', ['A19']);
    const restart = await fixture<CorrectionFixtureSnapshot>({ taskId }), beforeRestart = await workspace(brand), beforeStaff = await workspace(admin);
    persist('before-restart', restart);
    if (mode === 'sqlite') {
        await stop(auxPort);
        await stop(port);
        await start();
        const again = new Client(), gsgAgain = new Client();
        await again.login('luna@example.test');
        await gsgAgain.login('admin@example.test');
        check('G10-H24 new PID and actual relogin preserve public and internal records', processes[0].pid !== processes.at(-1)!.pid && hash(await workspace(again)) === hash(beforeRestart) && hash(await workspace(gsgAgain)) === hash(beforeStaff), ['D10', 'A20'], 'PROCESS');
        check('G10-H25 restart preserves all original facts product uses and receipt bytes', (await fixture<CorrectionFixtureSnapshot>({ taskId })).rowsSha256 === restart.rowsSha256 && (await fixture<CorrectionFixtureSnapshot>({ taskId })).businessSha256 === restart.businessSha256, ['D10'], 'DB_FIXTURE');
        check('G10-H26 historical file hash persists after restart', byteHash(Buffer.from(await (await again.send(publicFile.originalUrl)).arrayBuffer())) === byteHash(png), ['D10', 'D02']);
    }
    ...
    const corrupt = await fixture<CorrectionFixtureSnapshot>({ taskId, action: 'corruption', batchId: first });
    persist('malformed-stored', corrupt);
    const corruptRead = await admin.send(`/api/corrections/${corrupt.fixtureBatchId}`);
    check('G10-H29 known malformed stored public value is safe503 with no canary', corruptRead.status === 503 && !(await corruptRead.text()).includes('G10_KNOWN_CORRUPT'), ['A19']);
    reachedEnd = true;
}
catch (error) {
