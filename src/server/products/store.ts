...
export function uniqueCode(s: UnitOfWork, contextId: string, code: string, productId?: string) {
    if (s.list("contextProduct", contextId).some(r => r.data.productId !== productId && r.data.normalizedCode === normalizeProductCode(code)))
        fail("CONFLICT", 409, "자료가 변경되었거나 저장 조건이 맞지 않습니다. 입력을 유지한 채 최신 내용을 확인해 주세요.");
}
...
export function receipt(s: UnitOfWork, p: Principal, contextId: string, command: string, input: Record<string, unknown>, action: () => {
    ids: string[];
}, fault?: () => void) {
    ...
    const old = s.list("commandReceipt").find(r => r.data.key === key);
    if (old) {
    ...
    const result = action();
    fault?.();
    s.create("commandReceipt", { id: newId(), contextId, data: { key, bodyHash, command, actorId: p.user.id, result } });
    return result;
}
