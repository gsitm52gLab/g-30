...
export async function captureProductUse(s: UnitOfWork, p: Principal, input: CaptureProductUseInput, clock: Clock) {
    const r = (await resolveProduct(s, p, input.contextId, input.productId, clock));
    ...
    if (input.requestId && (!input.taskId || (await s.get("requestVersion", input.requestId))?.data.taskId !== input.taskId))
        unavailable();
    const bindingIds = ids(input.bindingIds), asOfDate = dateValue(input.asOfDate);
    const files = (await asyncMap(bindingIds, async (bindingId) => {
        ...
        if (!file)
            unavailable();
        (await canReferenceFile(s, p, file, productContextScope(input.contextId, input.productId), clock));
    ...
    if (input.retailPriceVersionId) {
        const version = (await s.get("retailPriceVersion", input.retailPriceVersionId)), root = version ? (await s.get("retailPrice", version.data.priceId)) : null;
...
export async function readProductUse(s: UnitOfWork, p: Principal, snapshotId: string, clock: Clock) {
    ...
    const files = (await asyncMap(d.files, async (entry) => {
        const f = (await s.get("fileVersion", entry.fileVersionId));
        if (!f)
            unavailable();
        (await canReferenceFile(s, p, f, productContextScope(row.contextId!, d.productId), clock));
        return { fileVersionId: f.id, sha256: text(f.data.sha256), binding: bindingDTO(entry.binding) };
    }));
