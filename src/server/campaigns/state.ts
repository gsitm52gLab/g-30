...
export async function storedMenuState(s: UnitOfWork, v: StoredRecord<'campaignVersion'>, menu: PublicMenu, through = Number.MAX_SAFE_INTEGER) {
    const selections = (await s.list('campaignSelection', v.contextId!)).filter(r => r.data.campaignVersionId === v.id && r.data.sequence <= through).sort((a, b) => a.data.sequence - b.data.sequence);
    for (const r of selections) {
        if (!['participate', 'decline', 'discuss'].includes(r.data.response) || !Array.isArray(r.data.selectedMenus))
            safe.corrupt();
        r.data.selectedMenus.forEach(safe.identity);
    }
    const facts = (await s.list('campaignExternalFact', v.contextId!)).filter(r => r.data.campaignVersionId === v.id && r.data.sequence <= through && menuIdentityKey(safe.identity(r.data.menu)) === menuIdentityKey(menu.identity));
    ...
    for (const e of events) {
        if (!e.selection) {
            state = withExternalFact(state, e.fact!);
            continue;
        }
        const includes = e.selection.data.selectedMenus.some(m => menuIdentityKey(m) === menuIdentityKey(menu.identity));
        state = withParticipation(state, e.selection.data.response);
        if (e.selection.data.response === 'participate' && !includes)
            state = { ...state, response: 'pending', cancellation: ['applied', 'withdrawal_requested'].includes(state.application) && state.cancellation !== 'cancelled' ? 'discussion' : state.cancellation };
    }
