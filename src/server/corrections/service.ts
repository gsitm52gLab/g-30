...
export type CorrectionCommand =
 | ({command:'save_opinion'}&ReturnType<typeof parse.parseSaveOpinion>)
 | ({command:'save_draft'}&ReturnType<typeof parse.parseSaveBatchDraft>)
 | ({command:'publish'}&ReturnType<typeof parse.parsePublishBatch>)
 | ({command:'reflect'}&ReturnType<typeof parse.parseReflectItems>)
 | ({command:'resolve'}&ReturnType<typeof parse.parseResolveItems>)
 | ({command:'record_review'}&ReturnType<typeof parse.parseRecordReview>);
export class CorrectionService {
    ...
    private checkDraft(s:UnitOfWork,p:Principal,taskId:string,d:BatchDraftInput){
        if(d.previousBatchVersionId)this.batch(s,p,d.previousBatchVersionId,taskId);
        for(const item of d.items){exactTarget(s,p,item.target,this.clock);const opinions=this.opinions(s,p,taskId,item.internalOpinionVersionIds);if(opinions.some(o=>o.target.submissionId!==item.target.submissionId))fail('TARGET_MISMATCH',422,'의견과 수정항목의 정확한 제출 버전이 달라졌습니다.');}
    }
    private event(s:UnitOfWork,p:Principal,task:StoredRecord<'task'>,kind:string,id:string){s.create('domainEvent',{id:newId(),contextId:task.contextId,data:{eventType:kind,targetId:task.id,sourceVersionId:id,actorId:p.user.id,at:this.clock()}});}
    async workspace(token:string|undefined,taskId:string){return this.identity.repo.transaction(s=>correctionWorkspace(s,this.identity.principal(s,token),taskId,this.clock));}
    async detail(token:string|undefined,batchId:string){return this.identity.repo.transaction(s=>{const p=this.identity.principal(s,token),b=s.get('correctionBatch',batchId);if(!b)unavailable();correctionTask(s,p,b.data.taskId,this.clock);return batchDTO(s,p,b,this.clock);});}
    async preview(token:string|undefined,draftId:string){return this.identity.repo.transaction(s=>{const p=this.identity.principal(s,token),d=s.get('correctionDraft',draftId);if(!d)unavailable();correctionTask(s,p,d.data.taskId,this.clock,'manage');const content=safe.draft(d.data.draft);this.checkDraft(s,p,d.data.taskId,content);return publicContent(content);});}
    async command(token:string|undefined,input:unknown){
        ...
        const command=str(v.command,30,true),{command:_command,...body}=v;void _command;
        if(!['save_opinion','save_draft','publish','reflect','resolve','record_review'].includes(command))fail('VALIDATION',422,'검토 동작을 확인해 주세요.');
        ...
        return this.identity.repo.transaction(s=>{
            ...
            if(command==='save_draft'){
                const x=parse.parseSaveBatchDraft(body);this.checkDraft(s,p,task.id,x.draft);const old=x.draftId?s.get('correctionDraft',x.draftId):null;if(x.draftId&&(!old||old.data.taskId!==task.id))unavailable();
                return receipt(s,p,task.contextId!,`correction.draft:${task.id}`,x as unknown as Record<string,unknown>,()=>{fresh(old,x.expectedRevision);if(old?.data.publishedVersionId)fail('PUBLISHED_IMMUTABLE',409,'이미 공개된 묶음입니다. 후속 묶음으로 추가해 주세요.');const d=old?s.update('correctionDraft',old.id,old.revision,{...old.data,draft:x.draft}):s.create('correctionDraft',{id:newId(),contextId:task.contextId,data:{taskId:task.id,draft:x.draft,publishedVersionId:null,createdBy:p.user.id}});audit(s,p,this.clock,task.contextId!,'correction.draft_saved',d.id,{revision:old?.revision??0},{revision:d.revision});return {ids:[d.id]};},()=>this.fault?.('draft'));
            }
            if(command==='publish'){
                const x=parse.parsePublishBatch(body),d=s.get('correctionDraft',x.draftId);if(!d||d.data.taskId!==task.id)unavailable();const content=safe.draft(d.data.draft);this.checkDraft(s,p,task.id,content);
                return receipt(s,p,task.contextId!,`correction.publish:${task.id}`,x as unknown as Record<string,unknown>,()=>{fresh(d,x.expectedRevision);if(d.data.publishedVersionId)fail('PUBLISHED_IMMUTABLE',409,'이미 공개된 묶음입니다. 후속 묶음으로 추가해 주세요.');parse.assertPublishableDraft(content);const previous=s.list('correctionBatch',task.contextId!).filter(b=>b.data.taskId===task.id);if(previous.length&&!content.previousBatchVersionId)fail('FOLLOWUP_REQUIRED',422,'이전 공개 묶음을 선택해 후속 의견으로 연결해 주세요.');const pub=publicContent(content),row=s.create('correctionBatch',{id:newId(),contextId:task.contextId,data:{taskId:task.id,draftId:d.id,draftRevision:d.revision,sequence:Math.max(0,...previous.map(b=>safe.integer(b.data.sequence,1)))+1,...pub,publishedBy:p.user.id,publishedAt:this.clock(),contentHash:createHash('sha256').update(JSON.stringify(pub)).digest('hex')}});s.update('correctionDraft',d.id,d.revision,{...d.data,publishedVersionId:row.id});audit(s,p,this.clock,task.contextId!,'correction.published',row.id,{},{});this.event(s,p,task,'CORRECTION_BATCH_PUBLISHED',row.id);return {ids:[row.id]};},()=>this.fault?.('publish'));
            }
            if(command==='reflect'||command==='resolve'){
                const x=command==='reflect'?parse.parseReflectItems(body):parse.parseResolveItems(body),b=this.batch(s,p,x.batchVersionId,task.id);batchDTO(s,p,b,this.clock);
                return receipt(s,p,task.contextId!,`correction.${command}:${task.id}`,x as unknown as Record<string,unknown>,()=>{const ids:string[]=[];
                    for(const value of x.items){const item=publicContent(b.data).items.find(i=>i.key===value.itemKey);if(!item)unavailable();const previous=itemState(s,p,b,item.key,this.clock),state=s.list('correctionItemState',task.contextId!).find(r=>r.data.batchVersionId===b.id&&r.data.itemKey===item.key);fresh(state??null,value.expectedItemRevision);
                        if(command==='reflect'&&'target' in value){laterTarget(s,p,item.target,value.target,this.clock);const r=s.create('correctionReflection',{id:newId(),contextId:task.contextId,data:{taskId:task.id,batchVersionId:b.id,itemKey:item.key,target:value.target,note:value.note,recordedBy:p.user.id,recordedAt:this.clock(),itemRevision:previous.revision+1}});const data={taskId:task.id,batchVersionId:b.id,itemKey:item.key,reflectionId:r.id,resolutionId:null};if(state)s.update('correctionItemState',state.id,state.revision,data);else s.create('correctionItemState',{id:newId(),contextId:task.contextId,data});ids.push(r.id);this.event(s,p,task,'CORRECTION_ITEM_REFLECTED',r.id);}
