...
export class InquiryService {
    ...
    async command(token: string | undefined, id: string, input: unknown): Promise<InquiryCommandResult> {
        ...
        return this.identity.repo.transaction(async (s) => {
            ...
            return (await auditOperation(s, receiptId, async () => {
                const exactIds: {
                    transitionId?: string;
                    linkId?: string;
                    readId?: string;
                ...
                const transition = async (q: StoredRecord<'inquiryQuestion'>, from: QuestionState | null, to: QuestionState, sourceMessageId: string | null, reason: string, externalWait: ExternalWait | null) => {
                    const transition = (await s.create('inquiryTransition', { id: randomUUID(), contextId: row.contextId, data: { conversationId: id, questionId: q.id, from, to, sourceMessageId, reason, externalWait, actorId: p.user.id, at } }));
                    exactIds.transitionId = transition.id;
                ...
                (await receipt(s, row.contextId!, key, h, p.user.id, 'inquiry.' + body.command, ids, receiptId));
                if (messageKey)
                    (await receipt(s, row.contextId!, messageKey, hash(semantic), p.user.id, 'inquiry.message', ids));
                (await appendAudit(s, p, this.identity.clock, row.contextId, 'inquiry.' + body.command, id, {}, { messageId: out.messageId, questionId: out.questionId, taskId: out.taskId, ...exactIds }));
