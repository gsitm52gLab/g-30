...
const names: Record<string, string> = { 'task.created': '업무 생성', 'task.draft': '업무 초안 저장', 'task.published': '요청 공개', 'task.reassigned': '담당자 변경', 'task.state': '업무 상태 변경', 'task.read': '요청 읽음', 'task.accept': '요청 수락', 'task.schedule': '기한 조정 요청', 'task.schedule_decide': '기한 조정 결정', 'task.manually_completed': 'GSG 수동 완료', 'task.reopened': '업무 재개', 'task.campaign_request': '행사 요청 범위 변경', 'external.action_recorded': '외부 진행 기록', 'completion.followup_linked': '후속 업무 연결', 'product.created': '상품 등록', 'product.save_common': '상품 공통 정보 수정', 'product.save_context': '컨텍스트 상품 정보 수정', 'product.save_files': '상품 자료 연결 변경', 'product.save_retail': '소비자가 기록', 'product.save_internal': '내부 공급가 기록', 'product.archive': '상품 보관', 'product.restore': '상품 복원', 'product.link_context': '상품 컨텍스트 연결', 'product.link_task': '상품 업무 연결', 'product.import': 'Excel 상품 반영', 'import.applied': 'Excel 일괄 반영', 'import.export': 'Excel 내보내기', 'submission.created': '답변 제출', 'submission.draft_saved': '답변 초안 저장', 'notice.created': '공지 생성', 'notice.draft_saved': '공지 초안 저장', 'notice.published': '공지 공개', 'context.created': '컨텍스트 생성', 'membership.changed': '멤버 권한 변경', 'user.status': '계정 상태 변경', 'invitation.created': '초대 생성', 'invitation.reissued': '초대 재발급', 'invitation.accepted': '초대 수락' };
...
export async function auditItem(a: SearchAccess, row: StoredRecord<'audit'>, docs: SearchDocument[]): Promise<AuditItem | null> {
    ...
    if (refs.some(r => ['requestVersion', 'submission', 'productUseSnapshot', 'productVersion', 'contextProductVersion', 'retailPriceVersion', 'internalPriceVersion', 'noticeVersion', 'correctionBatch', 'correctionReview', 'correctionOpinionVersion', 'completionSnapshot', 'completionExternalAction', 'scheduleVersion', 'aiAnalysisRun', 'importExport'].includes(r.kind) && !docs.some(x => x.sourceKind === r.kind && x.sourceId === r.id)))
        return null;
