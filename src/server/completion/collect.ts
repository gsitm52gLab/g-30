...
import { collectCampaignResidual } from './campaign';
import { collectAiResidual } from '@/server/ai-review/completion';
...
export const latestSubmission = async (s: UnitOfWork, t: StoredRecord<'task'>) => (await s.list('submission', t.contextId!)).filter(x => x.data.taskId === t.id).sort((a, b) => safe.count(b.data.sequence, 1) - safe.count(a.data.sequence, 1))[0] ?? null;
export async function available<T>(read: () => T | Promise<T>): Promise<Available<T>> {
    try {
        return { connected: true, state: 'available', value: (await read()) };
    }
    catch (e) {
...
export async function collectBasis(s: UnitOfWork, p: Principal, task: StoredRecord<'task'>, clock: Clock) {
    const current = task.data.currentRequestId ? (await s.get('requestVersion', task.data.currentRequestId)) : null, latest = (await latestSubmission(s, task)), original = latest ? (await s.get('requestVersion', latest.data.requestId)) : null;
    ...
    for (const kind of kinds)
        sourceRows.push(...(await s.list(kind, context)).filter(r => {
    ...
    const conversations = (await s.list('conversation', context)).filter(c => c.data.taskId === task.id && c.data.phase === 'active');
    sourceRows.push(...conversations);
    for (const kind of ['inquiryQuestion', 'inquiryTransition', 'inquiryMessage', 'inquiryTaskLink'] as const)
        sourceRows.push(...(await s.list(kind, context)).filter(r => conversations.some(c => c.id === r.data.conversationId)));
    const currentProducts = (await available(async () => (await asyncMap(task.data.productIds, async (productId) => { const r = (await resolveProduct(s, p, context, productId, clock)); sourceRows.push(r.product, r.relation, r.common, r.local); return { productId: safe.id(productId), productVersionId: r.common.id, contextProductVersionId: r.local.id, commonRevision: r.product.revision, contextRevision: r.relation.revision }; }))));
    const currentEvaluation = (await available(async () => {
        if (!current || current.data.taskId !== task.id)
            unavailable();
        return safe.evaluation(safeEvaluation(current.data.content, latest?.data.answers ?? [], original?.data.content ?? null, true, (await campaignRequestSource(s, current))?.noMaterials === true));
    }));
    const originalSubmittedEvaluation = (await available(async () => {
        if (!original || !latest)
            unavailable();
        return safe.evaluation(safeEvaluation(original.data.content, latest.data.answers, original.data.content, false, (await campaignRequestSource(s, original))?.noMaterials === true));
    }));
    const inquiries = (await available(async () => { const visible = (await readInquirySummary(s, p, context, clock, task.id)), items = (await asyncFlatMap(visible.items, async (c) => { const row = (await s.get('conversation', c.id))!; return (await asyncMap((await questions(s, row)), async (q) => { const dto = (await questionDTO(s, p, q, row)), message = (await s.get('inquiryMessage', dto.openingMessageId)); return safe.inquiry({ conversationId: row.id, questionId: q.id, revision: q.revision, text: message ? safe.text(message.data.body, 20000) : '질문 원문 확인 필요', state: dto.state, externalWait: dto.externalWait }); })); })); return { items, unresolved: items.filter(q => q.state !== 'resolved').length, externalWaiting: items.filter(q => q.state === 'external_waiting').length }; }));
    const corrections = (await available(async () => { const value = (await readCorrectionRemainder(s, p, task.id, clock)); const pendingScopes = (await s.list('correctionBatch', context)).filter(b => b.data.taskId === task.id).flatMap(b => batchContent(b).pendingScopes.map(x => ({ batchVersionId: b.id, agency: x.agency, scope: x.scope, expectedOn: x.expectedOn }))); return { items: value.items.map(safe.correction), unresolved: safe.count(value.unresolved), pendingScopes }; }));
    const externalActions = (await s.list('completionExternalAction', context)).filter(r => r.data.taskId === task.id).map(r => { const a = safe.external(r.data); return { id: r.id, visibility: a.visibility, purpose: a.purpose, latestProgress: a.latestProgress, waitingExternal: a.waitingExternal, observedAt: a.observedAt, recordedAt: safe.time(r.data.recordedAt) }; });
    const products = (await available(async () => latest ? (await asyncMap(latest.data.productUseIds, async (id) => safe.productUse((await readProductUse(s, p, id, clock))))) : []));
    const value: CompletionBasis = { currentRequest: current ? { id: safe.id(current.id), sequence: safe.count(current.data.sequence, 1) } : null, currentRequestUnavailable: !!task.data.currentRequestId && !current, latestSubmission: latest ? { id: safe.id(latest.id), requestId: safe.id(latest.data.requestId), sequence: safe.count(latest.data.sequence, 1), mode: safe.choice(latest.data.mode, ['partial', 'full']), contentHash: safe.hash(latest.data.contentHash), submittedAt: safe.time(latest.data.submittedAt), fileVersionIds: latest.data.fileVersionIds.map(safe.id), products } : null, latestMatchesCurrentRequest: latest && current ? latest.data.requestId === current.id : null, currentEvaluation, originalSubmittedEvaluation, currentProducts, inquiries, corrections, externalActions, campaign: (await available(async () => (await collectCampaignResidual(s, p, task, clock, sourceRows)))), ai: (await available(async () => (await collectAiResidual(s, p, task, clock, sourceRows)))) };
    // File/current relationship changes also invalidate a stale preview, even without task revision changes.
    const fileIds = new Set([...(current?.data.content.referenceFileIds ?? []), ...(latest?.data.fileVersionIds ?? []), ...(await s.list('completionExternalAction', context)).filter(r => r.data.taskId === task.id).flatMap(r => [...r.data.source.fileVersionIds, ...r.data.evidenceFileVersionIds])]);
    sourceRows.push(...(await s.list('fileVersion', context)).filter(r => fileIds.has(r.id)));
