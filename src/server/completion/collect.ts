...
export function collectBasis(s: UnitOfWork, p: Principal, task: StoredRecord<'task'>, clock: Clock) {
    ...
    const currentEvaluation = available(() => { if (!current || current.data.taskId !== task.id)
        unavailable(); return safe.evaluation(safeEvaluation(current.data.content, latest?.data.answers ?? [], original?.data.content ?? null, true, campaignRequestSource(s, current)?.noMaterials === true)); });
    const originalSubmittedEvaluation = available(() => { if (!original || !latest)
        unavailable(); return safe.evaluation(safeEvaluation(original.data.content, latest.data.answers, original.data.content, false, campaignRequestSource(s, original)?.noMaterials === true)); });
