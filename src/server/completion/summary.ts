...
export async function readCompletionSummary(s: UnitOfWork, p: Principal, taskId: string, clock: Clock) {
    const task = (await completionTask(s, p, taskId, clock)), rows = (await s.list('completionSnapshot', task.contextId!)).filter(c => c.data.taskId === task.id).sort((a, b) => safe.count(b.data.sequence, 1) - safe.count(a.data.sequence, 1)), latest = rows[0];
