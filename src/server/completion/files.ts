...
export class CompletionFiles {
    ...
    async download(token: string | undefined, id: string, query: URLSearchParams) {
        ...
        if (!actionId || !['download', 'preview', 'original'].includes(mode))
            fail('VALIDATION', 422, '전달 기록과 정확한 파일 버전을 선택해 주세요.');
        const authorize = async () => this.identity.repo.transaction(async (s) => {
            const p = (await this.identity.principal(s, token)), row = (await s.get('completionExternalAction', actionId));
            if (!row)
                unavailable();
            const action = (await externalDTO(s, p, row, this.identity.clock));
            if (!action.source.fileVersionIds.includes(id) && !action.evidenceFiles.some(f => f.id === id))
                unavailable();
