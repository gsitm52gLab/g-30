...
export class SchedulingService {
    ...
    async list(token: string | undefined, contextId: string, options: {
        ...
        return this.identity.repo.transaction(async (s) => {
            ...
            const actors = p.user.data.role === 'gsg' ? (await asyncFlatMap((await s.list('user')), async (u) => { const taskIds = (await asyncFilter(tasks, async (t) => (await actionRecipientIds(s, (await s.get('task', t.id))!, u.id)).length)).map(t => t.id); return taskIds.length ? [{ id: u.id, label: u.data.name, role: u.data.role, taskIds }] : []; })) : [];
