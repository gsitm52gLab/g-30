...
export class SchedulingService {
    ...
    async list(token: string | undefined, contextId: string, options: {
        taskId?: string;
        kind?: string;
        from?: string;
        to?: string;
        ...
        if (options.from)
            dateValue(options.from);
        if (options.to)
            dateValue(options.to);
        ...
        return this.identity.repo.transaction(async (s) => {
            ...
            if (options.taskId)
                (await sourceTask(s, p, options.taskId, this.clock));
            const items = (await asyncMap((await scheduleSources(s, p, contextId, this.clock)), async (row) => (await calendarDTO(s, p, row, this.clock)))).filter(r => (!options.taskId || r.taskId === options.taskId) && (!options.kind || r.kind === options.kind) && (!options.from || r.calendar.dueDay !== null && r.calendar.dueDay >= options.from) && (!options.to || r.calendar.dueDay !== null && r.calendar.dueDay <= options.to)).sort((a, b) => (a.calendar.dueDay ?? '9999').localeCompare(b.calendar.dueDay ?? '9999') || a.logicalKey.localeCompare(b.logicalKey));
            const tasks = (await asyncFlatMap((await s.list('task', contextId)), async (t) => (await decide(s, p, 'task.manage', taskScope(t), this.clock)).allowed ? [{ id: t.id, title: t.data.title }] : []));
            const actors = p.user.data.role === 'gsg' ? (await asyncFlatMap((await s.list('user')), async (u) => { const taskIds = (await asyncFilter(tasks, async (t) => (await actionRecipientIds(s, (await s.get('task', t.id))!, u.id)).length > 0)).map(t => t.id); return taskIds.length ? [{ id: u.id, label: u.data.name, role: u.data.role, taskIds }] : []; })) : [];
    ...
    async detail(token: string | undefined, id: string) {
        return this.identity.repo.transaction(async (s) => {
            const p = (await this.identity.principal(s, token)), v = (await manualSchedule(s, p, id, this.clock));
            const versions = (await asyncFlatMap((await s.list('scheduleVersion', v.row.contextId!)).filter(r => r.data.scheduleId === id).sort((a, b) => b.data.sequence - a.data.sequence), async (r) => { const dto = (await visibleSource(async () => (await manualVersionDTO(s, p, id, this.clock, r)))); return dto ? [dto] : []; }));
            return { id, contextId: v.row.contextId!, revision: v.row.revision, currentVersionId: v.version.id, current: (await manualVersionDTO(s, p, id, this.clock, v.version)), calendar: (await calendarDTO(s, p, v.source, this.clock)), versions, capabilities: { manage: (await decide(s, p, 'task.manage', taskScope(v.task), this.clock)).allowed } };
        });
    }
    async command(token: string | undefined, input: unknown) {
        ...
        if (!scheduleId && command !== 'save')
            unavailable();
        return this.identity.repo.transaction(async (s) => {
            const p = (await currentActor(s, (await this.identity.principal(s, token)), contextId, this.clock)), old = scheduleId ? (await manualSchedule(s, p, scheduleId, this.clock)) : null;
            if (old && old.row.contextId !== contextId)
                unavailable();
            const c = content ?? old!.content, task = (await sourceTask(s, p, c.taskId, this.clock));
            (await authorize(s, p, 'task.manage', taskScope(task), this.clock));
            if (task.contextId !== contextId || old && old.row.data.taskId !== task.id)
                unavailable();
            if (!(await activeRecipientIds(s, contextId, [c.deadline.responsibleUserId], 'gsg')).length && !(await activeRecipientIds(s, contextId, [c.deadline.responsibleUserId], 'brand')).length)
                fail('VALIDATION', 422, '현재 일정 확인 담당자를 선택해 주세요.');
            if (!['brand_reply', 'submission', 'correction'].includes(c.kind) && !(await actionRecipientIds(s, task, c.deadline.responsibleUserId)).length)
                fail('VALIDATION', 422, '이 업무의 현재 주·공동 담당자 또는 GSG 담당자를 선택해 주세요.');
            return (await receipt(s, p, contextId, `schedule.${command}:${scheduleId ?? 'new'}`, x, async () => {
                fresh(old?.row ?? null, x.expectedRevision);
                const root = old?.row ?? (await s.create('schedule', { id: newId(), contextId, data: { taskId: task.id, currentVersionId: null, createdBy: p.user.id } }));
