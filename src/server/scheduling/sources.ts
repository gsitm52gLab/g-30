...
export async function currentSubmissionNeed(s: UnitOfWork, p: Principal, task: StoredRecord<'task'>, request: StoredRecord<'requestVersion'>, clock: Clock) {
    const latest = (await latestSubmission(s, task)), old = latest ? (await s.get('requestVersion', latest.data.requestId)) : null;
    if (latest && (!old || old.data.taskId !== task.id || old.contextId !== task.contextId))
        unavailable();
    if (latest)
        (await submissionSnapshot(s, p, latest, clock));
    const source = (await campaignRequestSource(s, request));
    if (!requestRequirementsValid(request.data.content, source?.noMaterials === true))
        return { kind: 'brand_submission' as const, taskStatus: task.data.status, sourceAvailable: false, required: false, remaining: 0, participation: 'ordinary' as const };
...
export async function taskSchedules(s: UnitOfWork, principal: Principal, taskId: string, clock: Clock): Promise<SourceSchedule[]> {
    const task = (await sourceTask(s, principal, taskId, clock)), p = (await currentActor(s, principal, task.contextId!, clock));
    ...
    if (!request || request.data.taskId !== task.id || request.contextId !== task.contextId)
        unavailable();
    ...
    const need = (await currentSubmissionNeed(s, p, task, request, clock));
    const result: SourceSchedule[] = [{ ...base, logicalKey: `task:${task.id}:submission`, kind: 'submission', source: { kind: 'task_request', targetId: task.id, versionId: request.id, itemKey: 'deadline' }, deadline: sourceDeadline(request.data.content.deadline), visibility: 'public', need, ...recipientFacts(p, (await brandRecipientIds(s, task))), reminderSupport: 'current_need' }];
    for (const m of request.data.content.milestones) {
        if (m.visibility !== 'public' && m.visibility !== 'internal')
            unavailable();
        if (m.visibility === 'internal' && p.user.data.role !== 'gsg')
            continue;
        result.push({ ...base, logicalKey: `task:${task.id}:milestone:${m.id}`, kind: m.kind, source: { kind: 'task_milestone', targetId: task.id, versionId: request.id, itemKey: sourceText(m.id) }, deadline: sourceDeadline(m.deadline), visibility: m.visibility, recipientPolicy: 'external_gsg', nextAction: '현재 GSG 업무 담당자의 외부 일정 진행 확인', need: { kind: 'responsible_action', taskStatus: task.data.status, sourceAvailable: true, pending: true, recipientRole: 'gsg', participationActive: true }, ...recipientFacts(p, (await ownerRecipientIds(s, task))), reminderSupport: 'current_need' });
    }
    return result;
}
export async function campaignSchedules(s: UnitOfWork, principal: Principal, campaignId: string, clock: Clock): Promise<SourceSchedule[]> {
    const { row, task } = (await resolveCampaign(s, principal, campaignId, clock)), p = (await currentActor(s, principal, task.contextId!, clock));
    if (!row.data.currentVersionId)
        return [];
    const v = (await s.get('campaignVersion', row.data.currentVersionId));
    if (!v || v.data.campaignId !== row.id || v.contextId !== task.contextId)
        unavailable();
    // Existing version projection checks current original files/products AND campaign references.
    const content = (await versionDTO(s, p, v, clock)), result: SourceSchedule[] = [];
    for (const menu of content.menus) {
        const state = (await menuProgress(s, p, v, menu, clock)), key = menuIdentityKey(menu.identity);
        ...
        for (const item of rows) {
            ...
            if (item.kind === 'application') {
                pending = state.state.application === 'not_applied';
                nextAction = '외부 신청 확인';
            }
            if (item.kind === 'review') {
                pending = state.state.selection === 'pending';
                nextAction = '외부 검토 결과 확인';
            }
            if (item.kind === 'publication_use') {
                pending = state.state.execution !== 'finished';
                nextAction = '게시·사용 진행 확인';
            }
            for (const physical of state.physical) {
                if (item.key === `physical:${physical.definition.key}:ship`) {
                    pending = physical.facts.every(f => f.kind !== 'dispatch');
                    nextAction = '발송 기록 확인 · 수령과 별도';
                }
                if (item.key === `physical:${physical.definition.key}:arrival`) {
                    pending = physical.receiptFacts === 0;
                    nextAction = '수령 관측 확인 · 이행 완료 추정 없음';
                }
            }
            for (const followup of state.followups)
                if (item.key === `followup:${followup.definition.key}`) {
                    pending = followup.status === 'pending';
                    nextAction = '행사 후속 자료 수령 확인';
                }
            const duplicateSubmission = item.key === 'request';
            result.push({ ...base, logicalKey: `campaign:${row.id}:${key}:${item.key}`, kind: item.kind, source: { kind: 'campaign', targetId: row.id, versionId: v.id, itemKey: `${key}:${item.key}` }, deadline: sourceDeadline(item.deadline), recipientPolicy: duplicateSubmission ? 'task_assignees' : 'external_gsg', nextAction: duplicateSubmission ? '업무의 현재 제출 요청에서 안내' : `현재 GSG 업무 담당자의 ${nextAction}`, need: duplicateSubmission ? null : { kind: 'responsible_action', taskStatus: task.data.status, sourceAvailable: true, pending, recipientRole: 'gsg', participationActive: true }, ...recipientFacts(p, duplicateSubmission ? (await brandRecipientIds(s, task)) : (await ownerRecipientIds(s, task))), reminderSupport: duplicateSubmission ? 'source_schedule_only' : 'current_need' });
        }
    }
    return result;
}
export async function inquirySchedules(s: UnitOfWork, principal: Principal, conversationId: string, clock: Clock): Promise<SourceSchedule[]> {
    ...
    return (await questions(s, row)).flatMap(q => {
        const e = q.data.externalWait;
        if (q.data.state !== 'external_waiting' || !e)
            return [];
