...
import { currentActor, sourceTask, recipientFacts, activeRecipientIds, brandRecipientIds } from './access';
import { sourceText } from './projection';
import { storedContent, storedState } from './stored';
import { currentSubmissionNeed } from './sources';
...
export function manualSchedule(s: UnitOfWork, principal: Principal, id: string, clock: Clock, versionId?: string) {
    ...
    const state = storedState(version.data.state), isSubmission = ['brand_reply', 'submission', 'correction'].includes(content.kind), recipientIds = isSubmission ? brandRecipientIds(s, task) : activeRecipientIds(s, row.contextId, [content.deadline.responsibleUserId], 'gsg');
    const request = task.data.currentRequestId ? s.get('requestVersion', task.data.currentRequestId) : null;
    const need = isSubmission ? request ? currentSubmissionNeed(s, p, task, request, clock) : { kind: 'brand_submission' as const, taskStatus: task.data.status, sourceAvailable: false, required: false, remaining: 0, participation: 'ordinary' as const } : { kind: 'responsible_action' as const, taskStatus: task.data.status, sourceAvailable: true, pending: state === 'open' };
    const source: SourceSchedule = { logicalKey: `manual:${id}`, contextId: row.contextId, taskId: task.id, title: content.title, kind: content.kind, source: { kind: 'manual', targetId: id, versionId: version.id, itemKey: 'deadline' }, sourceRevision: row.revision, deadline: content.deadline, visibility: content.visibility, actionUrl: `/schedule/${encodeURIComponent(id)}?context=${encodeURIComponent(row.contextId)}`, unresolvedConflict: content.conflicts.some(c => c.state === 'unresolved'), active: state === 'open', need, ...recipientFacts(p, recipientIds), reminderSupport: 'current_need' };
