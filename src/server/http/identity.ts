...
export async function readBody(request: Request, keys: string[], limit = 16384) {
    ...
    if (!body || typeof body !== "object" || Array.isArray(body) || Object.keys(body).some(k => !keys.includes(k)))
        fail("VALIDATION", 422, "허용되지 않은 입력 항목입니다.");
    return body as Record<string, unknown>;
}
export async function route(request: Request, action: (service: IdentityService, token: string | undefined) => Promise<Response>) {
    try {
