...
export function createMockRepository(clock: Clock = systemClock): RecordRepository {
  ...
  const store: UnitOfWork = {
    ...
    create<K extends RecordKind>(kind: K, input: RecordInput<K>) {
      checkInput(input);
      if (records.has(key(kind, input.id))) throw new StoreError("CONFLICT");
      const now = clock();
      const record: StoredRecord<K> = { kind, ...jsonCopy(input), revision: 1, createdAt: now, updatedAt: now };
      records.set(key(kind, input.id), record); return jsonCopy(record);
    },
    update<K extends RecordKind>(kind: K, id: string, expectedRevision: number, data: RecordDataMap[K]) {
      const current = store.get(kind, id);
      if (!current) throw new StoreError("NOT_FOUND");
      if (current.revision !== expectedRevision) throw new StoreError("CONFLICT");
      ...
      records.set(key(kind, id), record); return jsonCopy(record);
    },
  };
  return { mode: "mock", get: async (kind, id) => store.get(kind, id), list: async (kind, contextId) => store.list(kind, contextId),
    async transaction<T>(operation: (uow: UnitOfWork) => T): Promise<T> {
      ...
      let active = true;
      const guarded = new Proxy(store, { get(target, prop: keyof UnitOfWork) { return (...args: unknown[]) => { if (!active) throw new StoreError("ASYNC_TRANSACTION"); return Reflect.apply(target[prop], target, args); }; } });
      try { return assertSynchronous(operation(guarded)); }
