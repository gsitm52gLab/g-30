...
interface Row { kind: RecordKind; id: string; context_id: string | null; data: string; revision: number; created_at: string; updated_at: string }
...
export function createSqliteRepository(db: Database.Database, clock: Clock = systemClock): RecordRepository {
  try { db.prepare("SELECT id FROM records LIMIT 1").get(); }
  catch { db.close(); throw new StoreError("STORAGE_UNAVAILABLE"); }
  const uow: UnitOfWork = {
    ...
    create<K extends RecordKind>(kind: K, input: RecordInput<K>) {
      checkInput(input); const data = jsonCopy(input.data); const now = clock();
      ...
      if (!result.changes) throw new StoreError("CONFLICT");
    ...
    update<K extends RecordKind>(kind: K, id: string, expectedRevision: number, data: RecordDataMap[K]) {
      const old = uow.get(kind, id); if (!old) throw new StoreError("NOT_FOUND");
      checkInput({ id, contextId: old.contextId, data });
      const copy = jsonCopy(data); const now = clock();
  ...
  return { mode: "sqlite", get: async (kind, id) => uow.get(kind, id), list: async (kind, contextId) => uow.list(kind, contextId),
    async transaction<T>(operation: (store: UnitOfWork) => T): Promise<T> {
      return db.transaction(() => {
        let active = true;
        const guarded = new Proxy(uow, { get(target, prop: keyof UnitOfWork) { return (...args: unknown[]) => { if (!active) throw new StoreError("ASYNC_TRANSACTION"); return Reflect.apply(target[prop], target, args); }; } });
        try { return assertSynchronous(operation(guarded)); } finally { active = false; }
      }).immediate();
    }, close() { db.close(); },
  };
}
