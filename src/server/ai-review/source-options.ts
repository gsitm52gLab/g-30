...
export async function correctionTargets(s: UnitOfWork, p: Principal, content: AiContent, finding: Finding, clock: Clock): Promise<ReviewTarget[]> {
    const ref = content.submission;
    if (!ref)
        return [];
    const sub = (await s.get('submission', ref.submissionId));
    if (!sub || sub.data.taskId !== ref.taskId || sub.data.requestId !== ref.requestId)
        unavailable();
    ...
    if (content.kind === 'text') {
        if (content.text === sub.data.narrative)
            candidates.push(base);
        for (const answer of sub.data.answers) {
            if ((answer.type === 'short_text' || answer.type === 'long_text') && content.text === answer.input.text) {
                const productUseIds = (await asyncFilter(ref.productUseIds, async (id) => answer.productId === null || (await s.get('productUseSnapshot', id))?.data.productId === answer.productId));
    ...
    return (await asyncFlatMap(candidates, async (target) => { try {
        return [(await exactTarget(s, p, target, clock)).target];
    }
    catch (error) {
