...
export async function sourceChoices(s: UnitOfWork, p: Principal, contextId: string, clock: Clock) {
    ...
    for (const version of (await s.list('contextProductVersion', contextId))) {
        const cp = (await s.get('contextProduct', version.data.contextProductId));
        if (!cp)
            continue;
    ...
    for (const request of (await s.list('requestVersion', contextId)))
    ...
    for (const submission of (await s.list('submission', contextId))) {
    ...
    return (await asyncFlatMap(candidates, async (c) => {
        try {
            const file = (await resolveSource(s, p, contextId, c.source, clock));
            return [{ source: sourceDTO(c.source), metadata: c.metadata, label: c.label, file: { ...fileMetadata(file), ...fileUrls(file, sourceReference(file)), uploaderLabel: (await userLabel(s, p, contextId, file.data.uploaderId)), uploadedAt: file.createdAt } }];
        }
        catch {
            return [];
        }
