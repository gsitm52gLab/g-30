...
export function versionDTO(s: UnitOfWork, p: Principal, versionId: string, clock: Clock) {
    const { version: v, file } = resolveEvidenceVersion(s, p, versionId, clock), contextId = v.contextId!;
    const links = visibleLinks(s, p, v, clock).map(({ link, product }) => ({ id: link.id, revision: link.revision, productId: link.data.productId, contextProductId: link.data.contextProductId, productName: text(product.common.data.common.name), productCode: text(product.common.data.common.code), active: link.data.active === true,
        assessments: s.list('evidenceAssessment', contextId).filter(a => a.data.linkId === link.id).sort((a, b) => b.data.sequence - a.data.sequence).map(a => ({ id: a.id, status: assessmentStatuses.includes(a.data.status) ? a.data.status : 'pending' as const, reason: text(a.data.reason), actorLabel: userLabel(s, p, contextId, a.data.assessedBy), at: text(a.data.assessedAt), sequence: a.data.sequence })),
    ...
    return { id: v.id, evidenceId: v.data.evidenceId, sequence: v.data.sequence, previousId: nullable(v.data.previousId), source: sourceDTO(v.data.source), metadata: metadataDTO(v.data.metadata), recorderLabel: userLabel(s, p, contextId, v.data.recordedBy), recordedAt: text(v.data.recordedAt), file: { ...fileMetadata(file), uploaderLabel: userLabel(s, p, contextId, file.data.uploaderId), uploadedAt: file.createdAt }, links };
}
