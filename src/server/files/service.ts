...
export function sourceReference(file: StoredRecord<"fileVersion">): FileReference {
    if (file.data.owner?.kind === 'inquiry')
        return { kind: 'inquiry', conversationId: file.data.owner.conversationId };
    if (file.data.owner?.kind === 'notice')
        return { kind: 'notice', noticeId: file.data.owner.noticeId };
...
export class FileService {
    ...
    private async access(s: UnitOfWork, p: Principal, file: StoredRecord<"fileVersion">, reference: FileReference, mode: "original" | "download" | "preview") {
        const origin = (await originalScope(s, p, file, this.identity.clock)), target = (await referenceScope(s, p, reference, this.identity.clock));
        if (file.contextId !== target.contextId || file.contextId !== origin.contextId)
            unavailable();
        let included = false;
        if (typeof reference === "string") {
            const task = (await s.get("task", reference))!;
            const versions = (await s.list("requestVersion", task.contextId!)).filter(v => v.data.taskId === task.id);
            included = (await decide(s, p, "submission.write", taskScope(task), this.identity.clock)).allowed && (await s.list("submissionDraft", task.contextId!)).some(d => d.data.taskId === task.id && contentFiles(d.data).includes(file.id)) || versions.some(v => v.data.content.referenceFileIds.includes(file.id)) || (await s.list("submission", task.contextId!)).some(v => v.data.taskId === task.id && v.data.fileVersionIds.includes(file.id)) || (await canReadTemporarySubmissionFile(s, p, file, target, this.identity.clock)) || p.user.data.role === "gsg" && (task.data.draft?.referenceFileIds.includes(file.id) === true || task.id === file.data.taskId);
        }
        else if (reference.kind === 'inquiry') {
            included = (await inquiryFileIncluded(s, p, file, reference.conversationId, this.identity.clock, reference.messageId));
        }
        else if (reference.kind === 'notice') {
            const r = (await resolveNotice(s, p, reference.noticeId, this.identity.clock, false, reference.versionId));
            ...
            const r = (await resolveProduct(s, p, reference.contextId, reference.productId, this.identity.clock));
            included = file.data.owner?.kind === "product" && file.data.owner.contextProductId === r.relation.id || (await s.list("contextProductVersion", reference.contextId)).some(v => v.data.contextProductId === r.relation.id && v.data.files.some(f => f.fileVersionId === file.id));
        }
        if (!included)
            unavailable();
        (await canReferenceFile(s, p, file, target, this.identity.clock));
        (await authorize(s, p, `file.${mode}`, { id: file.id, contextId: file.contextId, kind: "file", visibility: file.data.visibility, originalScope: origin, referenceScope: target }, this.identity.clock));
    ...
    async checkUpload(token: string | undefined, reference: FileReference) {
        return this.identity.repo.transaction(async (s) => (await referenceScope(s, (await this.identity.principal(s, token)), reference, this.identity.clock, true)));
    }
    async upload(token: string | undefined, reference: FileReference, files: {
        ...
        try {
            ...
            return await this.identity.repo.transaction(async (s) => {
                const p = (await this.identity.principal(s, token)), scope = (await referenceScope(s, p, reference, this.identity.clock, true));
                if (visibility === "internal" && p.user.data.role !== "gsg")
                    fail("FORBIDDEN", 403, "내부 자료를 만들 권한이 없습니다.");
                const owner: FileOwner = typeof reference === "string" ? { kind: "task", taskId: reference } : reference.kind === 'notice' ? { kind: 'notice', noticeId: reference.noticeId } : { kind: "product", productId: reference.productId, contextProductId: (await resolveProduct(s, p, reference.contextId, reference.productId, this.identity.clock, true)).relation.id };
                const rows = (await asyncMap(prepared, async (f) => (await s.create("fileVersion", { id: f.id, contextId: scope.contextId, data: { taskId: typeof reference === "string" ? reference : null, owner, storageKey: f.id, originalName: f.name, mime: f.mime, bytes: f.buffer.length, sha256: f.sha256, uploaderId: p.user.id, visibility, preview: f.preview } }))));
                this.fault?.();
    ...
    async download(token: string | undefined, fileId: string, reference: FileReference, mode: "original" | "download" | "preview") {
        const row = await this.identity.repo.transaction(async (s) => {
            const p = (await this.identity.principal(s, token)), file = (await s.get("fileVersion", fileId));
            if (!file)
                unavailable();
            (await this.access(s, p, file, reference, mode));
            return file;
        });
        const bytes = await readFile(this.destination(row.data.storageKey));
        await this.identity.repo.transaction(async (s) => { const p = (await this.identity.principal(s, token)); (await this.access(s, p, row, reference, mode)); });
