...
import { evaluateAnswers, compatibleRequirement } from '@/domain/submissions/evaluate';
...
import { contentDTO, contentFiles, fileDTO, providerDTO, storedAnswerDTO } from './projection';
...
export function snapshotDTO(s: UnitOfWork, p: Principal, row: StoredRecord<'submission'>, clock: Clock) {
    ...
    return { id: row.id, taskId: task.id, contextId: task.contextId!, requestId: request.id,
        request: { id: request.id, sequence: request.data.sequence, content: projectedRequest(request.data.content, false, request.data.content.referenceFileIds.filter(id => { const file = s.get('fileVersion', id); return !!file && file.data.visibility === 'public' && visibleFile(s, p, file, taskScope(task), clock); })) },
        ...
        evaluation: evaluateAnswers(request.data.content, d.answers), review: { connected: false as const, status: 'pending' as const }, completion: { connected: false as const, status: 'not_completed' as const } };
}
export function workspace(s: UnitOfWork, p: Principal, taskId: string, clock: Clock) {
    ...
    const products = task.data.productIds.map(id => {
    ...
    return { taskId: task.id, contextId: task.contextId!, taskRevision: task.revision, userId: p.user.id, taskStatus: task.data.status,
        request: { id: request.id, sequence: request.data.sequence, content: projectedRequest(request.data.content, false, request.data.content.referenceFileIds.filter(id => { const f = s.get("fileVersion", id); return !!f && f.data.visibility === "public" && visibleFile(s, p, f, taskScope(task), clock); })) }, capabilities: caps,
        ...
        draftEvaluation: caps.editDraft ? evaluateAnswers(request.data.content, currentContent.answers, previous?.data.content ?? request.data.content) : null,
        submittedEvaluation: latest ? { sourceSubmissionId: latest.id, isCurrentRequest: latest.data.requestId === request.id, evaluation: evaluateAnswers(request.data.content, latest.data.answers, s.get("requestVersion", latest.data.requestId)?.data.content ?? null, true) } : null,
