...
export function snapshotDTO(s: UnitOfWork, p: Principal, row: StoredRecord<'submission'>, clock: Clock) {
    ...
    return { id: row.id, taskId: task.id, contextId: task.contextId!, requestId: request.id,
        ...
        evaluation: safeEvaluation(request.data.content, d.answers), review: readSubmissionReview(s,p,task.id,row.id,clock), completion: readCompletionSummary(s,p,task.id,clock) };
}
export function workspace(s: UnitOfWork, p: Principal, taskId: string, clock: Clock) {
    ...
    return { taskId: task.id, contextId: task.contextId!, taskRevision: task.revision, userId: p.user.id, taskStatus: task.data.status,
        ...
        draftEvaluation: caps.editDraft ? safeEvaluation(request.data.content, currentContent.answers, previous?.data.content ?? request.data.content) : null,
        submittedEvaluation: latest ? { sourceSubmissionId: latest.id, isCurrentRequest: latest.data.requestId === request.id, evaluation: safeEvaluation(request.data.content, latest.data.answers, s.get("requestVersion", latest.data.requestId)?.data.content ?? null, true) } : null,
