...
export class SubmissionService {
    ...
    async snapshot(token: string | undefined, id: string) {
        return this.identity.repo.transaction(async (s) => {
            ...
            if (!row)
                unavailable();
            return (await snapshotDTO(s, p, row, this.clock));
        });
    }
    async evaluate(token: string | undefined, taskId: string, input: Record<string, unknown>) {
        object(input, ['baseRequestId', 'content']);
        return this.identity.repo.transaction(async (s) => {
            const p = (await this.identity.principal(s, token)), { task, request } = (await this.editable(s, p, taskId));
            assertRequest(task, input.baseRequestId);
            const content = parseDraft(input.content, request.id, request.data.content, (await materialProductIds(s, task, request)));
            const fileIssues = (await asyncFlatMap(contentFiles(content), async (fileVersionId) => {
                try {
                    (await publicFile(s, p, task, fileVersionId, this.clock));
                    return [];
                }
                catch {
            ...
            return { requestId: request.id, evaluation: safeEvaluation(request.data.content, content.answers, request.data.content, false, (await campaignRequestSource(s, request))?.noMaterials === true), fileIssues, productIssues, missingProductIds: (await materialProductIds(s, task, request)).filter(id => !content.productSelections.some(v => v.productId === id)), invalidLinkIndexes: content.links.flatMap((link, index) => validLink(link) ? [] : [index]) };
        });
    }
    async rebasePreview(token: string | undefined, taskId: string) { return this.identity.repo.transaction(async (s) => (await rebasePreview(s, (await this.identity.principal(s, token)), taskId, this.clock))); }
    private async editable(s: UnitOfWork, p: Principal, taskId: string) {
        const scope = (await submissionTask(s, p, taskId, this.clock, true));
        if (!(await capabilities(s, p, scope.task, this.clock)).editDraft)
            fail('CONFLICT', 409, '완료된 업무의 답변은 변경할 수 없습니다.');
        return scope;
    }
    private async saveRow(s: UnitOfWork, p: Principal, task: StoredRecord<'task'>, requestId: string, content: DraftContent, providedBy: SubmissionDraftData['providedBy'], old: StoredRecord<'submissionDraft'> | null, baseSubmissionId: string | null, carried?: StoredAnswer[]) {
        for (const id of contentFiles(content))
            (await publicFile(s, p, task, id, this.clock));
        ...
        return old ? (await s.update('submissionDraft', old.id, old.revision, data)) : (await s.create('submissionDraft', { id: newId(), contextId: task.contextId, data }));
    }
    async draft(token: string | undefined, taskId: string, input: Record<string, unknown>) {
        ...
        return this.identity.repo.transaction(async (s) => {
            const p = (await this.identity.principal(s, token)), { task, request } = (await this.editable(s, p, taskId)), old = (await sharedDraft(s, task));
            const command = enumValue(input.command, ['save', 'rebase_apply', 'copy_submission']);
            return (await receipt(s, p, task.contextId!, `submission.draft.${command}:${task.id}`, input, async () => {
                ...
                const providedBy = (await provider(s, p, task.contextId!, input.providedBy));
                if (command === 'save') {
                    assertRequest(task, input.baseRequestId);
                    if (old && old.data.baseRequestId !== request.id)
                        fail('REQUEST_CHANGED', 409, '요청 변경 내용을 먼저 비교하고 초안을 전환해 주세요.');
                    data = parseDraft(input.content, request.id, request.data.content, (await materialProductIds(s, task, request)));
                }
                    ...
                    const preview = (await rebasePreview(s, p, task.id, this.clock)), selected = list(input.carryAnswers, 400).map(v => { const a = object(v, ['requirementKey', 'productId']); return JSON.stringify([ids([a.requirementKey])[0], a.productId === null ? null : ids([a.productId])[0]]); });
                    ...
                    data = parseDraft({ ...prior, answers, artifacts: prior.artifacts.filter(a => !a.answer || selected.includes(answerKey(a.answer))), productSelections: (await asyncFilter(prior.productSelections, async (v) => (await materialProductIds(s, task, request)).includes(v.productId))) }, request.id, request.data.content, (await materialProductIds(s, task, request)));
                }
                else {
                    assertRequest(task, input.baseRequestId);
                    const source = (await s.get('submission', ids([input.sourceSubmissionId])[0]));
                    ...
                    (await snapshotDTO(s, p, source, this.clock));
    ...
    async submit(token: string | undefined, taskId: string, input: Record<string, unknown>) {
        ...
        return this.identity.repo.transaction(async (s) => {
            const p = (await this.identity.principal(s, token)), { task, request } = (await submissionTask(s, p, taskId, this.clock, true)), mode = enumValue(input.mode, ['partial', 'full']), draft = (await sharedDraft(s, task));
            return (await receipt(s, p, task.contextId!, `submission.submit:${task.id}`, input, async () => {
                assertRequest(task, input.baseRequestId);
                if (!draft)
                    fail('DRAFT_REQUIRED', 409, '먼저 공유 초안을 저장해 주세요.');
                const used = (await s.list('submission', task.contextId!)).find(v => v.data.draftId === draft.id && v.data.committedDraftRevision === input.expectedDraftRevision);
                if (used) {
                    if (used.data.mode !== mode || used.data.requestId !== request.id)
                        fail('CONFLICT', 409, '이미 제출한 초안입니다. 수정한 초안을 저장해 주세요.');
                    return { ids: [used.id] };
                }
                if (!(await capabilities(s, p, task, this.clock)).submit)
                ...
                const content = parseDraft(contentDTO(draft.data), request.id, request.data.content, (await materialProductIds(s, task, request)));
                const evaluation = safeEvaluation(request.data.content, content.answers, request.data.content, false, (await campaignRequestSource(s, request))?.noMaterials === true);
                ...
                if ((await materialProductIds(s, task, request)).some(id => !content.productSelections.some(v => v.productId === id)))
                    fail('PRODUCT_SELECTION_REQUIRED', 422, '연결된 각 상품의 정확한 버전과 자료·소비자가 선택을 확인해 주세요.');
                const files = (await asyncMap(contentFiles(content), async (id) => (await publicFile(s, p, task, id, this.clock)))), previous = (await latestSubmission(s, task)), submissionId = newId();
                const productUseIds = (await asyncMap(content.productSelections, async (selection) => {
                    const r = (await resolveProduct(s, p, task.contextId!, selection.productId, this.clock));
                    for (const bid of selection.bindingIds) {
                        const binding = r.local.data.files.find(b => b.id === bid);
                        if (!binding)
                            unavailable();
                        (await publicFile(s, p, task, binding.fileVersionId, this.clock));
                    }
                    const use = (await captureProductUse(s, p, { ...selection, contextId: task.contextId!, ownerType: 'submission', ownerId: submissionId, taskId: task.id, requestId: request.id }, this.clock));
