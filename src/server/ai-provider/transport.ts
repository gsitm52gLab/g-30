...
export type ApprovedPayload = Extract<TransferResult, {
    allowed: true;
}>['payload'];
...
export function providerRequest(model: string, payload: ApprovedPayload, corpus: CorpusRelease) {
    ...
    if (Buffer.byteLength(JSON.stringify(request)) > PROVIDER_LIMITS.requestBytes)
        throw new ProviderFailure('INPUT_LIMIT');
    return request;
}
...
export type Transport = (config: ServerConfig['openai'], request: ReturnType<typeof providerRequest>, beforeDispatch: () => Promise<void> | Promise<Promise<void>>, onDispatch: () => Promise<void> | Promise<Promise<void>>) => Promise<TransportResult>;
...
export const openaiTransport: Transport = async (config, request, beforeDispatch, onDispatch) => {
    ...
    try {
        ...
        receivedRequestId = identifier(response.headers.get('x-request-id'));
        ...
        const rawCandidate = texts.join('');
        result.issue = refusal ? 'REFUSAL' : data.status === 'incomplete' ? 'INCOMPLETE' : data.status !== 'completed' ? 'SERVER_ERROR' : !rawCandidate || Buffer.byteLength(rawCandidate) > 262144 ? 'PARSE_ERROR' : null;
        result.rawCandidate = rawCandidate && Buffer.byteLength(rawCandidate) <= 262144 ? rawCandidate : null;
        return finish();
    }
    catch (e) {
        if (e instanceof SyntaxError)
            return { ...emptyTransport('PARSE_ERROR'), requestId: receivedRequestId };
        if (e instanceof ProviderFailure)
            return emptyTransport(e.issue);
        if (e instanceof OpenAI.APIConnectionTimeoutError)
            return { ...emptyTransport('TIMEOUT', true), requestId: receivedRequestId };
        if (e instanceof OpenAI.APIConnectionError)
            return emptyTransport('NETWORK', true);
        if (e instanceof OpenAI.APIError) {
            const issue: ProviderIssue = e.status === 401 ? 'PROVIDER_AUTH' : e.status === 403 ? 'PROVIDER_PERMISSION' : e.status === 429 ? 'RATE_LIMIT' : e.status && e.status >= 500 ? 'SERVER_ERROR' : 'CONFIGURATION';
            return { ...emptyTransport(issue), requestId: identifier(e.requestID) };
        }
