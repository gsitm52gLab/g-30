...
async function access(s: UnitOfWork, p: Principal, contextId: string, clock: Clock, write = true, privatePrice = false) {
...
export class ImportService {
    ...
    async configuration(token: string | undefined, contextId: string) { return this.identity.repo.transaction(async (s) => { const p = (await this.identity.principal(s, token)), privatePrice = (await access(s, p, contextId, this.clock)); return { contextId, brandId: (await s.get('context', contextId))!.data.brandId!, schema: IMPORT_SCHEMA, fields: importFields.filter(f => privatePrice || !f.privatePrice).map(f => ({ ...f })), limits: importLimits, capabilities: { apply: true, internalPrice: privatePrice }, operationalWorkbookValidation: 'NOT_RUN' as const, operationalWorkbookNote: '현업 원본 Excel은 아직 제공되지 않았습니다. 표준 양식 검증과 별도로 확인합니다.' }; }); }
    async inspect(token: string | undefined, contextId: string, name: string, bytes: Buffer) {
        const actorId = await this.identity.repo.transaction(async (s) => { const p = (await this.identity.principal(s, token)); (await access(s, p, contextId, this.clock)); return p.user.id; });
        const workbook = await inspectWorkbook(name, bytes);
        await this.identity.repo.transaction(async (s) => (await access(s, (await this.identity.principal(s, token)), contextId, this.clock)));
    ...
    async preview(token: string | undefined, inputValue: PreviewInput) {
        ...
        const stage = await this.identity.repo.transaction(async (s) => {
            ...
            if (p.user.id !== source.actorId)
                unavailable();
            const canPrice = (await access(s, p, source.contextId, this.clock));
    ...
    async readPreview(token: string | undefined, id: string, page = 1) {
        const stage = await this.staging.get<PreviewStage>(id);
        await this.identity.repo.transaction(async (s) => {
            const p = (await this.identity.principal(s, token));
            if (p.user.id !== stage.actorId)
                unavailable();
            (await access(s, p, stage.contextId, this.clock, true, stage.privatePrice));
        });
    ...
    async apply(token: string | undefined, input: Record<string, unknown>) {
        ...
        const replay = await this.identity.repo.transaction(async (s) => {
            ...
            if (!batch)
                return null;
            (await access(s, p, batch.contextId!, this.clock, true, batch.data.includesInternalPrice));
    ...
    async batch(token: string | undefined, id: string) {
        return this.identity.repo.transaction(async (s) => {
            ...
            if (!b?.contextId)
                unavailable();
            (await access(s, p, b.contextId, this.clock, false, b.data.includesInternalPrice));
            return batchDTO(b);
        });
    }
    async workbook(token: string | undefined, contextId: string, kind: 'template' | 'export', includeInternal = false) {
        const data = await this.identity.repo.transaction(async (s) => {
            const p = (await this.identity.principal(s, token));
            (await access(s, p, contextId, this.clock, false, includeInternal));
        ...
        await this.identity.repo.transaction(async (s) => (await access(s, (await this.identity.principal(s, token)), contextId, this.clock, false, includeInternal)));
        return bytes;
    }
}
