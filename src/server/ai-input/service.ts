        ...
        if (claimed.claimId) {
            try {
                ...
                await this.identity.repo.transaction(async (s) => {
                    ...
                    (await audit(s, p, this.clock, run.contextId!, 'ai.extraction.finished', run.id, {}, { state }));
