...
export class AiAssets {
    ...
    async download(token: string | undefined, assetId: string) { const row = await this.identity.repo.transaction(async (s) => { const p = (await this.identity.principal(s, token)), row = (await s.get('aiAsset', id(assetId))); if (!row?.contextId)
        fail('NOT_FOUND', 404, '자료를 찾을 수 없습니다.'); return (await assetAccess(s, p, row.id, row.contextId, this.identity.clock)); }); let bytes: Buffer; try {
        bytes = await readFile(this.destination(row.data.storageKey));
    }
    catch {
        fail('STORAGE_UNAVAILABLE', 503, '원본 파일을 읽을 수 없습니다.');
    } await this.identity.repo.transaction(async (s) => (await assetAccess(s, (await this.identity.principal(s, token)), row.id, row.contextId!, this.identity.clock))); if (contentHash(bytes) !== row.data.sha256)
