...
export async function campaignRequestSource(s: UnitOfWork, r: StoredRecord<'requestVersion'>): Promise<CampaignRequestSource | null> {
    ...
    if (x === undefined)
        return null;
    if (!x || x.kind !== 'campaign_selection' || typeof x.noMaterials !== 'boolean' || !Number.isSafeInteger(x.eventSequence) || x.eventSequence < 1)
        unavailable();
    const strings = (v: unknown): string[] => { if (!Array.isArray(v) || v.some(x => typeof x !== 'string') || new Set(v).size !== v.length)
        unavailable(); return [...v]; };
    const text = (v: unknown): string => { if (typeof v !== 'string' || !v)
        unavailable(); return v; };
    ...
    const v = (await s.get('campaignVersion', result.campaignVersionId)), base = (await s.get('requestVersion', result.originalRequestId)), t = (await s.get('task', r.data.taskId)), sel = result.selectionVersionId ? (await s.get('campaignSelection', result.selectionVersionId)) : null, fact = result.sourceFactId ? (await s.get('campaignExternalFact', result.sourceFactId)) : null;
    if (!v || v.contextId !== r.contextId || v.data.taskId !== r.data.taskId || v.data.campaignId !== result.campaignId || v.data.requestId !== base?.id || base.data.taskId !== r.data.taskId || r.data.publishedBy !== result.actorId || !(await s.get('user', result.actorId)) || !t || result.materialProductIds.some(id => !t.data.productIds.includes(id)) || result.noMaterials !== (r.data.content.requirements.length === 0))
        unavailable();
    if (result.selectionVersionId && (!sel || sel.data.campaignVersionId !== v.id || sel.data.sequence > result.eventSequence) || result.sourceFactId && (!fact || fact.data.campaignVersionId !== v.id || fact.data.sequence !== result.eventSequence))
        unavailable();
    if (!fact && (sel?.data.recordedBy ?? v.data.recordedBy) !== result.actorId || fact && fact.data.recordedBy !== result.actorId)
        unavailable();
    return result;
}
export async function materialProductIds(s: UnitOfWork, task: StoredRecord<'task'>, request: StoredRecord<'requestVersion'>) { return (await campaignRequestSource(s, request))?.materialProductIds ?? task.data.productIds; }
/** Existing derived requests refer back to their GSG-approved basis, never the private task draft. */
export async function campaignPublicationBasis(s: UnitOfWork, task: StoredRecord<'task'>, campaignId: string, draftRequestId: string | null) {
    const current = task.data.currentRequestId ? (await s.get('requestVersion', task.data.currentRequestId)) : null;
    if (!current)
        conflict();
    const source = (await campaignRequestSource(s, current));
    if (source && source.campaignId !== campaignId)
        conflict();
    const saved = draftRequestId ? (await s.get('requestVersion', draftRequestId)) : null, savedSource = saved ? (await campaignRequestSource(s, saved)) : null;
    if (draftRequestId !== current.id && draftRequestId !== source?.originalRequestId && (!savedSource || savedSource.campaignId !== campaignId || savedSource.originalRequestId !== source?.originalRequestId))
        conflict();
    return source ? (await s.get('requestVersion', source.originalRequestId))! : current;
}
/** No route exposes this writer. It derives content from stored, approved definitions inside the caller UoW. */
export async function applyCampaignRequest(s: UnitOfWork, p: Principal, version: StoredRecord<'campaignVersion'>, clock: Clock, action: 'publish' | 'participate' | 'external', factId: string | null) {
    const task = (await s.get('task', version.data.taskId));
    if (!task || task.contextId !== version.contextId)
        unavailable();
    (await authorize(s, p, action === 'participate' ? 'submission.write' : 'task.manage', taskScope(task), clock));
    const current = task.data.currentRequestId ? (await s.get('requestVersion', task.data.currentRequestId)) : null, original = (await s.get('requestVersion', version.data.requestId));
    if (!current || !original)
        conflict();
    const prior = (await campaignRequestSource(s, current));
    if (current.id !== original.id && (!prior || prior.campaignId !== version.data.campaignId || prior.originalRequestId !== original.id))
        conflict();
    if (action !== 'publish' && prior?.campaignVersionId !== version.id)
        conflict();
    if (action === 'publish' && prior) {
        const old = (await s.get('campaignVersion', prior.campaignVersionId));
        if (!old)
            conflict();
        if ((await asyncSome(old.data.menus, async (m) => { const state = (await storedMenuState(s, old, safe.publicMenu(m))).state; return ['applied', 'withdrawal_requested'].includes(state.application) && state.cancellation !== 'cancelled'; })))
            conflict();
    }
    // A task has one published campaign scope at a time. Never erase another campaign's requirements.
    if ((await s.list('campaign', task.contextId!)).some(c => c.id !== version.data.campaignId && c.data.taskId === task.id && c.data.currentVersionId))
        conflict();
    const menus = version.data.menus.map(safe.publicMenu), states = (await asyncMap(menus, async (m) => (await storedMenuState(s, version, m)))), allKeys = new Set<string>(), definitions = new Map<string, Requirement>();
    for (const m of menus)
        for (const q of m.request.requirements) {
            const old = definitions.get(q.key);
            if (old && !same(old, q))
                conflict();
            definitions.set(q.key, q);
            allKeys.add(q.key);
        }
    ...
    if (requirements.some(q => q.condition && !requirements.some(parent => parent.key === q.condition!.key)))
        conflict();
    let content: RequestContent;
    try {
        content = parseContent({ ...base, requirements });
    }
    catch {
        conflict();
    }
    ...
    const selection = states[0]?.selection ?? null, fact = factId ? (await s.get('campaignExternalFact', factId)) : null;
    ...
    if (action === 'external' && prior && same(current.data.content, content) && same(prior.activeMenuKeys, source.activeMenuKeys) && same(prior.retainedCancellationMenuKeys, source.retainedCancellationMenuKeys) && same(prior.materialProductIds, products))
        return current.id;
    for (const id of content.referenceFileIds) {
        const f = (await s.get('fileVersion', id));
        if (!f || f.data.visibility !== 'public')
            unavailable();
