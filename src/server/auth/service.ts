...
export function hasScope(s: UnitOfWork, p: Principal, contextId: string, clock: Clock = systemClock) {
    return decide(s, p, "context.read", contextResource(contextId), clock).allowed;
}
export function needScope(s: UnitOfWork, p: Principal, contextId: string, clock: Clock = systemClock) {
    authorize(s, p, "context.read", contextResource(contextId), clock);
}
function needAdmin(s: UnitOfWork, p: Principal, contextId: string | undefined, clock: Clock) {
    authorize(s, p, contextId ? "membership.manage" : "account.manage", {
...
export class IdentityService {
    ...
    async me(token?: string) {
        return this.repo.transaction(s => {
            ...
            const contexts = s.list("context").filter(c => hasScope(s, p, c.id, this.clock)).map(projectContext);
            return { storageMode: this.repo.mode, user: sessionUser(p.user), contexts,
                memberships: s.list("membership").filter(m => m.data.userId === p.user.id).map(projectMembership),
    ...
    async members(token: string | undefined, contextId: string) {
        return this.repo.transaction(s => {
            ...
            return {
                ...
                tasks: s.list("task", contextId).map(t => projectTask(s, p, t, this.clock)),
                history: s.list("audit").filter(a => a.contextId === contextId ||
    ...
    async reissue(token: string | undefined, invitationId: string, input: Record<string, unknown>) { const expected = revision(input.expectedRevision); const raw = randomToken(); return this.repo.transaction(s => { const p = this.principal(s, token); const old = s.get("invitation", invitationId); if (!old)
        unavailable(); needScope(s, p, old.contextId!, this.clock); needAdmin(s, p, old.contextId!, this.clock); if (old.revision !== expected)
    ...
    async setUserStatus(token: string | undefined, userId: string, input: Record<string, unknown>) { const expected = revision(input.expectedRevision); const status = input.status; if (status !== "active" && status !== "suspended")
        fail("VALIDATION", 422, "계정 상태를 확인해 주세요."); return this.repo.transaction(s => { const p = this.principal(s, token); needAdmin(s, p, undefined, this.clock); const user = s.get("user", userId); if (!user)
        unavailable(); if (user.id === p.user.id)
    ...
    private markAssignments(s: UnitOfWork, userId: string, contextId?: string) { for (const task of s.list("task", contextId).filter(t => (t.data.assigneeId === userId || t.data.ownerId === userId || t.data.coAssigneeIds?.includes(userId)) && t.data.status !== "completed"))
    ...
    async setMembership(token: string | undefined, contextId: string, memberId: string, input: Record<string, unknown>) { const expected = revision(input.expectedRevision); const status = input.status; if (status !== "active" && status !== "suspended")
        fail("VALIDATION", 422, "멤버십 상태를 확인해 주세요."); return this.repo.transaction(s => { const p = this.principal(s, token); needScope(s, p, contextId, this.clock); needAdmin(s, p, contextId, this.clock); const member = s.get("membership", memberId); if (!member || member.contextId !== contextId)
        unavailable(); const user = s.get("user", member.data.userId)!; if (member.data.status === "invited")
        ...
        this.markAssignments(s, user.id, contextId); this.audit(s, p, "membership.changed", member.id, contextId, { status: member.data.status, scope: member.data.scope, internalPriceAccess: member.data.internalPriceAccess }, { status, scope, internalPriceAccess: price }); this.fault?.("membership"); return projectMembership(changed); }); }
    async reassign(token: string | undefined, contextId: string, input: Record<string, unknown>) { const taskId = text(input.taskId, "업무"); const toUserId = text(input.toUserId, "담당자"); const expected = revision(input.expectedRevision); const role = input.assignmentRole ?? "brand"; if (role !== "brand" && role !== "gsg")
        fail("VALIDATION", 422, "배정 역할을 확인해 주세요."); return this.repo.transaction(s => { const p = this.principal(s, token); needScope(s, p, contextId, this.clock); authorize(s, p, "task.manage", { id: taskId, contextId, kind: "task", visibility: "public" }, this.clock); const task = s.get("task", taskId); if (!task || task.contextId !== contextId)
        unavailable(); const target = s.get("user", toUserId); const member = activeMember(s, toUserId, contextId); if (!target || target.data.status !== "active" || target.data.role !== role || !member)
        fail("VALIDATION", 422, "같은 컨텍스트의 활성 담당자를 역할에 맞게 선택해 주세요."); const next = { ...task.data, ...(role === "brand" ? { assigneeId: toUserId, ...(task.data.coAssigneeIds ? { coAssigneeIds: task.data.coAssigneeIds.filter(uid => uid !== toUserId) } : {}) } : { ownerId: toUserId }) }; const assignmentNeedsAttention = [next.assigneeId, next.ownerId, ...(next.coAssigneeIds ?? [])].some(uid => s.get("user", uid)?.data.status !== "active" || !activeMember(s, uid, contextId)); const changed = s.update("task", task.id, expected, { ...next, assignmentNeedsAttention }); this.audit(s, p, "task.reassigned", task.id, contextId, { assigneeId: task.data.assigneeId, ownerId: task.data.ownerId, authorId: task.data.authorId, coAssigneeIds: task.data.coAssigneeIds ?? [] }, { assigneeId: changed.data.assigneeId, ownerId: changed.data.ownerId, authorId: changed.data.authorId, coAssigneeIds: changed.data.coAssigneeIds ?? [] }); if (task.data.schemaVersion === 2) s.create("domainEvent", { id: id(), contextId, data: { eventType: "TASK_ASSIGNMENT_CHANGED", targetId: task.id, sourceVersionId: task.data.currentRequestId ?? null, actorId: p.user.id, at: this.clock() } }); this.fault?.("reassign"); return projectTask(s, p, changed, this.clock); }); }
}
