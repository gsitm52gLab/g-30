...
export function privateFields(data: object, decision: Extract<Decision, {
    allowed: true;
}>) {
...
export async function projectTask(store: UnitOfWork, principal: Principal, row: StoredRecord<"task">, clock?: Clock) {
    const decision = (await authorize(store, principal, "task.read", taskScope(row), clock));
    const d = row.data;
    const submitted = (await store.list("submission", row.contextId!)).filter(v => v.data.taskId === row.id).sort((a, b) => b.data.sequence - a.data.sequence)[0];
...
export async function projectProduct(store: UnitOfWork, principal: Principal, row: StoredRecord<"product">, clock: Clock = systemClock, contextId?: string) {
    const scope = (await legacyProductScope(store, principal, row, clock, contextId));
    const decision = (await authorize(store, principal, "product.read", scope, clock));
    const d = row.data;
...
export function projectAudit(row: StoredRecord<"audit">) {
    ...
    function changes(value: Record<string, unknown>) {
        ...
        for (const key of auditKeys[d.action] ?? [])
            if (Object.hasOwn(value, key) && scalar(value[key]))
                safe[key] = value[key];
        if (d.action === "task.reassigned" && Array.isArray(value.coAssigneeIds) && value.coAssigneeIds.every(id => typeof id === "string"))
            safe.coAssigneeIds = [...value.coAssigneeIds];
        return safe;
    }
    return { ...metadata(row), data: {
...
export async function projectChannel(store: UnitOfWork, principal: Principal, action: import("./types").Action, scope: ResourceScope, source: Record<string, unknown>, clock?: Clock) {
    const decision = (await authorize(store, principal, action, scope, clock));
    return {
