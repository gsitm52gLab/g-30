...
export function ScheduleEditor({ value, onChange, dateTime, onDateTimeChange, tasks, responsibleOptions, responsibilityHelp, responsibilityLabel = "일정 확인 책임자", disabled = false, taskLocked = false, canSelectInternal, error }: Props) {
    ...
    <fieldset className={s.sub}><legend>날짜와 확정 수준</legend><div className={s.grid}>
      ...
      <label className={s.field}>날짜 정밀도<select aria-label="날짜 정밀도" aria-describedby={`${id}-precision-help`} value={d.precision} onChange={e => set('deadline', { ...d, precision: e.target.value as Deadline['precision'], value: d.value === null ? null : '' })}><option value="date">날짜만</option><option value="datetime">날짜와 시각</option></select><span id={`${id}-precision-help`} className={s.hint}>정밀도를 바꾸면 날짜를 다시 입력해 주세요.</span></label>
      {d.value !== null && (d.precision === 'date' ? <label className={s.field}>날짜<input type="date" required value={d.value} onChange={e => deadline('value', e.target.value)}/></label> : <><label className={s.field}>현지 날짜와 시각<input type="datetime-local" step="1" required value={dateTime.local} onChange={e => onDateTimeChange({ ...dateTime, local: e.target.value })}/></label><label className={s.field}>원문의 UTC 오프셋<input aria-label="원문의 UTC 오프셋" aria-describedby={`${id}-offset-help`} required value={dateTime.offset} onChange={e => onDateTimeChange({ ...dateTime, offset: e.target.value })} placeholder="+09:00 또는 Z" pattern="Z|[+-][0-9]{2}:[0-9]{2}" maxLength={6}/><span id={`${id}-offset-help`} className={s.hint}>서머타임 중복 시각은 명시한 오프셋으로 구분합니다.</span></label></>)}
