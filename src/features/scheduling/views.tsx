...
export function DeadlineFacts({ deadline, confirmationActorLabel }: {
    deadline: Deadline;
...
export function SourceFacts({ content }: {
    content: Pick<ScheduleContent, 'statements' | 'conflicts'>;
}) {
...
export function ScheduleTimeline({ state }: {
    ...
    if (state.status === 'loading')
        return <p className={s.note} role="status">일정을 불러오는 중입니다.</p>;
    if (state.status === 'denied')
        return <section className={s.empty}><h2>일정을 볼 수 없습니다</h2><p>{state.message}</p></section>;
