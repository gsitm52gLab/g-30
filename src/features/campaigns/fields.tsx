...
export function ProviderEditor({ label, value, onChange, c, external = true }: {
    ...
    c: Controller;
    external?: boolean;
}) { const d = c.state.data!; return <fieldset className={s.sub}><legend>{label}</legend><label className={s.field}>출처 구분<select value={value.kind} onChange={e => onChange(e.target.value === 'user' ? { kind: 'user', userId: d.catalog.userId } : { kind: 'external_source', label: '', source: '' })}><option value="user">현재 참여자</option>{external && <option value="external_source">외부 수행자·기관</option>}</select></label>{value.kind === 'user' ? <label className={s.field}>참여자<select value={value.userId} onChange={e => onChange({ kind: 'user', userId: e.target.value })}>{(d.task.canManage ? d.catalog.members : d.catalog.members.filter(m => m.id === d.catalog.userId)).map(m => <option value={m.id} key={m.id}>{m.name}</option>)}</select></label> : <div className={s.grid}><Field label="외부 이름·기관" required value={value.label} onChange={label => onChange({ ...value, label })}/><Field label="외부 기록 출처" value={value.source} onChange={source => onChange({ ...value, source })}/></div>}</fieldset>; }
