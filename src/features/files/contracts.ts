...
export class FileTransferError extends Error {
  constructor(readonly code: 'INVALID_INPUT' | 'EXPIRED' | 'TRANSFER_FAILED' | 'NOT_READY' | 'INTEGRITY' | 'ACCESS_CHANGED' | 'ABORTED') {
    super(code === 'ACCESS_CHANGED' ? '파일에 접근할 수 없습니다. 현재 권한을 확인해 주세요.' : code === 'INTEGRITY' ? '파일 내용을 확인하지 못했습니다. 다시 시도해 주세요.' : code === 'ABORTED' ? '파일 처리를 취소했습니다.' : '파일 처리를 완료하지 못했습니다. 이 파일만 다시 시도해 주세요.');
