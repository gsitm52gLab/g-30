...
export function ReadCoverage({ snapshot }: { snapshot: ExtractionSnapshot }) {
  return <section className={s.panel} aria-label="분석 대상의 실제 읽은 범위">
    ...
    <section className={s.section}><h3>명시적으로 선택한 범위</h3>
      {snapshot.selected.length ? <ul>{snapshot.selected.map((selected, i) => <li key={`${selected.sourceId}:${i}`}>{selected.sourceId} · {selected.pages !== null ? `PDF ${selected.pages.join(', ')}쪽` : selected.imageIndex !== null ? `이미지 ${selected.imageIndex + 1}` : '입력 텍스트'}</li>)}</ul> : <p>선택된 범위 없음</p>}
    </section>
    {snapshot.issues.length > 0 && <ul>{snapshot.issues.map(issue => <li key={issue}>{issueLabel[issue]}</li>)}</ul>}
    <div className={s.stack}>{snapshot.units.map((unit, index) => <article className={s.evidence} key={index} aria-label={unitPosition(unit)}>
      <h3>{unitPosition(unit)}</h3><p>{stateLabel[unit.status]} · {({ plain_text: '입력 텍스트', pdf_text_layer: 'PDF 텍스트 계층', ocr_partial: 'OCR 일부 읽기', none: '읽은 내용 없음' })[unit.coverage]}</p>
