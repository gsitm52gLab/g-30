...
export type Pending = {kind:'settings';url:string;body:{contextId:string;enabled:boolean;expectedRevision:number;idempotencyKey:string};receipt:string|null} | {kind:'retry';url:string;body:{expectedRevision:number;acknowledgeUnknown?:boolean;idempotencyKey:string};receipt:string|null} | { kind:'start'; url:string; body:StartAnalysis; receipt:string|null } | { kind:'review'; url:string; body:ReviewFinding; receipt:string|null } | { kind:'opinion'; url:string; body:CorrectionCommand; receipt:string|null };
