...
export async function ReviewPage({ kind, id, searchParams }: {
    ...
    const context = typeof q.context === 'string' ? q.context : undefined, workspace = await (await readWorkspace(context)).catch(workspaceFailure);
    if (!workspace)
    ...
    const initial = await (await (async () => { const auth = await identity(), token = await currentToken(), me = await auth.me(token), service = new AiReviewService(auth); const data = kind === 'list' ? await service.list(token, selection.contextId) : kind === 'input' ? await service.workspace(token, id!, selection.versionId) : kind === 'run' ? await service.detail(token, id!) : await service.corpus(token, selection.contextId, selection.releaseId); if ('contextId' in data && data.contextId !== selection.contextId)
