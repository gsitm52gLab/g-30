...
export type NotificationViewState = { status: 'loading' } | { status: 'denied'; message: string } | { status: 'error'; message: string } | { status: 'ready'; items: NotificationPresentation[]; unreadCount: number; total: number; filtered: boolean };
...
export function NotificationCenter({ state, sync, busyIds, onReadChange, onRetry, onRefresh }: Props) {
  if (state.status === 'denied') return <section className={s.empty}><h2>알림을 볼 수 없습니다</h2><p>{state.message}</p></section>;
