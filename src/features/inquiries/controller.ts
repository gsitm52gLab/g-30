...
class RetiredInquiryOperation extends Error {
}
export function useInquiry(id: string, contextId: string) {
    ...
    const state = useRef(recovery), current = useRef<ConversationDetailDTO | null>(null), key = useRef(''), user = useRef(''), locked = useRef(false), alive = useRef(true), halted = useRef(false), files = useRef(new Map<string, File>()), restore = useRef(false), loadSequence = useRef(0), generation = useRef(0);
    // Invalidate every pending continuation after denial or unmount, including local file hashing.
    const active = useCallback((version: number) => alive.current && !halted.current && generation.current === version, []);
    const update = useCallback((patch: Partial<Recovery>) => {
        if (!alive.current || halted.current)
            return;
        const next = { ...state.current, ...patch, at: Date.now() };
        state.current = next;
        setRecovery(next);
        if (key.current && !halted.current)
            save(key.current, next);
    }, []);
    const purge = useCallback(() => { if (!alive.current)
        return; generation.current++; halted.current = true; locked.current = false; setBusy(false); setNotice(''); key.current = ''; user.current = ''; current.current = null; state.current = blank(); files.current.clear(); clearAll(); setRecovery(blank()); setDetail(null); setBlocked(true); setConnection('현재 권한 확인 필요'); setError('현재 권한으로 문의를 볼 수 없습니다. 보호된 대화와 작성 내용을 지웠습니다.'); }, []);
    const report = useCallback((e: unknown) => {
        if (!alive.current || halted.current || e instanceof RetiredInquiryOperation)
            return;
        if (denied(e))
            purge();
    ...
    const refresh = useCallback(async () => {
        ...
        if (!active(version))
            throw new RetiredInquiryOperation();
    ...
    }, [contextId, id, purge, report, update, active]);
    const retire = useCallback(() => { alive.current = false; generation.current++; files.current.clear(); current.current = null; }, []);
    useEffect(() => {
        alive.current = true;
        halted.current = false;
        generation.current++;
        void refresh().catch(() => { });
        const onDenied = () => purge();
        const focus = () => {
            if (!halted.current)
                void refresh().catch(() => { });
        };
        window.addEventListener(deniedEvent, onDenied);
        window.addEventListener('focus', focus);
        return () => { retire(); window.removeEventListener(deniedEvent, onDenied); window.removeEventListener('focus', focus); };
    }, [purge, refresh, retire]);
    ...
    useEffect(() => {
        ...
        async function recover() {
            if (stopped || halted.current)
                return;
            setConnection('연결 끊김 · 현재 권한과 누락 대화 확인 중');
            try {
        ...
        function connect() {
            if (stopped || halted.current)
                return;
            stream?.close();
            stream = new EventSource(`/api/inquiries/${encodeURIComponent(id)}/stream?after=${encodeURIComponent(cursor)}`);
            stream.onopen = () => {
                if (!stopped)
    ...
    async function finish(command: InquiryCommand | null, version: number) {
        if (!active(version))
            return;
        await refresh();
        if (!active(version))
            return;
        if (command && 'content' in command) {
            files.current.clear();
            update({ body: '', files: [], intent: null, committed: null, title: '', questionId: '', mode: 'comment' });
        }
        else
            update({ intent: null, committed: null });
        setError('');
        setNotice('서버에 기록했습니다. 업무 수락·제출·완료와는 별도입니다.');
        changed();
    }
    async function run(command: InquiryCommand) {
        if (!alive.current || locked.current || halted.current || state.current.committed)
            return;
        const version = generation.current;
        locked.current = true;
        setBusy(true);
    ...
    async function reread() {
        if (!alive.current || locked.current || halted.current)
            return;
        const version = generation.current;
        locked.current = true;
        setBusy(true);
        try {
            await finish(state.current.intent, version);
        }
        catch (e) {
            report(e);
        }
    ...
    async function upload(selected: FileItem[]) {
        if (!alive.current || locked.current || halted.current)
            return;
        const version = generation.current;
        locked.current = true;
    ...
    async function choose(input: FileList | null, reselect?: string) {
        if (!input || !alive.current || halted.current)
            return;
        const version = generation.current;
        try {
            const chosen = [...input];
