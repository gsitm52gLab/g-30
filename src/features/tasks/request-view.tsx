...
export function RequestView({ content:c, catalog, files, taskId }: { content:PublicContent;catalog:TaskCatalog;files:TaskDetail["files"];taskId:string }) {
    ...
    {catalog.products.length>0&&<section className={s.panel}><h2>상품정보</h2><div className={s.row}>{catalog.products.map(p=><Link className={s.link} key={p.id} href={`/products/${p.id}?context=${encodeURIComponent(catalog.contexts.find(x=>x.id===catalog.tasks.find(t=>t.id===taskId)?.contextId)?.id??"")}`}>{p.name} ↗</Link>)}</div></section>}</div>;
}
