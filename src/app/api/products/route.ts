...
export async function POST(request: Request) { return (await route(request, async (identity, token) => json(await new ProductService(identity).create(token, await readBody(request, ["contextId", "brandId", "common", "fields", "idempotencyKey"], 262144)), 201))); }
