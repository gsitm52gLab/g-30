    ...
    return (await route(request, async (identity, token) => {
        ...
        if (last) {
            if (params.has('after') && params.get('after') !== last)
                fail('VALIDATION', 422, '실시간 연결 위치를 하나만 지정해 주세요.');
        ...
        let closed = false, heartbeat = 0, timer: ReturnType<typeof setTimeout> | undefined, wake: (() => void) | undefined;
        const stop = () => { closed = true; if (timer)
            clearTimeout(timer); wake?.(); };
            ...
            async start(controller) {
                ...
                try {
                    while (!closed && Date.now() - started < 25000) {
                        ...
                        const page = await service.events(token, id, params, page => { if (closed)
                            return; if (Date.now() - heartbeat > 5000) {
