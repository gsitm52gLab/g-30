...
export function GET(request:Request,context:{params:Promise<{id:string}>}){return route(request,async(identity,token)=>{
    const url=new URL(request.url),mode=url.searchParams.get("mode")??"download";
    if(!["download","original","preview"].includes(mode))fail("VALIDATION",422,"파일 동작을 확인해 주세요.");
    const {bytes,metadata}=await new FileService(identity).download(token,(await context.params).id,url.searchParams.get("taskId")??"",mode as "download"|"original"|"preview");
