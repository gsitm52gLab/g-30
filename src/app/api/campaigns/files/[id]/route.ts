...
}) { return route(request, async (s, t) => { const q = query(request, ['versionId', 'submissionId', 'mode'], ['versionId', 'submissionId']), mode = q.get('mode') ?? 'download'; if (!['original', 'download', 'preview'].includes(mode))
