...
}) { return route(request, async (s, t) => { const q = query(request, ['versionId'], []); return json(await new CampaignService(s).detail(t, (await c.params).id, q.get('versionId') ?? undefined)); }); }
