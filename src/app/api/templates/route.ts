...
export function POST(request: Request) { return route(request, async (identity, token) => {
    ...
    if (body.command === "save") return json(await service.saveTemplate(token, body), 201);
    if (body.command === "apply") return json(await service.applyTemplate(token, body));
    if (body.command === "preview_apply") return json(await service.previewTemplate(token, body));
