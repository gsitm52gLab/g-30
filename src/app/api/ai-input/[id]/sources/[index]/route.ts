...
}) { return (await route(request, async (i, t) => { const p = await c.params, q = query(request, ['versionId'], ['versionId']); if (!/^[0-3]$/.test(p.index))
    fail('VALIDATION', 422, '자료 순서를 확인해 주세요.'); const r = await loadRequest(i, t, p.id, q.get('versionId')!); if (r.kind === 'text')
