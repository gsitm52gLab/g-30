...
export async function POST(request: Request, c: Context) { return (await route(request, async (s, t) => json(await new NoticeService(s).command(t, (await c.params).id, await readBody(request, ['command', 'content', 'expectedRevision', 'versionId', 'idempotencyKey'], 65536))))); }
