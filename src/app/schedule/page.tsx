...
export default async function Page({ searchParams }: {
    ...
    const q = await searchParams, w = await readWorkspace(await contextFromSearch(Promise.resolve(q))).catch(workspaceFailure);
    if (!w)
        return <StorageFailure />;
    if (!w.selected)
        return <section className="panel"><h1>일정</h1><p>접근 가능한 컨텍스트가 없습니다.</p></section>;
    const data = await scheduleData(w.selected.id).catch(workspaceFailure);
