...
export default async function CampaignPage({ params, searchParams }: {
    ...
    if (!valid) {
        ...
        try {
            ...
            data = { task, catalog, list, detail, catalogs: task.canManage ? await campaigns.catalogs(token, context) : null, products: task.canManage ? await Promise.all(task.task.data.productIds.map(pid => new ProductService(auth).detail(token, pid, context))) : [] };
        }
        catch (e) {
