...
export default async function TaskDetail({ params, searchParams }: {
    ...
    for(const [key,value] of Object.entries(search))for(const v of Array.isArray(value)?value:value===undefined?[]:[value])query.append(key,v);
    ...
    if(reference.kind==='invalid')return unavailableReference;
    ...
    if (!task){ if(reference.kind!=='none')return unavailableReference; notFound(); }
